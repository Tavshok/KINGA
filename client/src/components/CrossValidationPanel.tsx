/**
 * PURPOSE: Render the Cross Validation Panel interaction or visual surface within the KINGA client.
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <Badge className={`ml-auto text-sm px-3 py-1 ${
            ...
            summary.overallRiskLevel === 'medium' ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-200' :
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          ...
          <div className="p-3 bg-amber-50 dark:bg-amber-950/30 rounded-lg text-center border border-amber-200 dark:border-amber-800">
            ...
            <p className="text-xs text-amber-700 dark:text-amber-300 font-medium">Quoted Not Visible</p>
          </div>
          ...
          <div className="p-3 bg-red-50 dark:bg-red-950/30 rounded-lg text-center border border-red-200 dark:border-red-800">
            ...
            <p className="text-xs text-red-700 dark:text-red-300 font-medium">Suspicious</p>
          </div>
          <div className="p-3 bg-gray-50 dark:bg-muted/50 rounded-lg text-center border border-gray-200 dark:border-border">
            <p className="text-2xl font-bold text-gray-600 dark:text-muted-foreground">{summary.legitimateHiddenCount}</p>
            <p className="text-xs text-gray-700 dark:text-foreground/80 font-medium">Hidden/Internal</p>
          </div>
        </div>
      </Card>
      ...
      {suspiciousItems.length > 0 && (
        <Card className="p-5 border-red-200 dark:border-red-800 bg-red-50/50 dark:bg-red-950/50">
          <div className="flex items-center gap-2 mb-3">
            <AlertTriangle className="w-5 h-5 text-red-600" />
            <h3 className="font-semibold text-red-800 dark:text-red-200">Suspicious: Externally Visible Parts Quoted But Not Seen in Photos</h3>
          </div>
          ...
          <div className="space-y-2">
            {suspiciousItems.map((item, i) => (
              <div key={i} className="flex items-center justify-between p-3 bg-white dark:bg-card rounded-lg border border-red-200 dark:border-red-800">
        ...
        <Card className="p-5">
          ...
          <div className="grid md:grid-cols-2 gap-2">
            {confirmed.map((item, i) => (
              <div key={i} className="flex items-center justify-between p-3 bg-green-50/50 dark:bg-green-950/50 rounded-lg border border-green-100 dark:border-green-900">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Badge className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 text-xs">{Math.round(item.confidence * 100)}%</Badge>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-2">
            {legitimateHidden.map((item, i) => (
              <div key={i} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-muted/50 rounded-lg border border-gray-100">
                ...
                <div className="flex items-center gap-2">
                  {item.quotedCost && <span className="text-sm font-medium text-gray-600 dark:text-muted-foreground">{fmt(item.quotedCost * 100)}</span>}
        ...
        <Card className="p-5 border-primary/20 bg-primary/5/30">
          ...
          <div className="space-y-2">
            {visibleNotQuoted.map((item, i) => (
              <div key={i} className="flex items-center justify-between p-3 bg-white dark:bg-card rounded-lg border border-primary/10">
                <div className="flex items-center gap-2">
      ...
      {fraudIndicators.length > 0 && (
        <Card className="p-5 border-amber-200 dark:border-amber-800 bg-amber-50/30 dark:bg-amber-950/30">
          <div className="flex items-center gap-2 mb-3">
            <AlertTriangle className="w-5 h-5 text-amber-600" />
            <h3 className="font-semibold text-amber-800 dark:text-amber-200">Fraud Indicators from Cross-Validation</h3>
          </div>
      ...
      {recommendations.length > 0 && (
        <Card className="p-5">
          <div className="flex items-center gap-2 mb-3">
            <Shield className="w-5 h-5 text-indigo-600" />
            <h3 className="font-semibold text-gray-900 dark:text-foreground">Cross-Validation Recommendations</h3>
          </div>
          <ul className="space-y-2">
            {recommendations.map((rec, i) => (
              <li key={i} className="flex items-start gap-2 text-sm text-gray-700 dark:text-foreground/80">
