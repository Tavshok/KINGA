 ...
 * WCAG 2.1 AA compliant. Uses CSS variables from index.css — fully theme-aware.
 */
...
export function Phase3DecisionBox({ enforcement, aiAssessment }: Phase3DecisionBoxProps) {
  ...
  // Use CSS variable-based status colours defined in index.css
  const decisionMap: Record<string, { label: string; bgVar: string; textVar: string; borderVar: string }> = {
    APPROVE:                { label: "APPROVE",         bgVar: "var(--status-approve-bg)",  textVar: "var(--status-approve-text)",  borderVar: "var(--status-approve-border)" },
    FINALISE_CLAIM:         { label: "APPROVE",         bgVar: "var(--status-approve-bg)",  textVar: "var(--status-approve-text)",  borderVar: "var(--status-approve-border)" },
    REVIEW:                 { label: "REVIEW",          bgVar: "var(--status-review-bg)",   textVar: "var(--status-review-text)",   borderVar: "var(--status-review-border)" },
    REVIEW_REQUIRED:        { label: "REVIEW REQUIRED", bgVar: "var(--status-review-bg)",   textVar: "var(--status-review-text)",   borderVar: "var(--status-review-border)" },
    ESCALATE:               { label: "ESCALATE",        bgVar: "var(--status-reject-bg)",   textVar: "var(--status-reject-text)",   borderVar: "var(--status-reject-border)" },
    ESCALATE_INVESTIGATION: { label: "ESCALATE",        bgVar: "var(--status-reject-bg)",   textVar: "var(--status-reject-text)",   borderVar: "var(--status-reject-border)" },
    REJECT:                 { label: "REJECT",          bgVar: "var(--status-reject-bg)",   textVar: "var(--status-reject-text)",   borderVar: "var(--status-reject-border)" },
  };
  ...
  return (
    <div
      ...
      style={{
        border: `2px solid ${cfg.borderVar}`,
        borderRadius: "8px",
        padding: "16px",
    ...
    >
      <div className="flex items-start justify-between gap-4 mb-3">
        <div>
          ...
          <p className="text-2xl font-black" style={{ color: cfg.textVar, fontSize: "24pt" }}>
            {cfg.label}
          </p>
        </div>
        <div className="text-right shrink-0">
          ...
          <p className="text-2xl font-black" style={{ color: cfg.textVar }}>
            {confidence}%
          </p>
        </div>
      </div>
        ...
        <ul className="space-y-1 mb-3">
          {keyDrivers.slice(0, 3).map((d, i) => (
            <li key={i} className="flex items-start gap-2 text-sm" style={{ color: "var(--foreground)" }}>
              <span className="shrink-0 mt-0.5" style={{ color: cfg.textVar }}>•</span>
      ...
      {nextSteps.length > 0 && (
        <div
          ...
          style={{ background: "var(--muted)", borderLeft: `3px solid ${cfg.borderVar}` }}
        >
          <p className="text-xs font-bold uppercase tracking-wide mb-1" style={{ color: "var(--muted-foreground)" }}>
            Next Steps
          </p>
          <ul className="space-y-0.5">
            {nextSteps.map((s, i) => (
              <li key={i} className="text-xs italic" style={{ color: "var(--muted-foreground)" }}>
                {i + 1}. {sanitiseField(s)}
              </li>
            ))}
          </ul>
      ...
      {advisories.length > 0 && (
        <div className="mt-2 space-y-1">
          {advisories.map((a, i) => (
            <p key={i} className="text-xs" style={{ color: "var(--fp-info-text)" }}>
              ℹ {sanitiseField(a)}
            </p>
          ))}
        </div>
    ...
    >
      <div className="flex items-center justify-between mb-2">
        ...
        {completenessScore !== null && (
          <span
            ...
            style={{
              background: completenessScore >= 90 ? "var(--status-approve-bg)" : completenessScore >= 70 ? "var(--status-review-bg)" : "var(--status-reject-bg)",
              color: completenessScore >= 90 ? "var(--status-approve-text)" : completenessScore >= 70 ? "var(--status-review-text)" : "var(--status-reject-text)",
            }}
          >
            {completenessScore}%
          </span>
        ...
        {rows.map(({ icon, field, value }) => (
          <div key={field} className="flex items-start gap-1.5">
            ...
            <div className="min-w-0">
              <p className="text-xs font-semibold" style={{ color: "var(--foreground)" }}>{field}:</p>
              <p className="text-xs truncate" style={{ color: "var(--muted-foreground)" }}>{value}</p>
            </div>
          </div>
        ))}
      </div>
      {(phase1?.allCorrections ?? []).length > 0 && (
        <div className="mt-2 pt-2" style={{ borderTop: "1px solid var(--border)" }}>
          <p className="text-xs" style={{ color: "var(--fp-info-text)" }}>
...
function severityColor(sev: string): string {
  ...
  if (s === "severe" || s === "catastrophic") return "rgba(220,38,38,0.50)";
  if (s === "moderate") return "rgba(217,119,6,0.50)";
  return "rgba(5,150,105,0.50)";
}
...
function VehicleView({ view, components }: { view: "front" | "rear" | "side" | "top"; components: Array<{ name: string; severity: string }> }) {
  ...
  return (
    <div className="flex flex-col items-center">
      <p className="text-xs font-semibold mb-1" style={{ color: "var(--muted-foreground)" }}>{viewLabels[view]}</p>
      <svg viewBox="0 0 100 170" width="90" height="153" style={{ border: "1px solid var(--border)", borderRadius: "4px", background: "var(--muted)" }}>
        <rect x="10" y="30" width="80" height="130" rx="8" fill="none" stroke="var(--muted-foreground)" strokeWidth="1.5" />
        <rect x="20" y="15" width="60" height="25" rx="4" fill="none" stroke="var(--muted-foreground)" strokeWidth="1" />
        <circle cx="22" cy="155" r="8" fill="none" stroke="var(--muted-foreground)" strokeWidth="1.5" />
        <circle cx="78" cy="155" r="8" fill="none" stroke="var(--muted-foreground)" strokeWidth="1.5" />
        {relevant.map((c, i) => {
          ...
          return (
            <g key={i}>
              <rect
                ...
                stroke={severityColor(c.severity).replace("0.50", "0.90")}
                strokeWidth="1"
              />
              <text
        ...
        {relevant.length === 0 && (
          <text x="50" y="90" textAnchor="middle" fontSize="7" fill="var(--muted-foreground)">No data</text>
        )}
      </svg>
    </div>
  );
...
export function ComponentHeatmap({ aiAssessment, enforcement }: ComponentHeatmapProps) {
  ...
  return (
    <div className="rpt-page-break-avoid mb-4" style={{ border: "1px solid var(--border)", borderRadius: "8px", padding: "16px", background: "var(--card)" }}>
      <p className="text-sm font-bold mb-3" style={{ color: "var(--card-foreground)" }}>Component Damage Heatmap</p>
      {!hasData ? (
        ...
        <>
          ...
          <div className="flex items-center gap-4 flex-wrap">
            {[
              { color: "rgba(220,38,38,0.50)", border: "rgba(220,38,38,0.90)", label: "Severe" },
              { color: "rgba(217,119,6,0.50)",  border: "rgba(217,119,6,0.90)",  label: "Moderate" },
              { color: "rgba(5,150,105,0.50)",  border: "rgba(5,150,105,0.90)",  label: "Minor" },
          ...
          {phase2PhotoComponents.length > 0 && (
            <p className="text-xs mt-2" style={{ color: "var(--fp-info-text)" }}>
...
export function CostComparisonChart({ aiAssessment, enforcement, quotes }: CostComparisonChartProps) {
  ...
  return (
    <div className="rpt-page-break-avoid mb-4" style={{ border: "1px solid var(--border)", borderRadius: "8px", padding: "16px", background: "var(--card)" }}>
      <p className="text-sm font-bold mb-3" style={{ color: "var(--card-foreground)" }}>Cost Comparison</p>
            ...
            <div key={label}>
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex items-center gap-2">
                  {varStr && (
                    <span className="text-xs font-bold" style={{ color: varStr.startsWith("+") ? "var(--fp-critical-text)" : "var(--fp-success-text)" }}>
                      {varStr}
                    </span>
                  )}
              ...
              <div className="relative h-5 rounded" style={{ background: "var(--muted)" }}>
                {value !== null ? (
                  <div
                  ...
                  <div
                    ...
                    style={{
                      width: "100%",
                      background: "repeating-linear-gradient(45deg, var(--border), var(--border) 4px, var(--muted) 4px, var(--muted) 8px)",
                    }}
                  />
                )}
              </div>
...
export function PhysicsConsistencyGauge({ enforcement }: PhysicsConsistencyGaugeProps) {
  ...
  return (
    <div className="rpt-page-break-avoid mb-4" style={{ border: "1px solid var(--border)", borderRadius: "8px", padding: "16px", background: "var(--card)" }}>
      <p className="text-sm font-bold mb-3" style={{ color: "var(--card-foreground)" }}>
        Physics Consistency{" "}
      ...
      <div className="relative h-6 rounded overflow-hidden mb-2" style={{ background: "var(--muted)" }}>
        <div className="absolute h-6" style={{ left: "0%",   width: "30%", background: "var(--fp-critical-text)", opacity: 0.7 }} />
        <div className="absolute h-6" style={{ left: "30%",  width: "40%", background: "var(--fp-warning-text)",  opacity: 0.7 }} />
        <div className="absolute h-6" style={{ left: "70%",  width: "30%", background: "var(--fp-success-text)",  opacity: 0.7 }} />
        <div
        ...
        >
          <div style={{
            ...
            borderTop: "10px solid var(--foreground)",
          }} />
        </div>
      </div>
...
export function PhotoGallery({ aiAssessment, enforcement }: PhotoGalleryProps) {
  ...
  if (!photosDetected) {
    return (
      <div className="rpt-page-break-avoid mb-4" style={{ border: "1px solid var(--border)", borderRadius: "8px", padding: "16px", background: "var(--muted)" }}>
        <p className="text-sm font-bold mb-1" style={{ color: "var(--foreground)" }}>Photo Gallery</p>
        <p className="text-xs" style={{ color: "var(--muted-foreground)" }}>No photos submitted with this claim.</p>
      </div>
    );
  }
  ...
  if (photosDetected && !photosProcessed) {
    return (
      <div className="rpt-page-break-avoid mb-4" style={{ border: "1px solid var(--fp-warning-border)", borderRadius: "8px", padding: "16px", background: "var(--fp-warning-bg)" }}>
        <p className="text-sm font-bold mb-1" style={{ color: "var(--foreground)" }}>Photo Gallery</p>
  ...
  return (
    <div className="rpt-page-break-avoid mb-4" style={{ border: "1px solid var(--border)", borderRadius: "8px", padding: "16px", background: "var(--card)" }}>
      <p className="text-sm font-bold mb-3" style={{ color: "var(--card-foreground)" }}>Photo Gallery</p>
  ...
  return (
    <div className="rpt-page-break-avoid mb-4" style={{ border: "1px solid var(--border)", borderRadius: "8px", padding: "16px", background: "var(--muted)" }}>
      ...
      <div className="tabular-nums text-xs space-y-1" style={{ color: "var(--foreground)" }}>
        ...
        <div className="pt-1">
          <span className="font-semibold" style={{ color: "var(--muted-foreground)" }}>Decision Trace:</span>
