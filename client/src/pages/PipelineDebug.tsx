  ...
  return (
    <div className="border border-gray-800 rounded-lg overflow-hidden">
      ...
      >
        ...
        <span className="text-xs font-mono text-gray-500">STEP {step.num}</span>
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { cls: string; label: string }> = {
    ...
    skipped: { cls: "bg-gray-600/20 text-gray-400 border-gray-600/30", label: "Skipped" },
    partial: { cls: "bg-amber-600/20 text-amber-400 border-amber-600/30", label: "Partial" },
    not_required: { cls: "bg-gray-600/20 text-gray-400 border-gray-600/30", label: "Not Required" },
    not_started: { cls: "bg-gray-600/20 text-gray-400 border-gray-600/30", label: "Not Started" },
    ...
    empty: { cls: "bg-gray-600/20 text-gray-400 border-gray-600/30", label: "Empty" },
  ...
  const cfg = map[status] || { cls: "bg-gray-600/20 text-gray-400 border-gray-600/30", label: status };
  return (
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 p-6">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-center gap-2 mb-6 text-xs text-gray-500">
          ...
          <span>/</span>
          <span className="text-gray-400">Pipeline Debug</span>
        </div>
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-4 px-4 py-3 rounded-lg bg-gray-900 border border-gray-800 text-sm">
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-400">Claim #{data.claimId}</span>
        </div>
      ...
      <Section
        step={stepConfig[0]}
        defaultOpen={true}
        badge={<Badge variant="outline" className="text-xs border-gray-700 text-gray-400">{data.documentRegistry?.length ?? 0} docs</Badge>}
      >
      ...
      <Section
        step={stepConfig[1]}
        badge={<Badge variant="outline" className="text-xs border-gray-700 text-gray-400">{data.rawTextOutputs?.length ?? 0} texts</Badge>}
      >
      ...
      <Section
        step={stepConfig[2]}
        badge={<Badge variant="outline" className="text-xs border-gray-700 text-gray-400">{data.structuredExtractions?.length ?? 0} fields</Badge>}
      >
...
function StageTimingBar({ stages, totalMs }: { stages: Record<string, any>; totalMs: number }) {
  ...
  return (
    <div className="px-4 py-3 rounded-lg bg-gray-900 border border-gray-800">
      <div className="text-xs text-gray-500 mb-2 font-mono">STAGE TIMING</div>
...
function DocumentRegistryTable({ docs }: { docs: DocumentEntry[] }) {
  if (docs.length === 0) return <p className="text-gray-500 text-sm">No documents found.</p>;
  return (
    <div className="space-y-3">
      {docs.map((doc, i) => (
...
function RawTextView({ texts }: { texts: RawTextEntry[] }) {
  ...
  if (texts.length === 0) return <p className="text-gray-500 text-sm">No text extracted.</p>;
  return (
    <div className="space-y-3">
      {texts.map((t, i) => (
        <div key={i} className="rounded bg-gray-900/50 border border-gray-800 overflow-hidden">
          <div className="flex items-center gap-3 px-3 py-2 border-b border-gray-800">
            ...
            <span className="text-xs text-gray-500">{t.textLength.toLocaleString()} chars</span>
            <span className="text-xs text-gray-500">{t.tableCount} tables</span>
            <span className="text-xs text-gray-500">OCR conf: {(t.ocrConfidence * 100).toFixed(0)}%</span>
            <button
          ...
          {expanded[i] && (
            <pre className="p-3 text-xs text-gray-400 font-mono whitespace-pre-wrap max-h-96 overflow-y-auto">
              {t.rawText || "(empty)"}
            </pre>
          )}
        </div>
...
function ExtractedFieldsTable({ fields }: { fields: ExtractedField[] }) {
  if (fields.length === 0) return <p className="text-gray-500 text-sm">No fields extracted.</p>;
  ...
  return (
    <div>
      <div className="flex gap-3 mb-3 text-xs">
        <span className="text-gray-500">{fields.length} fields total</span>
          ...
          <tbody>
            {fields.map((f, i) => (
              <tr key={i} className="border-b border-gray-800/50 hover:bg-gray-900/30">
                ...
                <td className="px-3 py-1.5 text-gray-500">{f.sourceDocument}</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    <div className="space-y-4">
          ...
          {validationIssues.map((issue: any, i: number) => (
            <div key={i} className="flex items-start gap-2 text-xs p-2 rounded bg-amber-950/20 border border-amber-900/30">
              ...
              <div>
                ...
                <span className="text-gray-400 ml-2">{issue.message}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {claimData && (
        <div>
          ...
          {showJson && (
            <pre className="p-3 rounded bg-gray-900 border border-gray-800 text-xs text-gray-400 font-mono whitespace-pre-wrap max-h-96 overflow-y-auto">
...
function EngineInputChecks({ checks }: { checks: EngineInput[] }) {
  if (checks.length === 0) return <p className="text-gray-500 text-sm">No engine input data.</p>;
  return (
    <div className="space-y-4">
      {checks.map((check, i) => (
        <div key={i} className="rounded bg-gray-900/50 border border-gray-800 overflow-hidden">
...
function EngineResultsView({ results }: { results: EngineResult[] }) {
  ...
  if (results.length === 0) return <p className="text-gray-500 text-sm">No engine results.</p>;
  return (
    <div className="space-y-3">
      {results.map((r, i) => (
        <div key={i} className="rounded bg-gray-900/50 border border-gray-800 overflow-hidden">
          <div className="flex items-center gap-3 px-3 py-2">
            ...
            <span className="text-xs text-gray-500 font-mono ml-auto">{(r.durationMs / 1000).toFixed(1)}s</span>
          ...
          {expanded[i] && Object.keys(r.outputData).length > 0 && (
            <pre className="px-3 py-2 border-t border-gray-800 text-xs text-gray-400 font-mono whitespace-pre-wrap max-h-64 overflow-y-auto">
...
function ReportSectionStatusView({ sections }: { sections: ReportSectionStatus[] }) {
  if (sections.length === 0) return <p className="text-gray-500 text-sm">No report sections.</p>;
  return (
    <div className="rounded border border-gray-800 overflow-hidden">
      <table className="w-full text-xs">
        <thead>
        ...
        <tbody>
          {sections.map((s, i) => (
            <tr key={i} className="border-b border-gray-800/50 hover:bg-gray-900/30">
              ...
              <td className="px-3 py-2 text-gray-500 text-[11px]">{s.dataSource}</td>
      ...
      {errors.map((err, i) => (
        <div key={i} className="p-3 rounded bg-red-950/20 border border-red-900/40">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] font-mono text-gray-600">· {err.component} · {err.errorType}</span>
          </div>
  ...
  return (
    <div className="space-y-4">
      ...
      {health.missingFieldsList.length > 0 && (
        <div>
          <div className="text-xs text-gray-500 mb-1 font-medium">Missing Fields</div>
          <div className="flex flex-wrap gap-1">
      ...
      {health.recommendedFixes.length > 0 && (
        <div>
          <div className="text-xs text-gray-500 mb-1 font-medium">Recommended Fixes</div>
          <div className="space-y-1">
