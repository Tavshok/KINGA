...
function drawKingaFooter(doc: JsPDFType, pageNum: number, totalPages: number, pageWidth: number, pageHeight: number): void {
  ...
  doc.text(`Generated by KINGA AI  ·  ${new Date().toLocaleDateString('en-ZA', { year: 'numeric', month: 'long', day: 'numeric' })}`, 10, footerY);
...
export async function generateComparisonPDF(data: ClaimData, currencySymbol: string = 'US$'): Promise<void> {
  ...
  const sigNote = 'This report has been generated by the KINGA AI system and is subject to review and authorisation by a qualified assessor. By signing below, the assessor confirms the accuracy of the AI-generated findings and takes responsibility for the final assessment decision.';
...
export async function generateDamageReportPDF(data: {
  ...
  doc.setFontSize(9);
  doc.setTextColor(80, 80, 80);
  const dmgSigNote = 'This damage analysis report has been generated by the KINGA AI system. By signing below, the assessor confirms review of the AI-generated damage findings and accepts responsibility for the final assessment decision.';
...
export async function generateClaimSummaryPDF(claim: {
  ...
  doc.text(`and quote comparison analysis, open Claim ${claim.claimNumber} in the KINGA AI platform.`, 23, yPos + 10);
  doc.setTextColor(0, 0, 0);
  yPos += 22;
