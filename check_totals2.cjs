...
async function main() {
  ...
  // Get last assessment with actual selected_quotes data
  const [assessments] = await conn.query(
    "SELECT id, cost_intelligence_json, created_at FROM ai_assessments WHERE claim_id = 6930001 ORDER BY id DESC LIMIT 10"
  );
  let found = null;
  for (const a of assessments) {
    const ci = JSON.parse(a.cost_intelligence_json || '{}');
    const sq = ci?.quoteOptimisation?.selected_quotes || [];
    if (sq.length > 0) { found = { id: a.id, ci, created_at: a.created_at }; break; }
  }
  if (!found) { console.log('No assessment with selected_quotes found'); await conn.end(); return; }
  const { id, ci } = found;
  const sq = ci.quoteOptimisation.selected_quotes;
  console.log('Assessment ID:', id, '| Created:', found.created_at);
  ...
  sq.forEach((q, i) => {
    ...
    console.log('   Difference:  ', diff.toFixed(2), Math.abs(diff) > 0.01 ? '<<< MISMATCH' : 'OK');
