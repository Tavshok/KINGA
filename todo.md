...
- [ ] AUD-P1-017 Claims Manager quote authority: replace the raw `quotes.byClaim` / `quotedAmount` lowest-quote reduction in ClaimsManagerComparisonView with canonical eligible ledger/L1/L2 evidence, while keeping non-active quotation history transparent and separate. Requires separate approval because it changes decision-facing professional UI authority.
- [ ] AUD-P1-018 Claims Manager lowest-quote cents display: the summary formats `quotedAmount` cents directly as dollars, overstating the actual rendered raw value by 100×; correct with AUD-P1-017 using a canonical evidence projection and tenant currency formatter. Requires separate approval because it changes a decision-facing professional display.
