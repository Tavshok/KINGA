...
- [ ] NCU-01 execution and closure: Tavonga Shoko separately authorized only the four hash-pinned NCU-01 statements on `KINGA-staging` through the owner’s local Claude Code environment. Hand off files 001–004 in ordinal order with mandatory immediate hash verification; stop/report on any tool, server, hash, privilege, or unexpected-result error, never retry blindly; do not use `mysql < file`, alter grants, or take any action outside NCU-01. After the owner reports execution, independently collect authenticated read-only postflight proving exactly the one expected prior index removal and three expected unique pair-index definitions, unchanged target structures, zero duplicate pairs, and unchanged zero-row baseline before closure. The sole-role exception expires at NCU-01 closure, stop, abandonment, or approved-window expiry; no broader staging or production authority transfers.
