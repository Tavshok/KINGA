...
- [ ] Separate application correction required: `server/services/tenant-config.ts` currently checks and updates `tenantRoleConfigs` by `tenantId` alone even though its public mutation receives a specific `role`. This can update all role rows for a tenant rather than only the requested role. Trace and correct in a separate authorised branch with a tenant/role-specific regression; do not bundle it into Gate C source-baseline work.
