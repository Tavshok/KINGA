...
- [ ] Correct the confirmed Vehicle Passport router runtime defect: `server/routers/vehicle-passport.ts` calls `inArray(...)` at line 91 without importing it from `drizzle-orm`, producing the supplied reports’ `Vehicle passport unavailable — inArray is not defined` error; add a focused regression in a separately approved fix branch.
