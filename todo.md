...
- [x] DRV-012: Vehicle Passport now fails closed without a non-empty session tenant and denies tenantless/non-owner sensitive routes. Owned-fixture live regressions cover tenantless vehicle denial, tenantless-session denial, and unrelated-tenant denial. Published for review as the first commit in stacked PR #39; not merged.
- [x] DRV-011: Vehicle Passport aggregate, timeline, claim-history, signal, alert, confidence, damage, inspection, and renewal-risk reads now carry the requesting tenant. Owned-fixture live regressions prove tenant-A receives only tenant-A data despite same-registration tenant-B rows, while a qualifying invited insurer receives only its `invited`/`viewed`/`responded` pre-loss snapshots and not claim/fraud/decision/repair/policy/payment data. Published for review as the second commit in stacked PR #39; not merged.
- [x] DRV-010: Vehicle Registry now contains cross-tenant VIN/registration matches before aggregate or identity mutation, leaves the incoming claim unattached, and records a tenant-scoped system claim event (`vehicle_registry_cross_tenant_match_contained`). The owned-fixture live regression proves no mutation, no duplicate tenant-B vehicle, no claim attachment, and events only for tenant B. Published for review as the third commit in stacked PR #39; not merged.
- [x] Validated DRV-010/DRV-011/DRV-012 against live owned fixtures. Final focused suite: 6 files / 59 tests passed; server bundle and Vite production build passed. TypeScript branch diagnostics were 1,011 versus 1,015 on current main with no new categories. Full suite residuals were unchanged at 49 failed / 3 skipped, with 9,204 passed on the branch versus 9,197 on main. Published exactly three clear commits as review-only PR #39, stacked on prerequisite PR #38; no merge and no CL/CI/FR, report export, L1/L2/cost, SAR, cross-insurer-network, schema, DDL, or production-record change.
...
- [x] Package B step 1: built `server/reporting/sharedQuoteEvidencePresentation.ts`, sourced solely from canonical `ReportCostIntegrity`/resolved-record evidence. It renders one column per active eligible quote, explicit `Not quoted` cells, canonical gated L1/L2 values, and continuation tables with repeated headers; it performs no per-tier quote query, currency ranking, or L1/L2 recalculation. Published in review-only PR #40; not merged.
- [x] Package B step 2: added canonical legacy-only/incomplete-evidence presentation. Historical submissions remain auditable, concrete canonical blockers are shown, and comparison, savings, settlement, L1, and L2 values are withheld when active evidence is absent. Published in review-only PR #40; not merged.
- [x] Package B step 3: replaced the visible CL, CI, and FR quotation-comparison blocks with the shared component while leaving other layout and canonical/security/cost-eligibility logic intact. Published in review-only PR #40; not merged.
- [x] Package B step 4: added permanent parity regressions for legacy-only evidence and an owned live tenant active/reconciled two-quote fixture. The live test creates exact-ID rows, renders CL/CI/FR, asserts byte-identical shared sections containing active repairer columns, `Not quoted`, L1, and L2, and proves teardown. Focused suite: 4 files / 6 tests passed; server/Vite builds passed. PR #40 is open review-only with six commits; code-complete remains in progress and no merge occurred.
