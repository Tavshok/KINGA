...
- [ ] Gate C Wave 5 approval gate: approve or specify the identity contract for `insurer_tenants`, `iso_audit_logs`, `risk_register`, `routing_history`, `routing_threshold_config`, and `tenant_role_configs` before generating Wave 5 SQL. Once authorised, regenerate/review source-derived SQL and prove two composed loopback-only scratch replays; do not access staging, production, migration accounts, Gate D, or live data.
