...
- [ ] Managed-workspace local-commit review: inventory and classify all 31 commits unique to the managed workspace head before any reconciliation; distinguish meaningful source/evidence/preservation work from checkpoint, merge, or experimental history; present the summary to the owner and obtain explicit preservation/merge authority before merging current GitHub `main`. Do not reset, discard, rewrite, or modify database, staging, or production state during the review.

- [ ] Managed-workspace preservation-first reconciliation: owner authorized merging current GitHub `main` while preserving all 31 local commits and full backlog history; no reset or discard. Stop and present the exact conflicting hunks before resolving any conflict in runtime code, `drizzle/schema.ts`, `.project-config.json`, or `todo.md`, including any apparently trivial schema conflict. Resolve only non-protected mechanical conflicts, then verify the reconciled result and create a new checkpoint.
