...
- [ ] Diagnose and repair the live portal failure `TypeError: Failed to fetch dynamically imported module …/assets/Home-…js`; verify that the published HTML and all referenced frontend assets are version-consistent and that the portal loads on desktop and mobile viewports without changing claim, report, or business logic.
...
- [ ] Publish the merged PR #64 asset set and verify the live portal serves the updated Claims Intelligence report code rather than stale generated HTML or retired frontend chunks.
