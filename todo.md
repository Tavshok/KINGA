...
- [ ] PR #17 updated-base resolution: merged current `main` into `fix/claim-comment-tenant-authority`; the sole conflict was an append-only `todo.md` collision resolved by retaining both histories. Re-run the comment tenant-authority regression, full-suite baseline comparison, and production builds before returning the PR for user-controlled merge.
