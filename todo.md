...
- [ ] **Intermittent report timing assertion in CI (20 September 2026):** `server/reports.test.ts` asserts `metadata.dbQueryTime > 0`, but a fast query can legitimately measure `0 ms`; this produced one failed hosted quality-gate run for PR #134 and passed on a clean rerun without source changes. Reconcile it as a separate, test-only package: preserve the metadata contract while replacing the non-deterministic strictly-positive expectation with an appropriate deterministic assertion. Do not bundle it into WorkOS, Render, or feature work.
