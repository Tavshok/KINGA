...
- [ ] AUD-P1-016 tenant-rate override loader: `server/db.ts` calls `getTenantRates` without importing the function exported by `server/db/intelligence-db.ts`; the non-fatal catch currently falls back to defaults and skips configured tenant overrides. Requires separate approval for a minimal runtime correction and regression proof.
