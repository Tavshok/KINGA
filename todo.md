...
- [ ] **WorkOS Package G proposal — service identity separation and human eligibility (19 September 2026):** Review-only proposal at `docs/authentication/workos-package-g-service-identity-separation-proposal-2026-09-19.md` defines a future KINGA-only opaque service-principal capability model to replace the `cron_` Manus `GetUserInfoWithJwt` bridge route by route. It requires additive durable credential metadata only, one-time clear-secret delivery, memory-hard verification, exact per-route scopes, expiry/rotation/revocation, sanitized auditing, no human-cookie/provider fallback, and explicit scheduling concurrency/idempotency design. It also introduces a closed WorkOS human-eligibility allow-list: `agency_restricted_proxy`, service/scheduled, QA, synthetic, unknown, malformed, or conflicting identities deny; a row referenced as `agency_assisted_claimant_identities.restricted_claimant_user_id` is permanently excluded even if other fields change. A separately verified claimant is denied unless a future owner policy classifies that distinct user as human and it passes every existing Package D gate. The proposal does not create any credential, code, schema, migration, WorkOS configuration, feature activation, scheduler, data action, deployment, staging, or production change. Owner decisions remain required for storage/algorithm, initial routes, credential lifetime/delivery/rotation, verified-claimant policy, and `cron_` retirement. Independent proposal review and source review publication remain required.
- [x] **WorkOS Package G proposal review remediation (19 September 2026):** The first independent review blocked the initial agency exclusion shortcut because existing `linked_to_verified_claimant` records intentionally place the same verified user ID in both current association fields, and the current schema cannot lock absent/changed associations safely during WorkOS linking. The revised proposal invalidates that shortcut: all legacy/shared-pointer associations remain deny-by-default (`legacy_ambiguous`) until owner-approved reconciliation; future WorkOS linking requires a non-null, audited `human_auth_eligibility` state, explicit proxy-versus-human agency semantics, indexed association predicates, and a transaction-wide serializable or reviewed next-key lock protocol. It also makes a shared durable `scheduled_job_leases` execution contract a prerequisite for any service route, so in-process and HTTP triggers cannot double-run a mutation. A fresh independent re-review remains required before source-review publication; no source/schema/data/credential/activation action occurred.
- [x] **WorkOS Package G final independent proposal review (19 September 2026):** Final security/architecture review approved the revised review-only proposal with no required changes. It confirmed that the proposal no longer overclaims current controls, remains default-off with Manus human login and KINGA local sessions active, requires deny-by-default human eligibility plus explicit association semantics/indexed locking before WorkOS activation, and handles stale scheduler takeover through a monotonic fencing generation, conditional ownership tuple, per-window effect idempotency, and transactional outbox. It verified explicit adversarial proof requirements for paused-worker takeover, duplicate scheduler paths, and agency/eligibility callback races. Governance, migration, credential, route, scheduler-of-record, network, and activation decisions remain owner gates. Package G is ready for documentation review publication only; no implementation is authorized.
