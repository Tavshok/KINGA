...
- [ ] PR #17 updated-base resolution: merge current `main` into `fix/claim-comment-tenant-authority`, resolve only a non-judgmental conflict after inspecting exact hunks, then re-run the comment tenant-authority regression, full-suite baseline comparison, and production builds before returning the PR for user-controlled merge.
