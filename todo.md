...
- [x] Standing-approved server administrative role-contract correction: replaced verified direct administrative comparisons in intelligence, fleet, insurance, and inspection routes with `isAdminRole()`; a deterministic regression confirms admin/platform-super-admin parity and the final server scan leaves the direct comparison only in the shared helper definition. Evidence: `audit/server-admin-role-contract-correction-2026-08-16.md`.
