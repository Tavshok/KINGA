...
- [ ] Restricted assisted-claimant authority enforcement: deny unregistered agency-assisted claimant identities independent portal, communication, document-control, financial, settlement, fraud, and non-claim report authority across relevant routes; add route-level regressions.
...
- [ ] Restricted assisted-claimant verified-link workflow: implement an explicit audited verified-link procedure with authorized actor checks, identity status transition, linkedBy/linkedAt preservation, and regression proof that ordinary claimant capability is not granted before completion.
- [ ] Restricted assisted-claimant non-claim route inventory: enumerate claimant-accessible fraud, financial, document-control, and communication actions; apply the reusable restricted guard to every applicable route and add a route-matrix regression proving only the agency-assisted canonical claim workflow remains available.
