...
- [ ] Approved report-tier canonical-layer remediation: inventory all CL, CI, and FR renderer raw queries/direct reads/duplicate derivations; extend only the canonical resolver where a field is demonstrably absent; migrate each tier in a separate reviewable commit; add live-data shared-field parity coverage; run full-suite comparison and both builds; hold `fix/report-tier-canonical-bypasses` without PR or merge. Do not touch authentication, database-access ownership, schema, migrations, data, or non-report paths.

- [ ] Approved explicit forensic-model dependency decision: review the separately held ForensicReportModel branch for incomplete or blocked work, incorporate only its independently validated canonical contract fields in a traceable commit before FR migration, and add value-level audit-event and dispute parity assertions to the live-data cross-tier regression. Keep the forensic structural split deferred pending staging.
