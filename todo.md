...
- [x] D-01 runner-account lifecycle finding: read-only `SHOW GRANTS FOR 'd01_runner'@'%'` reported no such host-qualified principal, and `mysql.user` lookup for exact user `d01_runner` returned an empty set. On current evidence, there is no extant `d01_runner` account to retain, reuse, or drop; no account or grant change was made. A differently named historical runner was not inferred.
