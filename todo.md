...
- [ ] Gate D authority gate: do not inspect, connect to, or change any staging/production environment until the owner separately approves a named staging target, read-only recovery/target readiness inspection, named account administrators/operators/approvers, and the first packet-specific action. Production requires separate later authority.
