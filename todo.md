...
- [ ] AUD-P1-019 Claims Manager KINGA assessment cents display: remove the `estimatedCost` double scaling before the tenant formatter so 150,000 stored cents display as USD 1,500.00. Requires separate approval for a professional presentation correction.
- [ ] AUD-P1-020 Claims Manager KINGA-versus-assessor variance: compare assessment and assessor amounts on one shared cents/dollars basis so USD 1,000 versus USD 1,500 displays -33.3%, not -99.3%. Requires separate approval with AUD-P1-019.
...
- [ ] AUD-P0-002 assessor evaluation tenant/identity authority: derive assessor identity from authenticated assignment, scope claim read/write to tenant and object authority, and persist tenant ID before accepting any evaluation submission. Must be corrected before assessor workflow extension.
- [ ] AUD-P1-021 assessor report evidence lifecycle: design and implement a claim/tenant/assessor-bound source-report lifecycle that distinguishes native upload and KINGA-assisted draft, preserves original files and metadata, and provides traceable extraction output. Requires schema-first approval.
- [ ] AUD-P1-022 assessor report attestation/review/version authority: add assessor attestation, review decision, reviewer, timestamp, version, and supersession model; project only the latest accepted report to the authoritative assessor evaluation summary. Requires schema-first approval.
- [ ] AUD-P1-023 assessor report review routing: route submitted assessor reports to the assigned claims assessor, with claims-manager fallback/escalation and in-app notification only; make review queue, decision, and reasons auditable. Requires schema-first approval.
