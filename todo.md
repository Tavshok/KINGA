...
- [ ] Open-PR updated-base sweep: inventory all open conflicted PRs; resolve security/tenant authority branches before report/policy, then documentation; halt only the individual branch requiring a substantive logic decision; re-verify each mechanical resolution against live TiDB, full-suite baseline, and production builds without merging any PR.
