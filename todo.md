...
- [ ] Gate B only — repair and prove the migration chain in uniquely named disposable scratch databases, prioritising `claims` and `ai_assessments`, then `claim_comments`, `governance_notifications`, and `rate_limit_tracking`. Preserve the 14 active-but-ambiguous declarations as excluded. Do not create a staging/production migration account, access/alter `kinga_staging` or production, or generate Gate C staging baseline waves.
