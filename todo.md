...
- [ ] D-03 review packet: prepare only the 50-table marker-aware source packet, canonical per-statement hash ledger, fresh read-only snapshot record, and source-derived privilege-gap assessment. Do not connect for preflight beyond approved read-only inspection; do not execute SQL, alter accounts/grants, change networking, restore backups, seed data, deploy, or touch production without later explicit authority.
