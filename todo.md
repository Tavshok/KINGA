...
- [ ] D-04 execution authority: owner authorised D-04 only and accepted a sole-operator/reviewer/application-validation-owner exception for Tavonga Shoko. Historical ordinals 1–98 were owner-reported as hash-verified accepted partial execution; a source compatibility stop at ordinal 99 superseded the original 40-table/7-FK/88-index/135-statement source. The only pending source is the revised 40-table/7-FK/87-index/134-statement source and its 36-file historical-ordinal 100–135 resumption ledger. Before any resumption, collect a new same-day snapshot and exact read-only preflight showing the 73-table baseline remains structurally unchanged and empty, the recorded partial D-04 state matches exactly, and no excluded `idx_recipients` index exists. Resume only original ordinals 100–135 through Claude Code, hash-verifying each and stopping on any mismatch or server error without blind retry. Independently verify 113 tables, all 7 D-04 FKs, all 87 D-04 explicit indexes, and zero rows in every new table before closure. The exception expires at D-04 closure, stop, abandonment, or the approved window’s expiry; no production action is authorised.
