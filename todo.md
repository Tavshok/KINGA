...
- [ ] Authorised staging restore rehearsal: capture the latest KINGA-staging snapshot timestamp and console restore option; use a newly named disposable non-production instance only; record Approver as KINGA owner and Operator as Manus AI acting under explicit owner authority; obtain final confirmation immediately before console submission; do not modify `kinga_staging`, production, or any schema/data.
- [ ] Authorised rehearsal cleanup: delete only the confirmed disposable instance `kinga-staging-restore-rehearsal-20260911` through the owner’s TiDB Cloud console, then capture evidence that it is gone while `KINGA-staging` and `KINGA-production` remain separate and Active. Do not delete or change either original instance, run SQL, or perform any schema/data action.
