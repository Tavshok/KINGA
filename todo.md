...
- [ ] Complete the approved governance, Stage 6, and DRV-006 through DRV-015 remediation programme phase by phase, using isolated review branches, exact scope checks, tenant-safe owned-fixture regressions where applicable, baseline-aware validation, readability/maintainability review, and decision gates for policy, legal, evidence, or portability questions.
- [x] Conducted a read-only discovery of the workflows that produced the DRV-009 police-report and DRV-015 agency service-request orphan records. Both retained rows are high-confidence fixture artefacts; the normal user workflows fail closed, but shared-database test/direct-write paths remain able to create retained orphans if cleanup fails. Evidence, confidence, unknowns, and decision boundaries are in `audit/orphan-workflow-discovery-2026-09-08.md`; no row, schema, or application behavior was modified.
- [x] Phase 1 — Reconfirmed and deleted only the three PR #58-confirmed orphan fixture rows (`police_reports` IDs 3 and 510001; `agency_insurance_service_requests` ID 1). The locked pre-delete gate matched exactly; no FK or known downstream references existed; three `audit_trail` entries were recorded; exact IDs and target-table orphan counts were zero afterward. Evidence: PR #59.
- [x] Phase 1a — Completed the user-authorised, read-only PR #58 parent-absence and exact fixture-fingerprint identification query. It returned exactly two police-report and one agency service-request record, all matching the documented fixture attribution without ambiguity.
- [x] Phase 2 — Hardened the identified police-report and Vehicle Passport fixture lifecycles without production-workflow change. The root cause, exact-ID cleanup helper, valid agency-client fixture parent, residue verification, on-demand known-pattern check, and validation are documented in PR #60. Focused real-database tests passed 12/12; the on-demand integrity check returned zero residue; build passed.
- [ ] Close Phase 1 of the remaining open-items programme: Package D server/A4 validation is complete—current-main focused server tests passed 14/14; generated one/three/four active plus legacy-only state PDFs were populated A4 documents; the specified legacy document rendered read-only to 12 populated A4 pages; and the Vehicle Passport/panel regression passed 7/7. Record: PR #61; implementation: merged PR #47. **Pending only:** fresh authenticated browser interaction through the actual Claims Intelligence view for the specified quote states; the sandbox browser is currently at the KINGA sign-in page with no authenticated tenant session.
- [ ] Investigate and repair the user-demonstrated Claims Intelligence portal print defect: a blank opening page and print output limited to the visible iframe viewport. Peer comparison confirmed that CI alone retained a fixed-height iframe while the Forensic viewer synchronised to the full same-origin document. PR #62 adds full document-height measurement, resize tracking, and child-window print sequencing; focused tests passed 3/3 and builds passed. **Pending only:** authenticated post-merge browser smoke test of the demonstrated report to confirm the actual print dialog contains the full document without a blank opening page.
- [x] Reviewed and merged every then-open KINGA pull request (#49 through #62) through normal merge commits after scope, ancestry, whitespace, validation-evidence, and mergeability review. The post-merge detached-main validation passed 54/54 selected security, reporting, Stage 6, fixture, and print regressions, and the production Vite/server build passed. No PR remained open; source branches were preserved. The known repository TypeScript baseline and unrelated BulkValuation warning remain outside this merge set.
- [ ] Complete the authenticated post-merge browser smoke test for the Claims Intelligence print path introduced by merged PR #62, using the demonstrated report or an authorised equivalent. Confirm the opening page is populated and the print document contains the full report rather than the portal viewport; investigate a further fix only if this exact check fails.
- [ ] Publish a documentation-only merge-governance record for PRs #49–#62 that accurately distinguishes their post-merge focused validation from the GitHub `UNSTABLE`/`UNKNOWN` status observed at merge time, records that `main` had no configured branch-protection endpoint, and identifies the PR #62 browser test as a post-merge validation gate rather than silently claiming all hosted checks were clean.
- [x] Repaired and published the live portal dynamic-import failure. The published HTML now references a current entry bundle, the screenshot-reported retired `Home-Hg08J0L8.js` path returns a truthful 404 rather than SPA HTML, and a fresh browser bootstrap reaches the KINGA authentication page without the former missing-module error. No claim, report, or business logic changed in this asset-recovery repair.
- [ ] Blocking incident — recurring published `Home-…js` dynamic-import failure. Root cause: the portal has 132 lazy imports whose content-hashed JavaScript paths are replaced on deploy; fresh HTML has no-cache headers, but an already rendered entry can still request a retired hash. Merged PR #67 (`606d0a61`) preserves code splitting while keeping JavaScript entry/chunk paths stable. Focused test passed 1/1; frontend and full production builds passed; hosted TypeScript failure had 999 inherited diagnostics and none in changed files. **Pending:** publish this merge and confirm the live domain loads current and prior Home module paths without a dynamic-import failure.
- [x] Diagnosed and repaired the supplied Claims Intelligence PDF defects for `DOC-20260810-84080652` in merged PR #64 (`96f170d7`). The report no longer forces the blank opening-page break; it explains that the two submitted `ZWL`/`USD` documents are legacy evidence rather than active comparison evidence; it withholds conversion, ranking, lowest total, L1, L2, savings, and settlement figures truthfully; and its multi-page source ledger repeats an explicit audit-only context label. Focused report regressions passed 13/13, the production build passed, and a read-only current-source 12-page A4 render had populated opening/final pages. **Pending only:** publish the merged asset set and complete the authorised portal/browser smoke check.
- [x] Published the merged PR #64 asset set and verified the live domain serves a new entry bundle while retired frontend assets return truthful 404s. A fresh browser bootstrap reaches authentication without the prior dynamic-import error.
- [ ] Verify the dynamic-import recovery on both desktop and mobile viewports, confirming that each freshly loaded portal reaches authentication without requesting the retired Home module.
- [ ] Verify the live deployed Claims Intelligence route serves current PR #64 output rather than stale generated HTML or cached/retired chunks, then complete the authorised actual print/browser smoke test before closing deployment-related CI report work.
- [ ] Diagnose and repair the user-confirmed post-publication Claims Intelligence failures: browser print still has a blank first page; the insurer-facing report shows an evidence-audit ledger rather than an intelligible side-by-side quote-line comparison; and the demonstrated report’s L1/L2 absence must be traced through canonical evidence before any presentation decision. Do not invent values or mutate claim, quote, or workflow data during diagnosis.
- [ ] Partially remediated the shared browser-print pagination failure in merged PR #65 (`c3425ab3`). Chromium reproduction proved the parent portal’s iframe print path emits a blank first page before embedded report content; the repair routes Claims Intelligence and Forensic printing through the same-origin report document. Focused routing and CI pagination regressions passed 2/2; production build passed; hosted check failure was confined to inherited unrelated TypeScript diagnostics. The standard Claims Report direct parent-window path has not yet been reproduced or validated and remains a separate required gate.
- [ ] Publish the merged PR #65 asset set and run authorised browser smoke tests for Claims Intelligence, Forensic, and standard Claims Report printing, confirming populated first pages and complete-document output for each.
- [ ] If the standard Claims Report still produces a blank first page after PR #65 publication, reproduce its direct parent-window print path and implement a bounded report-specific fix with regression coverage; do not claim the multi-report pagination incident closed before this gate passes.
- [x] Merged PR #66 (`c02f65a6`): the insurer portal’s parent `Print / Export PDF` route now delegates Claims Intelligence and Forensic output to their embedded report documents, rather than printing the portal. Shared behavior tests passed, the production build passed, and the hosted failure was limited to inherited unrelated TypeScript diagnostics. **Pending only:** publish and execute the report-type browser smoke tests.
- [x] Merged PR #66 (`c02f65a6`): aligned report-rendered Vehicle Passport snapshots to a neutral shared evidence-panel treatment with consistent teal/green KINGA design tokens and print-safe boundary styling. No vehicle, provenance, tenant, or evidence data changed. **Pending only:** visual browser verification after publication.
- [ ] Publish merged PR #66 and run authorised browser smoke tests through the insurer portal-level `Print / Export PDF` control for Claims Intelligence and Forensic reports. Confirm populated first pages, complete-document output, and no leaked portal controls.
- [ ] Visually verify the live Vehicle Passport section in Claims Intelligence and Forensic reports after PR #66 publication, confirming shared neutral evidence-panel styling, consistent colour semantics, and print-safe presentation.
- [ ] Run an authorised browser smoke test through the actual Claims Intelligence report print path after publication. Confirm a populated first page, no avoidable near-final blank page, complete-document output, and clear mixed-currency/L1-L2 explanation. If output diverges from the repaired current-source render, trace stale/persisted report HTML versus current preview generation and add a regression for that path.
- [x] Conducted a read-only comparative quality audit of the supplied Claims, Claims Intelligence, and Forensic PDFs for `DOC-20260810-84080652`. The evidence-based defect register and bounded remediation plan are recorded in `audit/three-report-defects-and-remediation-2026-09-09.md`. The audit confirms CI blank-page/iframe print failure, FR embedded-print/scroll-control risk, cross-tier quote-state communication defects, and Vehicle Passport visual inconsistency; it does not alter claim, quotation, evidence, workflow, report, or database data.
- [ ] Execute the approved multi-report repair programme in phases: restore live asset reliability; implement a dedicated CI/FR full-document print contract; unify cross-tier quote-comparison and L1/L2 explanation; standardise Vehicle Passport and targeted pagination; then run focused regressions, builds, and authorised browser acceptance checks before publishing each completed phase.
- [x] Restored and verified the KINGA development server. It returned HTTP 200 with a 369,587-byte application HTML response at the local root; the pending deployment and checkpoint reconciliation state remain unchanged.
- [ ] Reconcile the managed workspace with merged `main`, preserve the current backlog records, create a publishable checkpoint, then verify the deployed portal serves the dynamic-import and Claims Intelligence PDF-quality repairs.
- [ ] Resolve the managed checkpoint synchronisation conflict between the local Phase 1 audit/backlog records and merged GitHub PR #68, preserving both documentation histories before creating the Phase 2 publishable checkpoint. No report business data or production record may be changed.
- [x] Completed the read-only preservation audit before any support-led reinitialisation. The only non-ignored uncommitted artefacts are the managed backlog (`todo.md`) and `audit/live-asset-phase1-2026-09-09.md`; both are documented local history. There is no untracked source, test, configuration, report-generator, database, or user-data file sitting only in the workspace session. Ephemeral build/cache logs remain ignored and are not preservation candidates.
- [x] Performed the final managed-checkpoint diagnostic attempt. It failed identically: the checkpoint service fetched GitHub `main` at merged PR #68 (`8ef77b9c`) then stopped on a merge conflict with the local workspace. No overwrite, reset, deletion, source change, database change, or deployment occurred; the exact mechanism is recorded in the task handoff.
- [ ] Verify the platform-repaired managed repository integration by saving a new neutral checkpoint. Confirm whether the checkpoint succeeds without overwriting the managed backlog or merging/changing the separate Gate C review branches; record the exact resulting checkpoint version and status.
- [ ] Establish a secure, staging-only connection to the user-owned KINGA TiDB environment; verify database identity, TLS, least-privilege permissions, schema inventory, and application compatibility through read-only checks before any production onboarding, data migration, schema change, or deployment configuration change.
- [x] Resolved PR #70 against current `main` in an isolated worktree. The only conflict was a mechanical `todo.md` adjacency between completed Phase 3 and Phase 4 records; both were retained. Focused report safeguards passed 14/14, production build passed, TypeScript remained 999 inherited diagnostics with no new category, and the branch added no full-suite failure identifier against current main. Updated review head: `de7370dc`; GitHub conflict state cleared and its fresh hosted check is running.
- [ ] Preserve the three specified workspace-only documents, verify their GitHub-main presence/absence, and prepare a fresh workspace baseline from current `main` without resetting, deleting, or overwriting the managed workspace.
- [x] Provided an evidence-based current-state account of GitHub repairs, the managed-workspace synchronisation block, the preserved local document set, and the safe recovery options before any Replit import or staging connection action. The current GitHub main, PR #68/#69/#70 states, and preservation archive were rechecked directly.
- [ ] Retrieve the affected KINGA Manus task/project public link for the support request, requesting explicit confirmation before enabling public access if it is not already enabled.
- [x] Produced a source-backed inventory of KINGA’s active third-party services and external APIs, including ownership/credential boundary, runtime purpose, loss-of-service impact, and portability exposure without disclosing secrets. Evidence is in `/home/ubuntu/kinga-external-dependency-portability-inventory-2026-09-10.md`; it distinguishes active configured services from implemented-but-unconfigured and planned paths.
- [x] Tested KINGA staging TiDB connectivity through the dedicated least-privilege account only. The final read-only smoke test passed 3/3: database `kinga_staging`, TiDB Serverless `v8.5.3`, TLS cipher `TLS_AES_128_GCM_SHA256`, `SELECT`-only grants, and an empty metadata-only table inventory. No application table/row was read, and no schema, data, migration, production setting, or current managed `DATABASE_URL` changed.
- [x] Compared the empty `kinga_staging` metadata inventory read-only against KINGA’s checked-in application schema. Staging has zero visible tables while the source declares 221 distinct MySQL table names; the gap and safe options are documented in `/home/ubuntu/kinga-staging-read-only-verification-2026-09-10.md`. No DDL, migration, data copy, or production connection change was performed.
- [ ] Obtain an explicit decision on staging schema bootstrap/reconciliation before creating a temporary migration account, applying any DDL, copying data, or changing a production connection.
- [x] Prepared the review-only staging schema migration/reconciliation plan, including the complete 221-table source inventory, migration-chain/drift assessment, dependency-safe bootstrap waves, rollback/restore evidence, promotion gates, and a per-wave change-packet template. Evidence: `/home/ubuntu/kinga-staging-schema-bootstrap-reconciliation-plan-2026-09-10.md` and `/home/ubuntu/kinga-staging-schema-change-packet-template.md`. No DDL, migration, data movement, or production change was performed.
- [ ] Create a documentation-only GitHub review-branch manifest package: reproducible source-schema and migration-artifact manifests, an evidence-based classification ledger for every source-table declaration, and a scratch-replay design. Do not create DDL, a migration identity, a staging object, or a production change.
- [ ] Gate C (scratch-only): verify the 14 candidate required-baseline tables against genuine current runtime paths and operational state, align `claim_comments` with the live `claimId` physical contract, and generate/replay reviewed baseline waves only for verified tables. Do not create a staging/production migration account, alter `kinga_staging`/production, or make an unverified candidate part of a baseline wave.
- [x] Close Phase 2 after confirming Package C publication status. PR #45 (`fix(reports): share canonical L1 L2 state presentation`) was merged into `main` on 2026-09-08 at `4b46e1e07be5c3663fbed59daa3f0364dc05ae96`; no duplicate publication or unnecessary reimplementation was made.
- [x] Close Phase 3 after tracing Approval Rate semantics and confirming existing work. Main contains `9b92099561508f8391b20a4f24e6befaa0179257` (`fix(reports): count completed claims in approval rate`); current live-database owned-fixture regression passed 1/1, proving `completed` is the approved-outcome status and executive/claims-manager display the identical 33.3% rate.
- [x] Close Phase 4 after accounting for Stage 6 PR #50. The open review branch contains one isolated commit (`0cd1b3a90b6db7a7719064a6f52a83d111fc688f`) restoring the severity/PDF-direct helper bindings broken by an earlier file split; its current direct regression suite passed 32/32 and its diff is whitespace-clean. No claim replay, pipeline rerun, deployment, or merge was performed.
...
- [x] Phase 3 multi-report repair: reconciled cross-tier quote comparison presentation against the canonical active-evidence and cost-integrity contracts. The dormant Claims inline renderer was removed; CL, CI, and FR now share one insurer-facing Cost Decision Summary and renderer-independent matrix. The matrix presents L1/L2 only from eligible active same-basis evidence and otherwise retains ledger/audit context while withholding unsupported values. Evidence: `audit/phase3-canonical-quote-evidence-2026-09-09.md`.
- [x] Phase 4 multi-report repair: consolidated CL, CI, and FR Vehicle Passport presentation onto a neutral shared evidence panel and applied targeted report pagination rules that preserve first substantive content, repeat table context naturally, and avoid sparse late pages without changing Vehicle Passport data or provenance rules. Evidence: `audit/phase4-vehicle-passport-pagination-2026-09-09.md`.
- [x] Gate B only — reproduced the historical chain failure and proved an immutable corrected overlay in two uniquely named local scratch databases. The evidence covers `claims`, `ai_assessments`, `claim_comments`, `governance_notifications`, and `rate_limit_tracking`; all 14 active-but-ambiguous declarations remain excluded. No staging/production migration account, `kinga_staging`/production access, Gate C wave or external data operation occurred. Evidence: `audit/gate-b-scratch-replay-2026-09-10.md`.
- [x] Merged approved documentation-only PR #71 into GitHub `main` and verified the resulting mainline commit `f77b7a31ba691e0d38fcd8270f6f72692e6fedb3` before beginning any Gate B scratch replay work.
- [x] Option A only — created a documentation-only staging schema reconciliation package: reproducible source/migration manifests, an evidence-based classification of every source-table declaration, a foreign-key dependency inventory, and a scratch-replay design. The manifest verifier passed and confirms 242 production-schema declarations, 221 distinct MySQL/TiDB physical names, full ledger coverage and no required-baseline entry lacking direct non-test schema-import evidence. No DDL, migration-account creation, staging change, production change, or data movement occurred.
- [x] Gate B implementation constraint satisfied: historic journaled SQL and journal files remain byte-identical to merged main; the replay repairs are represented only in the guarded, checksummed scratch-only overlay and its retained evidence package.
- [ ] Gate B compatibility constraint: do not change the current application `claimComments.claimId` physical mapping solely to match the repaired fresh-overlay baseline. The managed runtime/full-suite target currently exposes legacy `claim_comments.claimId`; reconcile this live-versus-fresh-baseline divergence only in a separately authorised compatibility/migration decision.
- [x] Priority tenant-role configuration correction: `updateTenantRoleConfig()` now uses one `(tenantId, roleKey)` predicate for its existence lookup and update, while the absent-pair path inserts only the requested role. A collision-safe loopback-only scratch regression proves both a one-role update leaves every other role row unchanged and an absent-pair insert leaves existing roles unchanged; disposal was independently verified. Production build and exact-base DB-disabled TypeScript/full-suite comparisons are recorded in `audit/tenant-role-config-scope-correction-2026-09-11.md`. The DB-disabled legacy `tenant-config.test.ts` remains unavailable by design; the passing local scratch regression is the authoritative functional proof. No Gate C schema work, staging, production, migration account, Gate D, or live data was touched.
- [x] Priority tenant-role configuration publication: the independently rebased and revalidated correction is published as review-only PR #80 (`fix/tenant-role-config-scope`) against current main. It retains the service/test/report/backlog evidence and does not alter Gate C review branches.
- [ ] Gate C (scratch-only): verify `claim_comment_reads`, `tenant_usage_summary`, `tenant_tier_history`, `audit_logs`, `currency_exchange_rates`, `benchmark_deviations`, `adjuster_sign_offs`, `photo_reextraction_jobs`, `vehicle_models`, `measurement_types`, `vehicle_geometry_measurements`, `vehicle_landmarks`, `geometry_sources`, and `vision_calibration_results` against genuine current runtime paths and operational state. Align `claim_comments` to the live `claimId` physical contract. Generate/replay reviewed baseline waves only for verified candidates; do not alter `kinga_staging`/production, create a migration account, or generate a wave for an inactive, incomplete, mismatched, or untraced candidate.
- [x] Gate C Wave 1 decision resolved: `tenants.id` and `tenant_invitations.id` are declared primary keys; `users.openId` and `tenant_invitations.token` are real unique constraints; `idx_tenants_name` is retained as the sole tenants-name index. The source-derived scratch SQL contains no unexpected table, no `DROP`/`ALTER`, and no external target.
- [x] Gate C Wave 1 timestamp-default reconciliation: converted only `tenantInvitations.createdAt`, `tenants.createdAt`, `users.createdAt`, and `users.lastSignedIn` to `.defaultNow()`. Two independent guarded local scratch replays passed with identical SQL and structural-metadata fingerprints; both unique `kinga_gatec_*` databases were verified removed. Evidence: `audit/gate-c-scratch-baseline/wave-01-evidence/README.md`.
- [x] Gate C held-table decision report: documented the eight held candidates with exact current-path/operational evidence in `docs/staging-schema-reconciliation/gate-c-held-candidate-review.md`. Verified candidates are six, not five: `measurement_types` has a live Stage 6.5A path; `currency_exchange_rates` is verified and is not held.
- [x] Gate C Wave 2: reconciled the user-approved 16 auto-increment primary keys, explicit short `fk_vgm_vehicle_model`, and unnamed duplicate `claims.claimantId` index; generated/reviewed source-derived Wave 2 SQL and completed two prerequisite-composed `kinga_gatec_*` loopback scratch replays with identical structural metadata fingerprints and verified disposal. Evidence: `audit/gate-c-wave-two-review-2026-09-11.md`.
- [x] Gate C Wave 2 scratch execution: replayed the exact reviewed Wave 1 prerequisite SQL before Wave 2 in both fresh local scratch targets, retaining both SQL checksums and rejecting any prerequisite other than the three approved Wave 1 tables.
- [x] Gate C global timestamp pass: AST-verified and converted all 208 genuine `timestamp(...).default('CURRENT_TIMESTAMP')` source declarations, correcting the earlier line-based 173 count. The retained repository-only post-conversion inventory reports zero remaining occurrences, and the regenerated manifest verifier passed. No non-timestamp lookalike was changed.
- [x] Gate C global key-gap inventory: identified 145 auto-increment `id` source declarations outside Waves 1/2 without explicit `.primaryKey()` and recorded the complete table/declaration/line list in `audit/gate-c-scratch-baseline/auto-increment-primary-key-gaps-outside-waves-01-02.json`. No listed declaration has been changed pending a single future review decision.
- [x] Gate C Wave 2 validation follow-up: traced the `truthReconciliationEngine` idempotency assertion to the July 12 TRE introduction and verified it fails 5/5 on same-base current main and 5/5 on the Wave 2 branch. Wave 2 does not touch the engine or its test; the persistent mismatch is the test’s unstripped time-derived `certificateId`. Evidence: `audit/gate-c-global-primary-key-reconciliation-2026-09-11.md`.
- [x] Gate C global sole-key primary-key reconciliation: source-only AST classification accounted for all 145 inventory rows, added `.primaryKey()` only to 34 safe sole-identifier IDs, and retained 111 junction/composite/alternate-key candidates unchanged in a complete exception shortlist. Focused contracts (14/14), manifest verification, build, and touched-path TypeScript checks are recorded in `audit/gate-c-global-primary-key-reconciliation-2026-09-11.md`. No staging, production, migration account, or live-data operation occurred.
- [x] Gate C Wave 3 planning only: prepared the 50-table assessment/evidence/reporting dependency and source-key gate plan in `audit/gate-c-wave-three-planning-2026-09-11.md`. No Wave 3 SQL, scratch database, DDL, or external connection was generated or used; a fresh explicit generation/replay authority remains required.
- [x] Gate C global explicit-primary-key completion: all 111 residual auto-increment IDs passed the guarded source-shape compatibility proof (0 conflicts) and now explicitly declare `.primaryKey()`. The residual global auto-increment inventory is zero. The separate natural/composite-unique decision list is `audit/gate-c-natural-composite-unique-candidates-2026-09-11.md`; no unique constraint was added.
- [x] Gate C Wave 3 full baseline: generated and statically guarded the authorised 50-table assessment/evidence/reporting baseline, then completed two fresh `127.0.0.1:3317` `kinga_gatec_wave3_*` composed scratch replays with exact Wave 1/2 prerequisites, matching SQL/metadata fingerprints, and verified disposal. Evidence: `audit/gate-c-wave-three-review-2026-09-11.md`. No staging, production, migration account, or live data was accessed.
- [x] Gate C Wave 3 follow-up: corrected the existing agency `$returningId()` response paths to return the actual Drizzle object identifier exposed by explicit primary keys, and removed the pre-existing duplicate `rowIndex` property warning while retaining input row-index precedence. The mocked generated-ID regression and final focused suite pass; no data operation occurred.
- [x] Gate C natural/composite-unique decision support: published the four candidate relationship-table column sets and existing index evidence for a separate business-rule decision in `audit/gate-c-natural-composite-unique-candidates-2026-09-11.md`; no unique constraint was added.
- [x] Gate C Wave 4 planning: completed source-only planning for the requested 40-table operational portals/channels scope, including recovery and WhatsApp. The plan flags one missing key, one blank duplicate index, and the configured-source boundary for `claim_comment_reads` before any SQL generation or scratch database. Evidence: `audit/gate-c-wave-four-planning-2026-09-11.md`. No staging, production, migration account, or live data was accessed.
- [x] Gate C Wave 4: added `.primaryKey()` only to `tenant_workflow_configs.id`; removed only the blank duplicate `workflow_audit_trail` index while retaining `idx_audit_claim_timestamp`; consolidated the byte-equivalent `claim_comment_reads` declaration into canonical `drizzle/schema.ts` and removed the unreferenced supplementary duplicate schema. The manifest passed, reviewed 40-table SQL passed static guards, and two local composed Wave 1–4 replays created matching 113-table metadata then proved disposal. Evidence: `audit/gate-c-wave-four-review-2026-09-11.md`. No staging, production, migration account, Gate D, or live data was accessed.
- [x] Gate C Wave 5 final-wave source plan: identified the final 75-table intelligence/learning/analytics scope after the authorised Wave 4 reassignment of recovery/WhatsApp channels. Five existing string IDs lack explicit primary keys and `tenant_role_configs` has no source identity declaration; all other source key/index/dependency/held-table checks are clean. Evidence: `audit/gate-c-wave-five-planning-2026-09-11.md`. No SQL, scratch database, source schema, staging, production, migration account, Gate D, or live data was accessed.
- [x] Gate C Wave 5 final baseline: verified no active tenant-role version/history or duplicate tenant/role contract, added `.primaryKey()` only to the five approved existing string IDs, and added the authorised `tenant_role_configs` `(tenantId, roleKey)` composite key. Reviewed 75-table Wave 5 SQL passed static guards, and two composed `kinga_gatec_*` loopback replays created matching 188-table metadata then proved disposal. Evidence: `audit/gate-c-wave-five-review-2026-09-11.md`. No staging, production, migration account, Gate D, or live data was accessed.
- [x] Separate application correction: `server/services/tenant-config.ts` now scopes the tenant-role existence lookup and update to `(tenantId, roleKey)` and inserts only the requested missing pair. The dedicated collision-safe loopback regression proves role isolation and exact-target disposal; evidence is `audit/tenant-role-config-scope-correction-2026-09-11.md`. Published separately as PR #80; no Gate C source-baseline, staging, production, migration-account, or Gate D action is included.
- [x] Gate C final held-table disposition: retained the approved exclusions for `audit_logs`, `vision_calibration_results`, `geometry_sources`, `benchmark_deviations`, `tenant_usage_summary`, `tenant_tier_history`, and the confirmed development-only `vehicle_landmarks`; included only `photo_reextraction_jobs` as a post-Wave-5 source-derived supplemental baseline. The manifest and static guards passed, and two fresh 715-statement composed loopback replays created matching 189-table metadata then proved disposal. Evidence: `audit/gate-c-final-held-table-disposition-2026-09-11.md`. No staging, production, migration account, Gate D, or live data was accessed.
- [x] Priority tenant-role update investigation: traced `tenant.updateRoleConfig`, active UI callers, root registration, tenant-admin guard, and the tenant-only read/update predicate. The router/service are byte-identical to current `user_github/main`, but current staging/production deployment reachability remains unproven without separately authorised access. The `(tenantId, roleKey)` repair proposal and regression contract are recorded in `audit/gate-c-held-and-natural-key-deep-investigation-2026-09-11.md`; no application or schema code was changed.
- [x] Gate C held/natural-key deep investigation: completed repository-only per-table evidence review for the eight held candidates and four natural/composite-unique candidates, covering source contracts, Git history, non-test UI/API/runtime references, channels, flags, comments, and PR context. Recommendations are recorded in `audit/gate-c-held-and-natural-key-deep-investigation-2026-09-11.md`; no schema, application, database, or migration change was made.
- [x] Gate D preparation only: documented least-privilege migration-account scope, TiDB backup/PITR confirmation criteria, restore-rehearsal acceptance protocol, rollback decision gates, and six per-wave source/diff/validation change packets against the merged 189-table Gate C baseline. Evidence: `docs/staging-schema-reconciliation/gate-d-execution-readiness-plan.md`, `docs/staging-schema-reconciliation/gate-d-per-wave-change-packets.md`, and `audit/gate-d-preparation-status-2026-09-11.md`. No credential, staging/production connection, DDL/DML, backup/restore, migration account, or Gate D action occurred.
- [x] Gate D read-only staging target/recovery readiness inspection: authorised `kinga_verify` connected to `kinga_staging` over TLS 1.3 after the owner enabled TiDB Cloud AWS public-endpoint access. Its grants are `SELECT` only on `kinga_staging`; it sees zero tables. Owner console evidence identifies the target as free Starter, so TiDB's documented capability is daily one-day snapshot retention and restore to a new instance, with no Starter PITR. The latest snapshot timestamp/Backup-page action remains console-only evidence; no account, DDL/DML, backup, restore, migration, or production action occurred. Evidence: `audit/gate-d-readonly-staging-inspection-attempt-2026-09-11.md`.
- [ ] Gate D execution authority gate: before any staging change, obtain a recorded latest snapshot/restore-action confirmation, decide whether free Starter's one-day/no-PITR posture is acceptable or upgrade the recovery capability, name account administrators/operators/approvers, authorise a new-instance restore rehearsal, and approve a first packet-specific staging action. Production requires separate later authority.
- [x] Authorised staging restore rehearsal: the owner used the console-confirmed `2026-09-11 03:00:45 UTC` successful snapshot and Snapshot Restore option to create separate disposable Starter target `kinga-staging-restore-rehearsal-20260911` in AWS Frankfurt (`eu-central-1`). The target reached Active alongside unchanged Active `KINGA-staging` and `KINGA-production`; no SQL, DDL, DML, migration, import, or source-instance schema/data action occurred.
- [x] Authorised rehearsal cleanup: the owner deleted only `kinga-staging-restore-rehearsal-20260911` through the TiDB Cloud console. Final owner-supplied My TiDB resource-list evidence shows total `2`: only `KINGA-staging` and `KINGA-production`, each Active; the disposable target is absent. No SQL or schema/data action occurred.
- [ ] Gate D execution remains unauthorised: do not issue staging DDL/DML or data operations until the owner separately approves a specific named Wave 1 packet, constrained migration-account design, designated operator/reviewer, preflight/rollback conditions, and an explicit staging execution window. Production remains outside scope pending an Essential or Dedicated upgrade with real PITR and a separately authorised production plan.
- [x] Merged owner-approved documentation-only Gate D preparation PR #82 through the protected GitHub workflow. GitHub recorded merge commit `9773b417aa4de5d329cb91c8e634e260caa82d45`, and final evidence commit `241de39e9bd537c7589020f019f0001077707fab` is verified as an ancestor of `main`. This merge did not authorise database execution.
- [x] Drafted the Wave 1 staging change packet for `tenants`, `tenant_invitations`, and `users` from the approved template, naming the KINGA owner as provisional operator and reviewer. The document retains the mandatory independent-reviewer requirement and is submitted for review only; no migration account, staging access, DDL/DML, or execution authority has been requested or used. Evidence: `docs/staging-schema-reconciliation/gate-d-wave-01-staging-change-packet-draft-2026-09-12.md`.
- [x] D-01-only control exception: documented the KINGA owner’s explicit acceptance, dated 12 September 2026, that Tavonga Shoko may act as both operator and reviewer for this limited three-table, no-foreign-key, empty-staging D-01 packet. The exception expires at D-01 closure, preserves every other stop gate, and never applies to D-02–D-06 or production. Evidence: `docs/staging-schema-reconciliation/gate-d-wave-01-staging-change-packet-draft-2026-09-12.md`.
- [x] Prepared review-only D-01 preflight records: constrained verifier/runner capability design, current snapshot-evidence decision record, and UTC-bounded change-window decision record. The historical rehearsal snapshot is correctly recorded as expired and not reusable; a fresh current snapshot and a separately approved window remain prerequisites for any execution decision. No account was created, no credential used, no staging connection made, no SQL issued, and no D-01 execution occurred. Evidence: `docs/staging-schema-reconciliation/gate-d-wave-01-staging-change-packet-draft-2026-09-12.md`.
- [x] Merged owner-approved review-only D-01 preflight packet PR #83 through the protected GitHub workflow. GitHub recorded merge commit `a503f44e4cd61e1e8ec4e0f567222174106a62c3`, and packet head `90ed9e6467329f5433bcf438e21c9a7fd4f6d6a8` is verified as an ancestor of `main`. The platform `code-complete` failure was independently confirmed to be inherited TypeScript diagnostics: PR #83 changed only documentation/backlog paths and no TypeScript source or configuration. This merge did not authorise account creation, staging SQL, DDL/DML, or D-01 execution.
- [x] Recorded the owner’s D-01-only acceptance that a same-day current Free Starter snapshot is the valid recovery point for any later D-01 decision. The dated acceptance and proposed `2026-09-12 14:00–20:00 UTC` change window with a two-hour snapshot-expiry safety margin are documented. Owner-supplied `KINGA-staging` Backup evidence confirms a Succeeded snapshot at `2026-09-12 03:01:00 UTC+00:00`, expiring `2026-09-13 03:01:00 UTC+00:00`, which covers the proposed window plus margin by 5 hours 1 minute. No account was created, no staging connection made, and no D-01 execution occurred.
- [ ] Owner-authorised D-01 execution: within the revised immediate-to-`2026-09-12 13:00 UTC` window only, re-confirm the current successful `KINGA-staging` snapshot; create and validate separate constrained D-01 verifier/runner identities; preflight target/privileges/metadata; run only SQL SHA-256 `306797ab94529860bc9e88a69a955061916a94ddc3d08e3265229780ab08083c` (3 `CREATE TABLE` and 8 `CREATE INDEX` statements) on `kinga_staging`; complete independent metadata and basic three-table connectivity/read smoke checks; revoke/disable constrained access; and stop immediately on any mismatch. The 07:32:34 UTC Backup-page recheck passed: the same-day Succeeded snapshot remains valid through `2026-09-13 03:01:00 UTC`, exceeding the 15:00 UTC post-closure safety margin by 12 hours 1 minute. No production action is authorised.
- [x] D-01-only application-validation exception: documented Tavonga Shoko as the application-validation owner and observer because no independent owner is available. This is limited to D-01, expires at D-01 closure, and does not waive source/SQL pins, snapshot validity, target/privilege/metadata checks, runtime test evidence, closure, or restrictions on D-02–D-06 and production.
- [x] Merged owner-approved final D-01 preflight package PR #84 through the protected GitHub workflow. GitHub recorded merge commit `a451f313d4cd34e5e518afcd41ed2509a75a14b5`, and final-preflight head `016195abc6541c1a5b45baa225b4e22a7f29f6dd` is verified as an ancestor of `main`. The platform `code-complete` failure was independently scoped to inherited TypeScript diagnostics because PR #84 changed only documentation/backlog paths; the merge itself did not execute database work.
- [x] Revised D-01 execution window: the owner explicitly replaced the proposed 14:00–20:00 UTC window with an immediate start through `2026-09-12 13:00 UTC`. All other D-01 SQL pins, recovery requirements, target/privilege/metadata stop gates, same-day Starter limitation, D-01-only exceptions, and production exclusion remain unchanged.
- [ ] Resolve the D-01 least-privilege runner constraint before account creation: TiDB requires `CREATE` for `CREATE TABLE` and `INDEX` for `CREATE INDEX`, but `INDEX` also permits `DROP INDEX`. Do not create a runner whose effective capability contradicts the approved no-destructive-action control without a documented owner-accepted compensating control or revised authority.
- [x] D-01 existing-administrator exception: the owner selected option B on 12 September 2026, authorising use of the authenticated TiDB Cloud console administrator rather than creation of the planned verifier/runner accounts. The broad global privilege observation, D-01-only scope, immediate expiry at D-01 closure, and uncompromised requirements for exact SQL pinning, preflight, postflight, smoke checks, and production exclusion are documented before any DDL.
- [ ] D-01 execution stopped before first DDL: TiDB Cloud SQL Editor input truncated the first pinned `CREATE TABLE tenant_invitations` statement during load. Editor readback differs from SHA-256 `306797ab94529860bc9e88a69a955061916a94ddc3d08e3265229780ab08083c`; no Run action was taken and no table/index/data/account/privilege/backup/restore/network/production action occurred. The incomplete editor buffer was cleared at `2026-09-12 07:43:34 UTC`. Do not resume without a verified safe transfer mechanism and renewed owner authority.
- [ ] Owner-authorised D-01 safe transfer: use TiDB Cloud SQL Files to upload the exact local SHA-pinned Wave 1 artefact, verify its complete content and SHA-256 before it is loaded into the SQL Editor, and execute no SQL unless the loaded buffer matches exactly. Upload alone is not execution; all original target, snapshot, scope, stop, and production controls remain active.
- [ ] D-01 safe-transfer blocker: TiDB Cloud SQL Files exposes only a new-query editor, not a file-upload control (zero visible/file DOM controls). No artefact was uploaded or run. Obtain renewed owner authority for an alternative exact-content transfer and verification method before resuming; retain the existing stop condition otherwise.
- [ ] Owner-authorised D-01 exact editor paste: load the complete local SHA-pinned artefact in one browser-editor paste operation, calculate the full resulting editor-buffer SHA-256 in the browser, and compare it to `306797ab94529860bc9e88a69a955061916a94ddc3d08e3265229780ab08083c` before any Run action. Stop on any mismatch, wrong database, or statement-count/scope difference.
- [ ] D-01 execution stopped after safe-transfer failures: the verified atomic buffer did not register with the SQL Editor Run control, and the attempted native clipboard paste inserted only literal `V`; residue was cleared at `2026-09-12 07:52:38 UTC`. Query log still contains only `USE kinga_staging;`; no DDL/DML or account action occurred. Do not make further automated transfer attempts in this window; renewed owner authority and a new immediate preflight are required for a different execution mechanism.
- [x] Assessed and documented a safe D-01 workaround: a direct TLS MySQL-client route can preserve the exact source SHA, deterministic statement splitting, statement-level hashes, target selection, and machine-readable server evidence while avoiding the browser SQL Editor defects. No database action was resumed. Evidence: Section 5A of `docs/staging-schema-reconciliation/gate-d-wave-01-staging-change-packet-draft-2026-09-12.md`.
- [ ] Direct-client workaround decision: obtain specific owner approval for the documented route, including temporary runner/verifier account creation and later removal, secure credential delivery, dynamic IP/network confirmation, renewed immediate snapshot/target preflight, exact statement-hash manifest, revised window, and D-01-only residual `INDEX`/`DROP INDEX` risk acceptance. Do not connect or execute before that approval.
- [ ] Owner-approved D-01 direct TLS-client execution: create temporary TLS-required runner and verifier identities from the authenticated TiDB administrator only after protected credential generation; complete a fresh same-day snapshot, target, IP/network, grant, and zero-table preflight; execute the 11 pinned statements only with source/per-statement hashes; verify metadata and basic reads; then remove temporary accounts and credentials. Stop immediately on any mismatch; production remains excluded.
- [ ] D-01 runner-identity correction: initial prefixed runner username exceeded TiDB’s 32-character limit and was rejected before account creation. Generate a fresh protected runner secret and use a shorter unique prefixed name; do not reuse the failed verifier or proceed without validating the successful account and grants.
- [x] D-01 verifier-grant timing check: table-specific `SELECT` grant was correctly rejected while `tenant_invitations` does not exist, confirming the zero-table preflight. No broader database-wide `SELECT` was granted. Issue the three table-specific verifier grants only after all D-01 tables have been created and before verifier postflight.
- [x] Direct-client credential preparation: generated separate runner and verifier secrets locally with protected directory/file permissions. No raw secret, connection string, or verifier was exposed, committed, or used; no account or database connection has been created. Evidence: D-01 packet direct-client credential-preparation record.
- [ ] D-01 direct-client network stop: TiDB Cloud Connect warns that browser egress `54.87.139.152` is not authorised, while the local direct-client environment previously resolved to different public egress `54.172.182.52`. No account or connection was attempted. Obtain explicit owner authority for any exact-IP TiDB Cloud public-endpoint allow-rule change, then repeat the immediate preflight before resuming.
- [ ] Owner-authorised D-01 temporary staging network rule: add only local direct-client egress `54.172.182.52` to the `KINGA-staging` TiDB Cloud public-endpoint allow list, verify its active exact scope, and remove that temporary rule at D-01 closure. Do not alter any other staging rule or any production network setting.
- [x] D-01 network inspection: no new firewall rule was created because existing owner-managed rule `Allow_all_public_connections` already ranges from `54.172.182.52` to `137.59.229.33`, including the previously observed local direct-client address at its lower endpoint. The inherited range is broader than the requested temporary exact-IP rule; it was not modified. Reconfirm actual shell egress before any direct connection and stop if it differs.
- [ ] D-01 direct-client network stop — changed egress: immediate recheck at `2026-09-12 08:07 UTC` returned `154.255.52.171`, outside the visible existing `54.172.182.52`–`137.59.229.33` range and different from the owner-authorised earlier address. No account or connection was attempted. Do not resume unless the owner gives a new explicit decision for this exact current IP and it is then rechecked.
- [x] Owner-authorised D-01 current-egress temporary network rule: added only `154.255.52.171` to the `KINGA-staging` public-endpoint allow list as `D01_direct_client_20260912`, after the owner reviewed the exact form and explicitly confirmed save. TiDB Cloud reported Added successfully; no pre-existing rule or production setting was modified. Remove this rule at D-01 closure or immediately if D-01 stops.
- [ ] D-01 direct-client transport stop: despite a public-IP discovery result of `154.255.52.171`, TiDB observed the runner connection from `54.90.169.80` and rejected it before SQL with access-denied. No schema statement or client-side account/grant change occurred. Do not add a broad rule or retry unless the owner explicitly authorises exactly `54.90.169.80` and the next connection confirms it as the actual database-observed source.
- [ ] Owner-authorised D-01 two-hour network-range exception: add only `54.90.169.0`–`54.90.169.255` (`54.90.169.0/24`) to `KINGA-staging` for the direct-client workaround, with an expiry/removal deadline two hours from the owner’s 12 September 2026 approval. This is a 256-address bounded range—not a broad cloud range or wildcard—and does not modify existing rules or production.
- [x] Owner-authorised D-01 two-hour network-range exception: added only `54.90.169.0`–`54.90.169.255` (`54.90.169.0/24`) to `KINGA-staging` as `D01_direct_natpool_2h`, after the owner reviewed the exact form and explicitly confirmed Save. TiDB Cloud reported Added successfully; no existing rule or production setting changed. Remove the range no later than two hours after approval or immediately at D-01 closure.
- [x] D-01 bounded range cleanup: owner-confirmed removal of only `D01_direct_natpool_2h` completed at `2026-09-12 08:31 UTC`; TiDB Cloud reported Deleted successfully and rule count returned from six to five. No other rule or production setting changed.
- [x] D-01 single-IP cleanup: owner-confirmed removal of only `D01_direct_client_20260912` completed at `2026-09-12 08:32 UTC`; TiDB Cloud reported Deleted successfully and rule count returned from five to four. Both temporary D-01 firewall rules are absent; the four pre-existing owner-managed rules remain unchanged.
- [x] D-01 renewed transport stop and cleanup: after the approved `/24` rule, TiDB observed the direct runner from a third dynamic source `54.87.139.152`, outside the range, and rejected it before SQL. No schema/data action occurred. Owner-confirmed cleanup removed both temporary D-01 accounts, `D01_direct_natpool_2h`, and `D01_direct_client_20260912`; protected local credentials were destroyed. Do not add further network ranges or retry this route.
- [x] D-01 local credential cleanup: destroyed the protected temporary credential directory after account and firewall cleanup; final existence check passed. No raw secret or verifier was exposed, committed, retained in an environment file, or transferred outside protected local material.
...
- [x] Owner-executed D-01 postflight reconciliation: independently verified with read-only TiDB catalogue, `SHOW CREATE TABLE`, and row-count queries. `kinga_staging` contains exactly `tenant_invitations` (10 columns), `tenants` (26), and `users` (30); their approved PK/unique and eight explicit secondary indexes match the pinned SHA-256 `306797ab94529860bc9e88a69a955061916a94ddc3d08e3265229780ab08083c`, with no foreign key or extra Wave 1 object. All three tables read as zero rows. The owner-reported local-client marker interruption and remaining-statement recovery are documented without changing the immutable source artefact. No postflight DDL/DML, account, network, or production action occurred. Evidence: `audit/gate-d-d01-owner-executed-postflight-2026-09-12.md`.
- [x] D-01 closure: completed the owner-operated Wave 1 staging migration and independent postflight reconciliation. Earlier browser/sandbox execution channels remain abandoned historical evidence; their temporary accounts, firewall rules, and credentials were already removed. D-02 and later remain separately unauthorised; production remains blocked pending Essential/Dedicated service with real PITR and a separate production plan.
- [x] D-02 review-only packet: prepared the 20-table Wave 2 vehicle, claimant, driver, policy, inspection, and claim-core staging change packet with the approved SQL SHA-256 `15661c69490a4360931ef5fc3d17e2b4521f692730b2d113f1342117b067e7b9`, 20 tables, 9 FKs, 58 explicit indexes, Wave 1 prerequisite pin, same-day Starter recovery requirements, and short-window proposal. It identifies all 86 `--> statement-breakpoint` markers as non-MySQL-comment repository separators and prohibits raw-file MySQL execution. Evidence: `docs/staging-schema-reconciliation/gate-d-wave-02-staging-change-packet-draft-2026-09-12.md`.
- [ ] D-02 staging preflight/execution: no staging connection, snapshot check, verifier/runner account, network adjustment, DDL/DML, or application change is authorised. A separate owner decision must approve current-day snapshot inspection, named operator and independent reviewer, scoped access design, exact UTC window, read-only prerequisite/absence preflight, and only then any marker-aware 87-statement execution plan.
- [x] D-02 owner-operated preparation: Tavonga Shoko’s D-02-only exception and the immutable 87-statement ledger with individual SHA-256 values are complete. Authenticated Backup-page evidence at `2026-09-13 09:27 UTC` records a Succeeded `KINGA-staging` snapshot at `2026-09-13 03:01:00 UTC±00:00`, expiring `2026-09-14 03:01:00 UTC±00:00`. A proposed `2026-09-13 10:30–14:30 UTC` window meets the two-hour post-closure safety margin by 10 hours 30 minutes. No D-02 DDL/DML, account, network, or production action occurred.
- [x] D-02-only control exception: documented Tavonga Shoko as sole D-02 operator, reviewer, application-validation owner, and observer because no independent person is available. The exception is limited to the 20-table D-02 packet, expires at D-02 closure/stop/window expiry, and preserves source/statement pins, marker-aware splitting, recovery, target, preflight, postflight, smoke, closure, and production-exclusion controls. Evidence: Section 1A of `docs/staging-schema-reconciliation/gate-d-wave-02-staging-change-packet-draft-2026-09-12.md`.
...
- [x] Approved claim-lifecycle fixture ownership conversion: replaced borrowed `SELECT … LIMIT 1` claim usage with a uniquely stamped owned actor and claim after documenting the required lifecycle fixture fields; cleanup uses only captured IDs in foreign-key-safe order.
- [x] Read-only borrowed-record mutation audit: scanned arbitrary/non-owned test-row selections and mutations, confirmed only the approved claim-lifecycle suite borrowed and mutated a row, and retained evidence in the Batch 1 audit record.
...
- [x] Wave 1.3 regression-contract repair: replaced seven Stage 6 location-sensitive source assertions with behaviour/import-aware checks preserving canonical vocabulary normalisation and retry guarantees after the approved concern split; focused regressions passed and the complete per-module gate found no split-specific failure.
- [x] Stage 6 split gate: repeated `truthReconciliationEngine` idempotency five times each on unmodified current main and the held Stage 6 branch in the same environment; the test failed identically 5/5 on both, proving it predates and is unrelated to Stage 6.
- [ ] Test-suite reliability: current main produces non-deterministic full-suite failure sets across consecutive unchanged-code runs; separately diagnose the truth-reconciliation idempotency defect and the broader unstable baseline before treating full-suite results as deterministic release evidence.
- [x] Wave 1.2 held split: separated `client/src/pages/ClaimDecisionReport.tsx` into a legacy default-export barrel, page orchestration, and presentational sections; compiler-derived export name remained 1/1 and direct importers 0/0; focused test, tsc baseline comparison, full-suite failure set, server bundle, and Vite build validated; no PR or merge.
- [x] Wave 1.1 held split: separated `shared/vehicleParts.ts` into types, catalogue, lookup, plausibility, and normalisation modules behind the legacy barrel; compiler-derived public exports remained 14/14 with unchanged canonical signatures; direct importers remained 8/8; focused test, tsc baseline comparison, full-suite failure set, server bundle, and Vite build validated; no PR or merge.
- [x] Maintained named-constant batch: classified all 92 preserved scan candidates; extracted 45 verified executable references into named constants without changing effective values; excluded 47 already-named, prompt/example, stale, low-level, or separately grouped values with rationale in `audit/maintained-constant-classification-2026-08-23.md`; retained the isolated branch without a PR or merge pending GitHub merge-control remediation.
- [x] Priority IN-001 / EX-001 branch: removed operational demo-data fallbacks from ClaimsManagerDashboard and ExecutiveAnalyticsCharts; added distinct loading, unavailable, and no-data states; suppressed unsourced weekly, SLA-risk, and processing-stage values; validated with focused coverage, full live-TiDB suite, and both builds; held branch without a PR or merge.
- [x] Priority UI-001 branch: removed the duplicate `Network` import from `AdminPortalLayout`; added focused compile coverage; verified production `/` and `/login` route execution through the authentication handoff without a client compile exception; completed full-suite and both-build validation; held branch without a PR or merge.

- [x] Approved isolated ForensicReportModel implementation: added immutable forensic report types and a tenant-scoped `resolveForensicReportModel()` alongside the unchanged current generator; proved populated field-family and observable-rendered-value parity against the same live claim plus tenant isolation with real-database tests; held on `feat/forensic-report-model` without a PR or merge. Evidence: `audit/forensic-report-model-isolated-contract-2026-08-27.md`. The data-fetcher/engine/visuals/renderer split remains deferred until staging exists.
- [x] Approved report-tier canonical-layer remediation: inventoried all CL, CI, and FR renderer raw queries/direct reads/duplicate derivations; extended only the canonical resolver where a field was demonstrably absent; migrated the active CL, CI, and FR paths in separate reviewable commits; added live-data shared-field parity coverage; ran full-suite comparison and both builds; held `fix/report-tier-canonical-bypasses` without PR or merge. Evidence: `audit/report-tier-canonical-bypass-remediation-2026-08-28.md`. Authentication, database-access ownership, schema, migrations, data, and non-report paths were unchanged.

- [x] Approved explicit forensic-model dependency decision: reviewed the separately held ForensicReportModel branch, found no known correctness block, incorporated only its independently validated canonical contract fields in a traceable commit before FR migration, and added value-level audit-event and dispute-availability parity assertions to the live-data cross-tier regression. The forensic structural split remains deferred pending staging.

- [x] Approved isolated ForensicReportModel implementation: added immutable forensic report types and a tenant-scoped `resolveForensicReportModel()` alongside the unchanged current generator; proved populated field-family and observable-rendered-value parity against the same live claim plus tenant isolation with real-database tests; held on `feat/forensic-report-model` without a PR or merge. Evidence: `audit/forensic-report-model-isolated-contract-2026-08-27.md`. The data-fetcher/engine/visuals/renderer split remains deferred until staging exists.

- [x] Approved report-tier verification remediation: replaced the two stale CI/FR document-evidence source-shape assertions with live canonical-record plus rendered-output tests; individually corrected the six branch-only reporting TypeScript diagnostics with typed contracts or explicit safe narrowing; completed client TypeScript source partitions; reran server/shared tests, type checks, and builds; committed each logical unit and held the existing branch without PR or merge. Evidence: `audit/report-tier-verification-remediation-2026-08-28.md`. No module split, database-access-scope change, schema, migration, or unrelated alteration.

- [x] Report-tier updated-base contract resolution: retained only verified Claim PDF panel-beater and causal-verdict fields, removed duplicate incident description, and documented intentional omission of unconsumed policy/forensic fields. Canonical contract test passed; the shared-field test retains the identical main-baseline four-connection assertion; server/Vite builds passed; 42-shard suite completed with one non-reproducible upload acceptance-test variance.
- [x] Approved urgent report-access security fixes: secured the REST audit-export endpoint using the existing authenticated tenant-scoped governed-claim authority pattern, and restricted globally unscoped Shadow observation/report procedures to platform super administrators. Added unauthenticated, foreign-tenant, same-tenant, and no-cross-tenant-data regressions; held as one commit per fix. Evidence: `audit/report-export-security-hardening-2026-08-28.md`. SAR, legacy PDFs, placeholder downloads, metrics, CL/CI/FR, schema, migrations, and unrelated code remain unchanged.
- [x] Priority UI-001 follow-up: whole-client AST scan found five duplicate import bindings. Removed duplicate `useAuth` imports in Home and InsurerRoleSelection, duplicate `useLocation` imports in AdminDashboard and PortalSelection, and duplicate `trpc` in FleetDriverDashboard; retained the original import in every case. Clean build and browser render verification remain required before final validation.

- [ ] Separate follow-up — `BulkValuation.tsx` has a duplicate `border` style-object key observed during the PR #22 whole-client build. It is not an import collision or this PR’s compile-blocker scope; leave it unchanged and track it for a separate small correction.

- [x] PR #22 validation continuation: AST sweep confirmed zero remaining duplicate import bindings; focused compile regression passed 1/1; root route reached normal sign-in handoff rather than failing Home module load; server bundle passed. The full suite completed 42 shards but retained inherited failures and one unrelated server acceptance-test variance (`upload-to-report`) requiring separate suite-reliability follow-up. Vite transformed all 3,753 client modules with only the pre-existing BulkValuation duplicate-style-key warning, then resource-terminated during chunk rendering on two attempts; this is recorded as the established sandbox ceiling, not a duplicate-import failure.
- [ ] Priority IN-001 / EX-001 branch: replace operational demo-data fallbacks with honest, visibly distinct empty states in ClaimsManagerDashboard and ExecutiveAnalyticsCharts; validate real rendering plus the full suite and both builds; hold the branch without a PR or merge.
- [ ] Deferred authenticated portal sweep: after both held UI branches validate, exercise all portals, tabs and navigation with tenant-A and tenant-B sessions; record runtime, link, state and visual tenant-isolation evidence without opening a PR or merging.
...
- [x] Approved SAR tenant-isolation remediation: scoped every existing SAR request, claimant match, claim, and assessment path to the authenticated requesting tenant using the existing canonical authority pattern; confirmed the generator has no later vehicle, document, quotation, or related-record query; added owned live-TiDB direct, duplicate-identity, indirect-parent, and same-tenant regressions; preserved SAR output and legal/disclosure policy; held `fix/sar-tenant-isolation` without PR or merge. Evidence: `audit/sar-tenant-isolation-p0-2026-08-28.md`.
- [x] PR #27 resolution: retained typed approval-stage normalisation, odometer scalar filtering, and additive parity/no-fabrication regression coverage from current main; derived optional human decision/sign-off stages without payment-authorisation framing; focused live-TiDB coverage passed and inherited full-suite failures were compared to current main before publication.
- [x] PR #27 publication: committed and pushed the validated human-approval resolution, updated review-only PR #27 with the Vite memory-ceiling caveat, and require its CI Vite build to be the authoritative production-build verification because the sandbox reaches a known chunk-rendering memory ceiling.
...
- [x] Repository and workspace synchronization verification: GitHub `main` is `211f9b180743d70f800876a780be967612e9a526`; the managed workspace remains at checkpoint/head `3361bdaccde37b87f60dc2c51001c7b993b43f15`, 31 commits ahead and 33 commits behind current `main`. Its only uncommitted file is this verification record. The current checkpoint predates the merged Gate D/NCU stack and therefore does not include it. Historical D-05/NCU worktrees are not current-main workspaces; their merged evidence is present on GitHub `main`. No source, schema, staging, or production state was modified by this verification.

- [x] Managed-workspace local-commit review: inventoried all 31 local-only commits before reconciliation; they comprise meaningful checkpoint/recovery history, read-only governance/report investigations, historical Git merges, and a small number of old source/test/config changes. The owner reviewed the preservation assessment and authorized a no-reset, preservation-first merge. No local history, backlog record, database, staging, or production state was discarded or modified during the review.

- [x] Managed-workspace preservation-first reconciliation: merged current GitHub `main` (`211f9b180743d70f800876a780be967612e9a526`) into the managed workspace without reset or discard, producing merge commit `47f9257ebd5b416e00a98ff5aa796f689b5c28b9`. The only conflict was protected `todo.md`; the owner reviewed its exact hunk and approved the union of both append-only history blocks. There were no runtime-code, `drizzle/schema.ts`, or `.project-config.json` conflicts. The reconciled tree is clean, contains current Gate D/NCU closure evidence plus preserved local audit/backlog history, has no unresolved conflict markers, passes Git connectivity verification, and introduces no new whitespace issue from the GitHub-main merge. A new checkpoint is now required.

- [x] Stage 1 auth discovery: completed read-only inventory and end-to-end trace of every `OAUTH_SERVER_URL`, Manus OAuth, session, token-verification, login/logout, and identity-resolution touchpoint. Assessed D-01 `users` schema fields for future WorkOS identity mapping; documented provider coupling, scope, special identities, and migration decision gates in `/home/ubuntu/Downloads/KINGA-stage-1-auth-discovery-2026-09-16.md`. No application code, configuration, dependency, WorkOS/Manus account, schema, database, staging, or production state was changed.
...
- [x] Controlled open-PR review completed: independently revalidated and merged clean PRs #35, #37, #38, #39, #40, #44, #45, #46, and #47 in dependency order. Obsolete contaminated PRs #41, #42, and #43 were closed without source-branch deletion. Main includes the verified Package D merge `d2a8b9c99abae05f740f0486e829b858cf1d8712` and no pull requests remain open.
- [x] Package D: Claims Intelligence prints the loaded iframe document; report footers use stable section labels rather than false two-page counters; print CSS retains explicit A4 section breaks; and a generated-document regression covers the A4 structure. Browser validation confirmed one iframe print call and an authorised rendered report produced an 11-page A4 PDF with content on the first, intervening, and final pages.
- [x] Managed-workspace reconciliation after Package D: retained GitHub main’s external-staging backlog history and appended the controlled merge ledger; only `todo.md` conflicted, with no runtime-code conflict.
- [x] Produced the evidence-based current-state account in `audit/kinga-current-state-account-2026-09-08.md`. It distinguishes verified merged remediation, workspace/checkpoint and GitHub status, report/evidence boundaries, pre-existing TypeScript debt, live Stage 6 degradation, open governance/DRV/evidence-reconstruction work, and the recommended controlled next sequence without changing system behavior.

- [ ] Execute the approved remaining remediation in strict phases: evidence-safe governance-export resolution; investigate then, only if bounded, repair the live Stage 6 severity degradation; remediate DRV-006; remediate DRV-007/008; investigate DRV-009; remediate DRV-013; investigate DRV-015; and conclude with baseline-aware verification. Keep each implementation independently validated and reviewable; stop on any product, legal, evidence-governance, or data-correction decision gate.
- [ ] Classify and preserve the broader managed-workspace divergence from current GitHub main—four audit reports, managed metadata, Home/bootstrap files, and backlog history—before splitting only independently reviewable changes. Do not merge the published reconciliation branch as a one-file backlog fix.
- [x] Diagnostic-only Manus support package: reproduced the managed-workspace checkpoint synchronization failure after it fetched `user_github/main` at `b3e0edb9` (merged PR #85). The checkpoint failed before save on the same workspace-versus-main merge-conflict class; no repair, overwrite, reset, deletion, database/schema/data action, or deployment was attempted. Evidence: `audit/manus-checkpoint-sync-conflict-support-evidence-2026-09-12.md`.

- [x] User-authorized managed-workspace recovery: merged `user_github/main` (`956ab8055d590157ef10d091e1f8a4e6ba34542d`) into local `kinga-replit` in merge commit `880fb8545f5447a3ce65d617fc41ab971b12d76e`; the local preservation set was temporarily stashed only because Git refused to merge over modified `todo.md`, then reapplied and retained in the merge. There were no unresolved merge-conflict files: remote `todo.md` content and the local documentation/backlog history were both retained, alongside `.project-config.json`, two audit records, and the staging TiDB smoke-test file. No reset, force-push, reinitialization, database/schema/data action, or deployment occurred.
- [x] D-02 PR #87 wording reconciliation: remote head `72678167abd8d503d3a9ed10bc828343291f0f9e` contains the Section 1A owner-operated exception and the 87-statement ledger, but it also retained contradictory earlier text naming an unassigned operator, mandatory independent reviewer, and separately named application-validation owner. The correction aligns all role, ledger-review, postflight, and closure language to the dated D-02-only exception. D-02 read-only preflight and all database action remain paused until the corrected GitHub revision is reviewed.

- [x] D-02 closure reconciliation: independently compared the owner-reported 87-statement ledger-order execution outcome with read-only live staging metadata, the pinned Wave 1/Wave 2 source, canonical ledger, and approved packet. Results: 23/23 tables, 586/586 columns, 9/9 fully matched FKs, 58/58 explicit indexes and 69/69 explicit index-column rows, 33/33 primary/unique keyed-column rows, and zero rows in every D-01/D-02 table. Immutable ledger/source verification passed. Evidence: `audit/gate-d-d02-independent-postflight-2026-09-13.md`.
- [x] D-01 runner-account lifecycle finding: the earlier read-only `SHOW GRANTS FOR 'd01_runner'@'%'` and exact-name `mysql.user` lookup returned no unprefixed principal. This was later corrected as a false-negative identity check: the actual runner is tenant-prefixed; see the completed D-02 runner-account correction below. No account or grant change was made by the earlier check.

- [x] D-02 runner-account correction: replaced the false-negative unprefixed-account finding in the closure record with the owner-verified tenant-prefixed principal `289ZyKGJwbC2SkB.d01_runner` at host `%`, which holds `SELECT`, `CREATE`, `REFERENCES`, `ALTER`, and `INDEX` on `kinga_staging.*`; recorded the owner decision to retain it for D-03 through D-06 reuse. No grant, account, or schema change was made by this correction.
- [x] D-03 review packet: prepared only the 50-table marker-aware source packet, canonical 187-row per-statement SHA-256 ledger, 187 individually hash-verified statement files, fresh read-only `KINGA-staging` snapshot observation, and exact tenant-prefixed runner-account privilege assessment. Source SHA-256 `ccdca4a47d9d82c04cc9b9a12e6d49960d4271a052773a342282086a9aff9254`; statement classes 50 tables, 26 FKs, 111 indexes; no source-derived privilege gap. No SQL execution, grant/account change, network change, restore, data seed, deployment, or production action was performed.
...
- [x] D-04 compatibility confirmation and review packet: confirmed that immutable historical Wave 4/5 Gate C source preserves six formerly incompatible literal JSON-shaped TEXT defaults for audit history, while the revised TiDB-compatible execution sources contain all six reconciled as `JSON_ARRAY()` or `JSON_OBJECT()` expressions with no residual `DEFAULT ('[]')` / `DEFAULT ('{}')` syntax. Prepared the separate review-only D-04 operational-portals packet pinned to revised Wave 4 source SHA-256 `8317ccc6…83f61`: 135 literal markers and statements, class totals 40 tables / 7 FKs / 88 indexes, canonical per-statement ledger SHA-256 `2f3aa497…97c9f`, 135 exact statement files, and ordered-file SHA-256 `8909d7cf…93ee`. Fresh authenticated review evidence recorded the successful `2026-09-14 03:00 UTC` snapshot expiring `2026-09-15 03:00 UTC` and the exact tenant-prefixed runner grant set `SELECT, CREATE, REFERENCES, ALTER, INDEX ON kinga_staging.*`; no source-derived privilege gap. D-04 SQL, account/grant, networking, recovery, deployment, data, and production actions remain prohibited pending a later distinct decision.

- [x] D-04 execution authority: owner authorised D-04 only and renewed the sole-operator/reviewer/application-validation-owner exception for Tavonga Shoko specifically for resumption. Historical ordinals 1–98 were owner-reported as hash-verified accepted partial execution; a source compatibility stop at ordinal 99 superseded the original 40-table/7-FK/88-index/135-statement source. The revised 40-table/7-FK/87-index/134-statement source and its 36-file historical-ordinal 100–135 resumption ledger passed a new authenticated preflight, then the owner completed the resumption. Independent final reconciliation passed: 113 tables, 7 D-04 FKs, 87 D-04 explicit indexes, correct structural/key definitions, and zero rows in all 40 D-04 tables. The renewed exception expired with D-04 closure; no production action was authorised or performed.

- [x] D-04 statement-99 TEXT-index compatibility stop: owner reported exact ledger ordinals 1–98 completed hash-verified, creating all 40 D-04 tables, all 7 D-04 foreign keys, and 51 explicit indexes with zero rows; ordinal 99 (`CREATE INDEX idx_recipients ON governance_notifications (recipients)`) stopped because `recipients` is `TEXT NOT NULL` and the source contains no prefix length. The Gate B trace proved it is the same previously excluded no-consumer whole-TEXT index: its historical overlay deliberately omitted it but did not reconcile the canonical schema, from which Gate C later regenerated Wave 4. Removed only the canonical declaration, not historical evidence. The exhaustive revised Wave 4–5 scan found no other unprefixed TEXT/BLOB index. The revised source changes only historical ordinal 99 by omitting it: 1–98 and 100–135 remain byte-identical. The 134-statement revised ledger/files and the 36-file original-ordinal 100–135 resumption ledger/files passed deterministic verification. No post-stop DDL was sent; a fresh preflight and owner Claude Code resumption decision remain required.

- [x] D-04 closure reconciliation: owner-reported completion of original ordinals 100–135 after accepted 1–98 was independently reconciled through authenticated read-only TiDB metadata and row-count queries. The final staging state matched the corrected source exactly: 113 expected/live tables, 637/637 D-04 columns with 0 mismatches, 42/42 cumulative FK definitions, 264/264 cumulative source-explicit indexes including 87 D-04 definitions, 153/153 primary/unique keyed-column rows, and 0 rows across all 40 D-04 tables. D-04 formally closed on 15 September 2026; its renewed D-04-only sole-operator/reviewer/application-validation exception expired and does not transfer to later work.

- [x] D-05 final-wave review packet: prepared a separate review-only packet for the 75-table intelligence, learning, analytics, governance, and optimisation wave. The repeated compatibility audit passed: 290 literal markers with one discarded empty terminal fragment, no legacy JSON-shaped TEXT literal defaults, one TiDB-compatible `JSON_OBJECT()` expression default, and no unprefixed `TEXT`/`BLOB` index. The canonical ledger and 290 exact statement files passed deterministic SHA-256 verification. Fresh authenticated review evidence recorded a successful same-day `KINGA-staging` snapshot and the exact retained tenant-prefixed runner grant set, with no source-derived privilege gap. No D-05 SQL, role exception, account/grant change, network change, backup/restore action, deployment, recovery/cutover, or production action is authorised.

- [x] D-05 execution authority: the owner executed the 290-statement ledger through the approved local Claude Code route and reported individual hash verification, strict ordinal execution, 75 tables, 25 D-05 foreign keys, 190 D-05 explicit indexes, and zero D-05 rows. The D-05-only sole-operator/reviewer/application-validation-owner exception for Tavonga Shoko expired at formal closure on 15 September 2026. No production action is authorised.

- [x] D-05 independent closure reconciliation: authenticated read-only TiDB checks and source-derived deterministic comparators verified 188/188 cumulative tables, 67/67 cumulative foreign-key definitions, 190/190 D-05 explicit index pairs with 213/213 ordered columns, 75/75 D-05 tables with 1,236/1,236 matching columns, 220/220 primary/unique structures across 242 keyed-column rows, and 188 checked tables with zero total/minimum/maximum rows. The retained evidence is `audit/gate-d-d05-independent-postflight-2026-09-15.md`; no DDL/DML, grant, recovery, deployment, or production action occurred.

- [x] Final Gate D cumulative closure: all D-01–D-05 baseline controls passed. The verified final baseline is 188 tables, 67 foreign-key definitions, 454 source-explicit index definitions (the prior 446 report omitted 8 D-01 indexes), 220 primary/unique structures, and zero rows in all tables. Closure evidence is `audit/gate-d-final-staging-schema-reconciliation-closure-2026-09-15.md`; the D-05-only exception expired at closure. D-06, recovery, cutover, deployment, and production remain prohibited absent distinct later authority.

- [x] Natural/composite-unique implementation: owner confirmed `assessor_insurer_relationships.tenant_id` is the insurer identity and `fleet_drivers.user_id` is the driver identity. Added only `uq_assessor_insurer_relationship (assessor_id, tenant_id)`, `uq_policy_claim_link (policy_id, claim_id)`, and `uq_fleet_driver_membership (fleet_id, user_id)` to the final D-05 closure source baseline; `entity_relationships` remains unchanged and source-level unique-index-free. Two independently named, empty loopback scratch replays each composed the exact 188-table baseline, applied the four-statement transition SHA-256 `a8b8e8fda51a0830116d2516a7d017ee7401e3b35a72e7a7773e1689c10ab9be`, verified all three unique index definitions, rejected all three duplicate pairs with `ER_DUP_ENTRY`, and accepted valid distinct pairs in both dimensions. All scratch databases were dropped and confirmed absent. Focused Vitest source contract: 3/3 passed. Evidence: `audit/natural-composite-unique-2026-09-15/scratch-proof-evidence.md`. No staging or production action is authorised or performed.
...
- [x] NCU-01 independent postflight: completed fresh authenticated read-only evidence that `unique_assessor_tenant` is absent; `uq_assessor_insurer_relationship`, `uq_policy_claim_link`, and `uq_fleet_driver_membership` are unique and have the expected ordered columns; all 188 tables and all target structures remain source-matching; the three duplicate-pair controls and all 188 row counts remain zero. The pinned packet, transition source, all four individual statement files, ordered file hash, and focused source-contract regression were reverified. No mismatch occurred; closure evidence was prepared for protected review publication.

- [x] Gate D/NCU main reconciliation: recorded the owner-authorized protected merge of PRs #87–#93. Final `main` head `f29c1263cc242f36a39701729380b7be1110ee10` contains every D-02–D-05 and NCU-01 PR head plus the already-present D-01 record; no Gate D/NCU pull request remains open. Evidence: `audit/gate-d-ncu-main-reconciliation-2026-09-15.md`. The final 188-table staging baseline remains closed while D-06, recovery, cutover, deployment, and production remain unauthorized.

- [x] Stage 2 WorkOS implementation proposal: completed proposal-only design for the approved architecture—WorkOS authenticates humans while KINGA retains its own session JWT; future additive nullable unique `users.workos_user_id` (128 chars) and `tenants.workos_organization_id` support the approved one-tenant-to-one-organization model; linking is verified-email-only behind duplicate-email preflight; cron and agency-assisted identities remain KINGA-only; and rollout is staging-first with dual-read/linking, rollback, and tenant-isolation regression. The proposal specifies exact file sequencing, guarded migration design, a provider-neutral login helper replacing the nine direct login-helper call sites, test matrix, rollback plan, and deferred backlog items for stale `/api/oauth/logout` and removable `/api/auth-test`. Evidence: `/home/ubuntu/Downloads/KINGA-stage-2-workos-implementation-proposal-2026-09-16.md`. No application code, configuration, dependency, account, schema, database, staging, or production state was changed.

- [ ] WorkOS Package A schema contract: on an isolated review branch, add only nullable unique `users.workos_user_id VARCHAR(128)` and nullable unique `tenants.workos_organization_id VARCHAR(128)` source declarations with reviewed additive migration SQL and source-contract tests. Prove the result in disposable scratch replay before any staging consideration. Do not add WorkOS dependencies, secrets, accounts, routes, callback behavior, provider switch, or live database schema change. Separately identify—without reading it yet—the authoritative current production user-data location for the owner-authorized future duplicate-email preflight; staging is known empty and is not sufficient.

- [ ] WorkOS Package A review and identity-readiness report: owner approved the clean Package A review branch as inert/additive and requested source-review publication; merge only if protected review conditions are satisfied. The active `DATABASE_URL` target is confirmed as the current Manus-hosted project production user-data database, not `KINGA-staging`; run only the authorized read-only aggregate/minimal-PII duplicate-email, unverified-email, inactive-user, and tenant-mapping report against that target. No production/staging write, WorkOS setup, dependency, route, callback, provider, or session change is authorized. Report all findings before Package B.

- [ ] Production user-population discrepancy investigation: owner reports only test accounts plus one real admin should exist and has paused Package B/remediation. Run aggregate-only read-only analysis of `is_unregistered_claimant`, login-capability/lifecycle fields, duplicate-email group behavior, `loginMethod`, creation and last-sign-in patterns, tenant mapping, synthetic/test markers, and source references for bulk/seed/training data. Do not select direct PII, propose cleanup, write to any database, or progress WorkOS setup/provider work until the population is explained or clearly classified as unresolved.

- [x] Production user-population discrepancy investigation: aggregate-only read-only analysis of the active Manus-hosted project database and committed identity/seed/load paths classifies the dominant live population as **attributable synthetic/test contamination with the exact historical writer unresolved**. The four 8,120-row duplicate-email cohorts contain 32,480/44,885 users; every cohort row has a test-pattern `openId`, absent login method, unverified email, invalid tenant reference, no agency-assisted linkage, `is_unregistered_claimant = 0`, and `lastSignedIn = createdAt`, so the timestamp is a creation-default artifact rather than login evidence. The post-8-August `LOGIN_ROLE_RESOLVED` audit lower bound is five events for one user, all outside the large cohorts; it cannot establish an exact historic login count because audit writing is non-fatal and predates no earlier history. Committed seed/load utilities explain the 15 QA rows and small fixtures but not this 32,480-row recurring pattern. Evidence: `audit/live-user-population-discrepancy-investigation-2026-09-17.md`. No direct PII/IDs were selected or recorded; no code, schema, account, WorkOS setup, staging/production database state, or other write was changed.
- [ ] **Paused pending owner decision:** WorkOS Package B, WorkOS account/secret/provider work, Package A staging application, provider flags, identity linking, user import, duplicate remediation, cleanup, tenant repair, and all database writes remain unauthorized. Do not treat the synthetic-population classification as authorization to alter any row or relationship.
- [x] Follow-up active-vs-historical and write-access boundary investigation (read-only, aggregate-only): latest full-table `users.createdAt` is 2026-09-11 11:49:37; 0 users were created today/yesterday, but 424 in the preceding 7 days and 7,868 in the preceding 30 days, including clearly test-pattern/unknown-tenant/loginless rows on 8–11 September. Classification updated to recently active, not proven ongoing today. Residual contamination is material: 8,705/12,405 non-headline users carry an inspected test marker; 4,310/7,840 unknown-tenant residual users and 8,699/8,904 loginless residual users do too. The managed current database principal has ALL PRIVILEGES on the live project schema; five running KINGA worktrees have the same live `DATABASE_URL`; no seed/load/migration process was running; in-process background jobs and the sole paused Manus schedule have no direct users writes. Source inspection additionally found public, unauthenticated WhatsApp webhook/test routes which can reach `users` creation conditional on tenant resolution; no Twilio variables were present in inspected runtimes and no endpoint was exercised. Current DB access cannot enumerate external principals, so external/historical writers remain unproven, not excluded. Evidence appended to `audit/live-user-population-discrepancy-investigation-2026-09-17.md`. No write, credential disclosure, route call, code/config change, or remediation occurred.
- [x] Complete the read-only aggregate-only live `users` population-discrepancy investigation and source/runtime correlation: conclusion is confirmed synthetic/test contamination with exact historical writer unresolved. Key evidence: 32,480 rows in four synchronized 8,120-row duplicate-email cohorts, all co-created in 575 four-cohort minute batches over 74 dates (2026-02-12 through 2026-09-11); 41,183/44,885 rows carry the broader inspected test marker; 8,703/12,405 residual rows carry the narrower test marker; no rows are unregistered claimants or agency-assisted-linked; four cohorts have no login method, verified email, valid tenant, or audit event; OAuth audit records only five `LOGIN_ROLE_RESOLVED` events for one user after its 2026-08-08 introduction. Latest row creation was 2026-09-11 11:49:37 with zero later rows as checked, so recently historical but not proven permanently contained. No reviewed committed source explains the cohort scale/cadence. Five active worktrees share one fingerprinted live credential, not five distinct credentials; their shared principal has broad privilege. Source-present unsigned WhatsApp webhook and unguarded test route are documented exposure paths but were not invoked, changed, or used. Full final privacy-preserving report: `audit/live-user-population-discrepancy-investigation-2026-09-17.md`. Package B, WorkOS configuration, Package A staging application, flags, identity linking, any cleanup/remediation, schema/database writes, or external access changes remain explicitly paused and unauthorized.
- [x] Urgent read-only usage assessment for `POST /api/whatsapp/test`: introduced once in commit `2a025fd1` (2026-08-07), with no later functional changes; no reference in Vitest/Playwright, browser automation, package scripts, CI, documentation, development workflow, or source outside its two implementation locations; no retained local log hit. Repository evidence therefore classifies it as orphaned, while not proving an undocumented external caller never used it. Proposal recorded in `audit/whatsapp-test-route-security-proposal-2026-09-17.md`: preferred WA-SEC-01 is removal of the route and handler, not a dev-only guard; separate WA-SEC-02 must add proper Twilio signature validation to the remaining public provider webhook under separate approval. No code/runtime/configuration/schedule/database changes made; monitor remains pending verified WA-SEC-01 closure and explicit daily/weekly selection.
- [x] Draft WA-SEC-02 isolated provider-webhook signature-validation proposal after the owner confirmed this is deliberately deferred work rather than an unexplained gap. Proposal: validate `X-Twilio-Signature` with installed `twilio@6.0.2` SDK against all form parameters and a required fixed canonical `TWILIO_WEBHOOK_URL`, never against Render-proxied request/forwarded headers; fail closed 403 for absent/invalid signatures and 503 for incomplete/invalid provider configuration; test valid, missing, tampered, full-body-tamper, configuration-invalid, GET, and Render proxy-header cases with no engine/database call. Requires future managed configuration/deployment preflight but authorizes none yet. Proposal artifact: `audit/whatsapp-provider-webhook-signature-proposal-2026-09-17.md`. WA-SEC-01 remains independently proposed and unimplemented; no source/runtime/configuration/database/WorkOS/remediation change made.
- [x] WA-SEC-02 approved, implemented, independently security-reviewed, and opened for source review as GitHub PR #96 (`fix/wa-sec-02-twilio-signature`, commit `75288e79b161a2abf800dba66e6daf13f595eb5a`). Scope is the existing provider webhook only: Twilio SDK signature validation over all form parameters and fixed canonical `TWILIO_WEBHOOK_URL`; no proxy-header trust; 403 before intake/database on missing/invalid signature; 503 before intake/database on absent/malformed provider configuration; signed GET verification covered. Added 15 dedicated security tests plus existing identity regression (18 passing total), server bundle and formatting/diff checks passed. Independent security review approved with non-blocking notes; its callback-URL malformed-case test expansion was included before commit. PR is open and CI `code-complete` is in progress. No merge, deployment, Twilio/Render configuration, live endpoint call, database change, data cleanup, WorkOS work, or WA-SEC-01 route removal has occurred. After both WA-SEC-01 and WA-SEC-02 are merged, create the separately authorized daily aggregate-only monitor; before any WA-SEC-02 deployment, return for the owner’s separate Twilio Console/callback and real-traffic preflight authorization.
- [x] Add final WA-SEC-02 verification results record to PR #96: `audit/wa-sec-02-twilio-webhook-signature-verification-2026-09-17.md`, committed as `a15d2b5755219a77f09b70cb905024dad95dd72d` on the isolated review branch and linked in the PR. It records 18 passing focused tests, successful server bundle/format/diff checks, independent security approval, and the important negative result: GitHub `code-complete` failed its repository-wide `tsc` with 998 errors, but the controlled current-main comparison at `211f9b180743d70f800876a780be967612e9a526` produced the same 998 errors and neither output mentions either WA-SEC-02 changed file. The inherited CI failure is documented, not waived; PR #96 remains unmerged and source-review only.
- [x] Merge approved WA-SEC-02 GitHub PR #96 at `228ac4182d5af233627d7ed0ec8102a8e31a097f`; the merged main change adds the verified source-only Twilio signature-validation boundary. No deployment, Twilio/Render configuration, live endpoint call, database change, cleanup, WorkOS work, or monitor creation followed.
- [x] Implement and send approved WA-SEC-01 removal package for review from post-WA-SEC-02 main: GitHub PR #97 (`fix/wa-sec-01-remove-test-route`, commit `55e91b6ffaf7e0204dc10e492b2ccb7528018eb8`). It removes the public unauthenticated `POST /api/whatsapp/test` route and handler, retains only provider GET/POST registration via a testable helper, and preserves WA-SEC-02 unchanged. Regression evidence: 20 focused tests passed across route-removal, signature-boundary, and claimant-identity suites; server bundle, changed-file formatting, and diff hygiene passed; source scan permits the old route literal only in the 404 regression assertion; independent security review approved with no blockers. Results record committed in `audit/wa-sec-01-whatsapp-test-route-removal-verification-2026-09-17.md`. Owner explicitly deferred the daily monitor and all Twilio Console/Render/deployment/real-traffic preflight until separately authorized, even after both source PRs are merged.
- [x] Merge approved WA-SEC-01 GitHub PR #97 at `cecc18a0e8c3c71939f441ff65beb418989c4a3b`; `main` now contains both completed source controls: WA-SEC-02 signed Twilio provider-webhook boundary and WA-SEC-01 removal of the public WhatsApp test route. Neither merge deployed or reconfigured anything.
- [ ] **Pre-live WhatsApp readiness — URL-encoded payload parser ordering:** before real WhatsApp intake is relied on, reconcile the existing global 1 MB URL-encoded parser with the provider route’s intended 5 MB allowance. The current registration order can reject a provider payload above 1 MB before the route-specific 5 MB parser is reached. This finding predates WA-SEC-01, is not a current emergency, and must be handled in a separately reviewed/tested package; no source/configuration/deployment action is authorized now.
- [ ] **Explicitly paused pending owner Twilio Console check:** all deployment preflight, Twilio Console inspection, callback-URL confirmation, Twilio/Render/runtime configuration, live endpoint tests, and daily aggregate-only monitor scheduling. No data cleanup, WorkOS linking, Package A staging work, provider flags, database writes, or credential/access change is authorized.
- [x] Prepare the owner-authorized, read-only live-`users` cleanup review package: `audit/live-users-cleanup-proposal-2026-09-17.md`. Exact current dry-run is **35,085** safe user-only targets of 44,885 total, with an expected 9,800 retained rows. The predicate requires the literal test marker, `is_unregistered_claimant = 0`, absent/unknown tenant, direct owner/known-real exclusion, no observed login audit, no audit history, no claim link, and no reviewed enforced user FK reference. The configured owner directly matched once and is excluded; the sole observed completed-login identity is the same owner and independently excluded. The package preserves—not nullifies or deletes—5,332 claim-linked candidate users and 9,736 linked claims (including 3,794 large-cohort users and 7,588 claims), plus 461 audit-held and 305 FK-held users. No backup/export, deletion, database write, claim change, WorkOS activity, provider configuration, or deployment is authorized or performed. Future execution requires separately approved fresh preflight, encrypted archived exact target rows, isolated restore rehearsal, a frozen-ID transactional delete, and postflight integrity checks.
- [x] Add the owner-requested read-only claim chronology assessment to `audit/live-users-cleanup-proposal-2026-09-17.md`: the 9,736 claim-linked synthetic-marker claims were created in recurring batches from 8 May through 11 September 2026, with the latest September user and claim creation days exactly aligned (1, 8, 9, 10, and 11 September). No held claim was created or updated after 11 September. This is recent, ongoing test activity rather than isolated historic residue; no full test-data reset, backup/export, deletion, update, or configuration action is authorized or performed. A full-reset decision must explicitly establish the disposable scope/cutoff and whether a replacement test corpus is required before any new cleanup proposal.
- [x] Complete the owner-authorized, read-only attribution dry run for approximately 70 owner-processed claims and record the identification proposal in `audit/protected-owner-claims-identification-proposal-2026-09-17.md`. The direct configured-owner match returned **zero** claims across every available actor path: claimant, audit trail/log, claim event, assignment creator/assignee, approval actor, comment author, and document uploader. `claims` has no creator field, and neither claim source nor date is a valid owner-specific substitute. The full reset is therefore not yet proposed. The sole safe next step is an owner-supplied exact protected-claim list or other direct reproducible fingerprint, then a read-only aggregate match/closure check that must land near the expected count before any execution package. No archive/export, deletion, reset, update, claim reassignment, configuration, WorkOS work, or deployment is authorized or performed.
- [x] Complete the owner-requested read-only lifecycle, intake-queue, and synthetic-signature assessment: `audit/claim-lifecycle-queue-and-synthetic-signature-assessment-2026-09-17.md`. The corrected business order is supported by the Claims Processor implementation: Pending Review → In Review → KINGA Complete. The UI’s 18-item count is, with high confidence, the selected tenant’s `intake_pending` / `intake_queue` claim subset (one tenant group has exactly 18; no document-processing queue has exactly 18), while the processor’s Pending Review display is broader and includes other non-complete, non-running states. Batch timing cannot safely replace the owner’s manual protected-claim list: 97.15% of non-marker intake-pending claims and 99.28% of non-marker analysis-complete claims have the synthetic batch-like signature. No selection, archive/export, reset, deletion, update, configuration, deployment, or WorkOS action is authorized or performed; await the owner’s secure exact protected-claim list for an aggregate-only match and dependency-closure dry run.
- [x] Produce the owner-requested read-only document inventory for all Claims Processor dashboard sections: `audit/dashboard-claim-document-inventory-2026-09-17.md`. Pending Review has 540 attached PDF records across 60 claims; In Review has none; KINGA Complete has 1,413 attached PDF records across 157 claims; total 1,953 attachments across 217 claims. Every attachment-bearing claim has the same nine-record category bundle, and 101 separate source-ingestion PDFs are linked to these sections. The report records the attached/source distinction, metadata completeness, ingestion-state inconsistency, and fixture-like timing without exposing filenames, URLs, claim records, or PII. No document was opened, run through KINGA, downloaded, exported, changed, reclassified, or deleted; no claim, user, configuration, deployment, cleanup, or WorkOS action is authorized or performed.
- [x] Complete the owner-requested final automated preservation assessment using claim-number formats and the four approved claim-form source documents: `audit/claim-number-and-approved-document-attribution-assessment-2026-09-17.md`. The DOC format is a documented system-generated document-ingestion format, not a manual-entry marker; it covers only 97 of 18,064 dashboard claims, while the 17,967 non-DOC records include known bulk `CLM`, `TEST`, and `WB` families. Excluding those families leaves only 15 heterogeneous rare-prefix claims, not approximately 70. The four approved claim-form documents map to four distinct non-marker `COR` claims in KINGA Complete, but three share synthetic-user creation days and they are far too few to infer a wider protected set. No automatic protected set is proposed; the only honest next choices remain a separately reviewed full reset with zero preservation or an owner-reviewed exact preservation list. No reset, deletion, export, archive, claim/user/document change, configuration, deployment, or WorkOS action is authorized or performed.
- [ ] **UI defect — Claims Processor KINGA Complete pagination:** The dashboard’s Next control is non-functional client-side, not a backend offset/limit issue. `client/src/pages/ClaimsProcessorDashboard.tsx` fetches up to 200 records with `workflowQueries.getClaimsByStatus({ limit: 200, offset: 0 })`, but the rendered table is hard-coded to `activeClaims.slice(0, 20)` and the `Next →` button has no `onClick` or pagination state. `workflow-queries.ts` supports count, limit, offset, and `hasMore`; its pagination behavior has backend test coverage, whereas the dashboard has no pagination regression test. Implement separately with explicit local page state (reset on tab/search changes), slice bounds, Prev/Next handlers and disabled states, accurate “Showing X–Y of Z” text, and a focused UI regression test proving the second page of KINGA Complete results is reachable. Do not conflate this UI repair with the data-cleanup/reset investigation.
- [x] Resolve the owner’s outstanding approved-source-document lead and append the direct-reference addendum to `audit/claim-number-and-approved-document-attribution-assessment-2026-09-17.md`. The four approved source PDFs link to `COR-6002812-1778764224965`, `COR-6002812-1778764762703`, `COR-6002812-1778764773287`, and `COR-6002812-V2-1779097223383`. They are all assessment-complete, share a `COR-6002812` base reference, and align with historic repository references to the Voltron/Minecor validation claim. Their individual creation minutes contained zero literal synthetic-marker claimant claims and only one or two claims total, so they differ from the high-volume synthetic-claim batch signature. Three occur on a synthetic-user creation day, however, and the group is only four records; classify it as a plausible high-confidence individual preservation candidate group only, never as an inferred 70-claim or wider automatic preservation rule. No live record was changed.
- [x] Produce the owner-authorized, **proposal-only** full test-data reset review package: `audit/live-test-data-full-reset-proposal-2026-09-17.md`. The owner-confirmed preservation rule retains 101 claims (97 document-ingestion-format claims plus four sealed owner-confirmed revision records), their 101 isolated source documents and populated direct evidence/report closure, and the one configured owner account. Current dry run: 17,965 of 18,066 claims and 44,884 of 44,885 users are reset candidates. Live metadata found 55 enforced claim child relations, 17 with reset rows, 74 base claim-reference tables, one reference view, 364 reset recovery cases blocking claim deletion under `RESTRICT`, and six `SET NULL` paths requiring explicit cleanup rather than silent residue. The package requires maintenance freeze, fresh preflight, frozen primary-key manifest, encrypted owner-recoverable archive, separate restore rehearsal, manifest-driven dependency-order deletion, and independent postflight. It explicitly excludes parent-root deletion and all live execution. Await owner review/approval of rehearsal-only package; no archive/export, restore, deletion, reset, implementation SQL, configuration, deployment, WorkOS work, or credential change is authorized or performed.
- [x] Complete the owner-requested fresh, aggregate-only full-reset preflight and record it in `audit/live-test-data-full-reset-fresh-preflight-2026-09-17.md`. It exactly reproduced the reviewed counts: 18,066 claims / 101 protected / 17,965 reset candidates and 44,885 users / 1 owner / 44,884 reset candidates; newest claim and user timestamps remain 11 September 2026. The 97 DOC claims are distributed across 40 days and 95 minutes (only two shared minutes, largest group two), have zero CLM/TEST/WB family overlap, and retain 101 non-shared source documents. Full dependency closure passed: 55 enforced child relations, no protected `RESTRICT`/`NO ACTION` child rows, and the expected 364 reset-only `recovery_cases` blocker remains. The mandatory future order is confirmed: fresh preflight → freeze target set → encrypted archive → proven-readable restore rehearsal on a separate disposable target → referential recheck → reviewed manifest-driven transactional/batched deletion → independent postflight. Archive and successful restore proof must precede every delete statement without exception. This read-only gate authorizes no freeze, archive/export, rehearsal, deletion, reset, implementation SQL, configuration, deployment, WorkOS work, or credential change.
- [x] Clarify the 364 recovery-case blocker before reset authorization: all 364 rows reference distinct existing reset-candidate claims; zero references a protected or unresolved claim; `recovery_cases.claim_id` is the sole enforced parent relation and uses `RESTRICT`; no enforced child table references recovery cases; the only discovered non-enforced correspondence table is empty. Include every recovery case and the empty correspondence-table proof in the encrypted frozen archive before deletion. **Operational gate remains open:** the current live application has active intake-escalation and stuck-assessment-recovery writers, the database reports `read_only`/`super_read_only` OFF, and no complete deployed write-freeze control exists. Do not start an archive or rehearsal until the owner confirms an actual public-app freeze method and owner-recoverable archive encryption/storage destination; stopping the sandbox development server alone is explicitly insufficient.
- [x] Merge PR #98 (`682cbf95`) and activate the owner-approved 19:30–21:30 UTC maintenance-freeze control for the archive/restore rehearsal. The committed server-only `KINGA_MAINTENANCE_MODE=true` gate was independently reviewed and test-proven against the real production Express composition: `/healthz` remains live while readiness, OAuth, WhatsApp form webhook, malformed upload routes, tRPC, and all scheduled HTTP routes return maintenance `503` before parsers or handlers run; intake escalation, stuck recovery, and recovery-deadline startup writers are suppressed. The owner stored the unique archive passphrase. Recent aggregate activity was quiet (no claim/user/audit/ingestion/report/privileged-session activity in 24 hours). The maintenance window is archive-and-rehearsal only; deletion remains explicitly unauthorized pending the owner’s review of restore proof. Non-blocking: the separate WebSocket listener is not gated, but its current implementation was reviewed as non-writing.
- [x] Complete the owner-authorized frozen-window encrypted archive and **separate disposable restore rehearsal**, documented in `audit/live-test-data-full-reset-archive-and-restore-proof-2026-09-17.md`. Public `/readyz` returned `503 MAINTENANCE_IN_PROGRESS`; final live aggregate preflight was unchanged (44,885 users / 18,066 claims; zero new in prior hour; latest timestamp 11 September 2026 11:49:37). A full logical snapshot was streamed directly to owner-passphrase GnuPG AES-256 encryption with no retained plaintext dump. The encrypted archive SHA-256 is `0bc94ee9e78622ccf4b59ab338aaad1ea44ede9808d0e8d02a20641abbd33da6` (175,703,125 bytes). Its isolated local-only restore exactly matched source aggregate fingerprint; 238 tables, 79 foreign keys, and `mariadb-check` verification completed. A restore-only TiDB→MariaDB compatibility stream added four missing auto-increment keys, uniquely renamed 79 legacy FK constraints, removed two invalid blank-name legacy indexes, and mapped one unsupported collation; the encrypted source archive and live TiDB source were never altered. The disposable target and temporary source credential file were removed after proof. **Deletion is still explicitly unauthorized:** await owner review of this restore proof, then obtain a separate final deletion authorization; do not freeze a target set, issue archive/export again, execute reset SQL, change configuration, deploy, alter WorkOS, or change credentials until then.
- [x] Execute the owner-authorized live full test-data reset after a final frozen-set preflight matched exactly (17,965 reset claims; 44,884 reset users; 101 protected claims; one protected owner). The primary transactional, primary-key-manifest deletion removed the approved frozen data set from 33 affected tables with exact per-table counts. A first independent postflight found eight non-enforced legacy user-reference relations still pointing to deleted test users; a separate manifest-keyed transaction removed the 7,299 frozen residual rows and then the full verifier passed. Final live state: 101 claims (97 DOC-format plus four owner-confirmed revision records), one owner account, zero remaining non-protected claims, zero orphaned entries in the validated claim-reference inventory, and zero orphaned entries in the validated user-reference inventory. The verified protected closure remains populated (85 assessments, 300 audit rows, 178 quotes, 173 valuations, 67 evidence ledger entries, seven report snapshots, six evidence findings, three approvals). Evidence: `audit/live-test-data-full-reset-postflight-2026-09-17.md`. The encrypted pre-delete recovery archive remains intact and unchanged. The environment setting `KINGA_MAINTENANCE_MODE` was set to `false` and its focused test passed; **public reopening is still blocked only by a required publish/redeploy because the public deployment continues to serve the prior maintenance configuration.** No further database deletion or reset action is required; publish current configuration and verify public `/readyz` before declaring service reopened.
- [x] Publish the approved maintenance-mode disablement after the verified reset postflight. Public verification at 2026-09-18 10:17 UTC returned HTTP 200 from both `/readyz` (JSON `status: ready`) and `/` (application HTML), with no `MAINTENANCE_IN_PROGRESS` response. The live site is restored. No additional database mutation, WorkOS activity, cleanup action, or WhatsApp configuration change occurred during restoration.
- [x] **Stage 1 — deleted-user re-sync guard status:** KINGA-AUTH-01 is fixed, not active or deprioritized. `server/auth.resync.test.ts` was rerun on 18 September 2026: 1 file / 4 tests passed. It proves absent hard-deleted users fail closed with `User not found`, no OAuth-server recovery call occurs, existing active users use the update-only activity writer, and deactivated users are rejected. Evidence remains `audit/deleted-user-resync-fail-closed-p0-review-2026-08-22.md`; no code change was needed.
- [x] **Stage 2 — daily contamination monitor:** Activated `KINGA Daily User-Contamination Monitor` at 09:00 Africa/Harare, aggregate-only and remediation-prohibited. Because the task supports one recurring schedule, it replaces the previously paused/unexecuted legacy Recovery Deadline Sweep entry. First baseline is clean: 1 user; 0 test-marker, absent-tenant, unknown-tenant, and absent-login-method rows; latest user creation `2026-02-06 10:54:13`. Evidence: `audit/daily-user-contamination-monitor-baseline-2026-09-18.md`.
- [x] **Stage 3A — WorkOS Package A:** Owner-approved inert Package A merged as GitHub PR #95, merge commit `ab1adcc11b564e131dc1c98bb8f99f7566d6c1e0`. The source-only merge adds nullable unique `users.workos_user_id` and `tenants.workos_organization_id` metadata with scratch proof/evidence only; it changes no dependencies, provider, route, secret, callback, runtime behavior, or database. Its focused schema contract passed (2 tests); an attempted direct replay invocation stopped safely because its required explicit disposable database arguments were deliberately absent.
- [ ] **Stage 3B — WorkOS Package B decision required:** Review proposal completed: `docs/authentication/workos-package-b-provider-adapter-proposal-2026-09-18.md`. It proposes an isolated server-only WorkOS AuthKit adapter, pinned `@workos-inc/node` dependency, optional call-time `WORKOS_API_KEY` / `WORKOS_CLIENT_ID` / `WORKOS_REDIRECT_URI` contract, PKCE-aware URL/code-exchange unit tests, and zero routes, callbacks, linking, session, flags, or secret setup. Do not implement until owner approval. Package C remains downstream and unproposed for implementation pending the Package B decision.
- [ ] **Stage 4 — CI/CD decision required:** Review proposal completed: `docs/ci-cd/github-actions-and-branch-protection-proposal-2026-09-18.md`. It proposes a unique `KINGA Quality Gate / quality-gate` workflow for PRs/pushes to main with frozen install, conflict-marker check, TypeScript new-diagnostic baseline comparison, full tests, and production build. Current main has 1,000 TypeScript diagnostics across 102 files; the proposed wrapper fails only on new diagnostics while reporting inherited debt. `main` is currently unprotected. Workflow source/config and later branch protection remain unimplemented pending review.
- [x] **Stage 5 — backlog housekeeping confirmed:** The WhatsApp 1MB/5MB body-parser mismatch remains logged and untouched; WhatsApp is not live. The stale `ImpersonationBanner` `/api/oauth/logout` reference and removable `/api/auth-test` diagnostic endpoint remain durable separate WorkOS follow-ups and are not folded into Packages A/B.
