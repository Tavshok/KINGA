...
- [ ] Record the owner’s D-01-only acceptance that a same-day current Free Starter snapshot is the valid recovery point for any later D-01 decision. The dated acceptance and a proposed `2026-09-12 14:00–20:00 UTC` change window with a two-hour snapshot-expiry safety margin are documented; capture the current `KINGA-staging` snapshot console record for final review only. Do not create accounts, connect to staging, or execute D-01.
