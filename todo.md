...
- [ ] Approved focused L2 formula audit: map and reconcile the existing executable L2 component-selection formula, credibility thresholds, benchmark role, L1 relationship, all-in reconciliation, savings, settlement, report, and client presentation before any formula change.
- [ ] Approved L2 edge-case matrix: execute authoritative formula tests for multiple eligible quotes, one quote, benchmark present/absent, below-floor, above-ceiling, incomplete scope, unreconciled totals, comparison-only/ineligible history, negative savings, and no quote; classify each outcome as intended, defect, or policy decision.
- [ ] Approved L2 traceability report: provide the user a concise source-to-L2-to-report map with exact code locations, business interpretation, confirmed inconsistencies, and controlled correction notices where required.
...
- [ ] AUD-P1-002 Stage 9 quote workflow status: implementation and selection-boundary proof are complete; execute a real no-write `runCostOptimisationStage` mixed-status path before closure.
- [ ] Approved AUD-P1-002 status-versus-evidence boundary: execute mixed submitted, cancelled, commercial/process-rejected, explicitly ineligible, and revised records through the real Stage 9 composite output and downstream history projection without deleting source evidence.
- [ ] Approved AUD-P1-002 L2 eligibility acceptance: execute no-write real Stage 9 output proving only eligible active submissions reach final L1/L2 and `quotesEvaluated`, rather than relying on source-string wiring assertions.
- [ ] AUD-P1-002 downstream eligibility projection: add report-facing acceptance that explicitly suppresses final L2, savings, and settlement conclusions when all available quote prices are comparison-only or ineligible.
...
- [ ] Approved AUD-P1-003 mixed-status result acceptance: execute a no-write real Stage 9 decision fixture for submitted, cancelled, rejected, explicitly ineligible, and revised quotes; prove historical/ineligible prices cannot drive highest-quote, deviation, anomaly, negotiation, recommendation, savings, or settlement conclusions.
...
- [ ] Benchmark-validated L2 regression matrix: formula, ledger, and contract tests pass; complete real no-write Stage 9 mixed-status execution and explicit CL/CI/FR component evidence rendering assertions before closure.
- [ ] Benchmark-validated L2 legacy-gate retirement: remove or deprecate the unused `applyCredibilityGate` P25/P75 path and add regression proof that final L2 selection uses only the confirmed P50/30% policy.
- [ ] Benchmark-validated L2 executed pipeline fixture: run real no-write `runCostOptimisationStage` with mixed workflow, benchmark, tolerance, and high-variance inputs; assert final L1/L2, `quotesEvaluated`, and conclusion suppression.
- [ ] Benchmark-validated L2 rendered-surface acceptance: execute CL, CI, FR, and client `ComponentCostMatrix` rendering for benchmark-selected, outside-30%-lower-value, and >20%-spread rows; prove selection source, benchmark deviation, and variance remark are visible without unsupported savings or settlement treatment.
- [ ] AUD-P1-006 report-surface L2 traceability: implementation is in place; complete generated CL/CI/FR rendering acceptance on benchmark-bearing component evidence before closure.
- [ ] Approved AUD-P1-006 executed report rendering: render benchmark-selected, outside-30%-lower-value, and high-variance component rows through CL, CI, and FR; prove each report presents selection method, benchmark deviation, and variance remark without altering cost authority.
