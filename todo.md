...
- [ ] **P0 — Recovery-deadline sweep broken-function authorization (19 September 2026):** Independent read-only review found that `POST /api/scheduled/recovery-deadline-sweep` accepts any active authenticated human session, then runs the global cross-tenant `checkRecoveryDeadlines()` sweep. The operation can notify the owner using operational case details and update alert timestamps; concurrent HTTP/startup calls have no durable execution lease or notification-effect idempotency. The source record is `audit/recovery-deadline-sweep-authorization-review-2026-09-19.md`. No code, route, scheduler, deployment, credential, database, staging, or production change is authorized by that review. Before an approved source remediation, request an owner-approved deployment/edge containment package that denies interactive traffic and independently verifies the dedicated scheduled caller. A future source package must be distinct from G1 activation and prove exact service authorization, concurrent single-execution/effect idempotency, correct failed-notification behavior, and all-negative tests exclusively on `kinga_ci_test`.
