...
- [x] **P0-A quantitative evidence contract (22 September 2026):** PR #142 merged at `a18d692d`. The source-only P0-1.0 contract classifies current VGE/VGR and raw Stage 6 crush evidence as advisory or unavailable, forbids governing visual crush authority, source-rebinds every decision, and suppresses collision physics, Physics Truth numerical/narrative outputs, causal outputs, and Stage 10 physics publication. The guarded quality gate passed before merge. No migration, DDL/DML, seed, staging/production, credential, or deployment action occurred.
- [ ] **P0-A NB-1 (deferred review note):** Retain the non-blocking label and its PR #142 review context as a tracked follow-up. No implementation is authorised now. The retained final review record has no independent description for this historical label; do not infer or expand its scope without an authoritative review-note source.
- [ ] **P0-A NB-2 — visible quantitative abstention marker (proposed 22 September 2026):** Add a small, separate presentation package that makes P0 advisory/unavailable collision physics visible and actionable in structured output, persisted/report projection, and rendered report/UI. The marker must name the withheld quantitative outputs, explain that descriptive visual evidence did not govern, and identify required next evidence. Proposal: `docs/quantitative/p0-follow-up-package-proposals-2026-09-22.md`. No implementation is authorised by this entry.
- [ ] **P0-A NB-3 (deferred review note):** Retain the non-blocking label and its PR #142 review context as a tracked follow-up. No implementation is authorised now. The retained final review record has no independent description for this historical label; do not infer or expand its scope without an authoritative review-note source.
- [ ] **P0-A NB-4 (deferred review note):** Retain the non-blocking label and its PR #142 review context as a tracked follow-up. No implementation is authorised now. The retained final review record has no independent description for this historical label; do not infer or expand its scope without an authoritative review-note source.
- [ ] **P0-B1 — fraud and automated-decision consumer gates (proposal, 22 September 2026):** Before score, disposition, DOE, or fast-track consumers can use fraud evidence, require an explicit source-bound P0 eligibility result. Withhold raw Stage 6 visual numeric, ungoverned collision derivative, photo-forensic model score, and fallback score from automated fraud/routing outcomes; emit a non-score manual-review reason instead. This is data-integrity-critical and requires separate owner approval, adversarial review, isolated CI tests, and no staging/production change. Proposal: `docs/quantitative/p0-follow-up-package-proposals-2026-09-22.md`.
- [ ] **P0-B2 — cost and valuation consumer gates (planned):** Separately propose fail-closed gates for generated cost, benchmark, CGI, repairability, total-loss, settlement, and valuation conclusions. First obtain an owner decision on positive financial authority sources; do not infer authority from numeric values, confidence, defaults, benchmarks, or record count.
- [ ] **P0-B3 — learning and reference-data consumer gates (planned):** Separately propose gates that block visual/advisory, fallback, proxy, and unqualified numerical inputs from learning, calibration, benchmark, historical-reference, and truth-synthesis writes. First obtain owner allowlists for verified quote, human decision, settlement, and fraud-adjudication provenance.
