...
- [ ] User-authorized managed-workspace recovery: merge `user_github/main` into the local `kinga-replit` history in one non-destructive pass, resolve every conflict explicitly while preserving all local documentation and backlog history, commit the merge before any follow-on action, then save a managed checkpoint. Do not reset, force-push, reinitialize, or alter database/schema/data state.
