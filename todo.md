...
- [ ] P0 fleet document procedure-level acceptance: invoke documents.getClaimContext, documents.byClaim, documents.allByClaim, documents.upload, and documents.delete through their actual tRPC procedure contracts for same-tenant fleet_manager/fleet_admin/fleet_driver authorization and foreign/unrelated-vehicle denial.
- [ ] P0 fleet-admin document authority: add an explicit fleet_admin success proof through the actual document procedures, not only the shared helper.
...
- [ ] Approved platform-wide future-write separation: map every future valuation, insurance service-request, quotation, policy, repair, and settlement writer; retain legacy records as labelled history only; reject any new mixed-workflow persistence path before write.
