...
- [x] Managed synchronization confidence check: completed a real code-touching source-file checkpoint after focused authority regression 5/5, bundled server build, and Vite production build. Managed checkpoint `885b0e7a` succeeded cleanly and is reflected in `user_github/main`.
- [x] User-requested synchronization proof: aligned to GitHub main `c78b48f1`, added a non-functional tenant-authority provenance annotation to `server/routers/inspections.ts`, ran focused and full builds, saved managed checkpoint `885b0e7a`, and verified the committed annotation remains present in a clean workspace afterward.
- [ ] Overnight PR 1 — report consistency regression: create a single-claim CL/CI/FR shared-field consistency test and document every raw-SQL report-generator path without changing report behavior, schemas, or data.
- [ ] Overnight PR 2 — cost tier gating verification: establish whether Process/Protect/Prove tier gating is implemented and enforced in code or remains documentation-only; produce evidence without changing business logic.
- [ ] Overnight PR 3 — isolated deformation calibration first pass: implement and test the approved reference-dimension recalibration module without pipeline wiring, schema changes, migrations, or DDL.
- [ ] Overnight PR 4 — CI health cross-check: assess the post-PR #6/#7 health baseline and identify remaining `ctx.user: User | null` pattern exposure without changing schemas or production behavior.
