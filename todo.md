...
- [x] WorkOS Package A schema contract: added only nullable unique `users.workos_user_id VARCHAR(128)` and `tenants.workos_organization_id VARCHAR(128)` in this isolated review branch. Deterministic four-statement SQL SHA-256 `eae14bcce580a75346edd4813757b795327229701a6f4f64e787486012d255e6`, manifest, focused 2/2 source-contract test, and two independently created/disposed loopback 188-table plus NCU-01 scratch replays passed. Each replay verified the two nullable `VARCHAR(128)` columns, exact unique indexes, duplicate rejection (`ER_DUP_ENTRY`), null acceptance, unchanged 188-table baseline, and zero tables after explicit scratch drop. Evidence: `audit/workos-package-a-2026-09-16/scratch-proof-evidence.md`. The active project database is the authoritative future duplicate-email preflight target; no read has been made. No WorkOS dependencies, secrets, accounts, routes, callback behavior, provider switch, or live staging/production schema action occurred.

- [x] WorkOS production identity-readiness preflight: owner-authorized aggregate-only read executed against the current Manus-hosted active project database `YbS42LwGroxbVepAMjk4bS`, not empty `KINGA-staging`. No direct identifiers were selected and no writes occurred. The automatic verified-email linking gate is blocked: 13 duplicate normalized-email groups affect 32,499 accounts; 40,584 users are unverified; 4,490 lack a tenant; and 40,320 reference unknown tenants across 8,817 distinct tenant values. Detailed aggregate evidence: `audit/workos-package-a-2026-09-16/production-identity-readiness-2026-09-16.md`. Package B/linking, WorkOS setup, and all live schema/provider changes remain unauthorized.
...
- [x] **Stage 3B — WorkOS Package B implemented for review:** Owner-approved Package B was implemented on isolated branch `feat/workos-package-b-provider-adapter`, commit `2211936b5cc8857c416de1d20e304988dc05be38`, and opened as GitHub PR #99. It adds only locked `@workos-inc/node@10.13.0`, an inert server-only AuthKit/S256 adapter, fake-client tests, and review evidence. Two independent AppSec reviews required and then verified redirect-default and malformed-input hardening: no startup config/SDK construction, HTTP loopback rejected by default, typed no-client-call malformed input rejection, no route/callback/linking/session/database/flags/logging/token return. Focused WorkOS + existing resync regression: 35/35 passed; server bundle, formatting, conflict-marker, and changed-path TypeScript checks passed (0 changed-path errors; exact main and branch both 998 inherited diagnostics). PR #99 remains open for source review; no WorkOS account, secret, callback registration, schema application, provider switch, Package C/D, or deployment is authorized.
- [x] **Stage 4 CI dry run:** Completed the owner-required dry run against current GitHub main `ab1adcc11b564e131dc1c98bb8f99f7566d6c1e0`. Frozen install and conflict-marker checks passed; full `pnpm test` failed after 184 seconds: 63 failed files / 112 failed tests, with 489 files / 9,082 tests passing and 2 files / 181 tests skipped. Documented exact result and major failure clusters in `audit/ci-quality-gate-current-main-dry-run-2026-09-18.md`. The CI workflow and branch-protection source PR are deliberately not opened; no test is downgraded or made non-blocking. Owner decision required: repair bounded test debt first, approve a narrow explicit temporary exception policy, or defer CI.
- [x] **WorkOS Package B merge:** Owner accepted and GitHub PR #99 was merged at `d5d784d973d7402855dce5d68fb667280bb77cf6`. The adapter remains inert: no `WORKOS_*` value has been configured, no WorkOS account/callback exists, and no login/session/linking/database/deployment behavior changed. Package C remains a proposal-only next step.
- [x] **CI dry-run failure-origin investigation:** Completed the requested isolated-versus-full and exact pre–Package A parent comparison; full evidence: `audit/ci-dry-run-failure-origin-investigation-2026-09-18.md`. WA-SEC-02 passes 15/15 alone on both current and pre-Package-A commits, but fails 15/15 in the full suite due to reproducible leaked global `fetch` stubbing from `vehicle-structural-intelligence.test.ts` (and related global-fetch test hygiene), not a signed-webhook regression. Maintenance/probe failures have the same full-suite-only fetch-pollution cause. Role-assignment (19), approval-tracking (14), and three reporting fixtures newly fail because Package A schema source includes `users.workos_user_id` while the active test database has neither WorkOS mapping column; all fail before their intended assertions with `ER_BAD_FIELD_ERROR`. Seven reporting-file failures existed in isolated runs on the pre-Package-A parent and remain independent expectation/content debt. No test/source/database/configuration action was taken. CI workflow/branch protection remain paused: first repair test isolation, then separately authorize Package A schema alignment for the test DB, then reconcile pre-existing report tests and rerun the full suite.
- [x] Run the owner-authorized full test suite against one disposable integration worktree containing the two pending CI-remediation packages: CI-ISO-01 (PR #100) and CI-RPT-01 (PR #101). Result: **not green** — 523 files / 9,300 tests passed; 29 files / 66 tests failed; 1 file / 5 tests skipped; duration 245.36s. The seven reporting files specifically repaired by CI-RPT-01 passed (26/26 tests). The full run proved CI-ISO-01 is necessary but insufficient: WA-SEC-01/02 and maintenance/probe HTTP tests again received no response object under full-suite shared state, so an additional test-harness mutation remains. Several operational/real-fixture failures are expected to need self-contained test data after the owner-approved live test-data reset, while authority-sensitive, analytics, portal, pipeline, and static-expectation clusters remain unclassified. Do not merge PR #100 or PR #101 as CI-clearing work, enable branch protection, or introduce a CI workflow until each cluster is traced and a new integrated full run is green. Full result: `audit/ci-full-suite-integrated-validation-2026-09-18.md`.
- [x] **CI safety Priority 1 — dedicated disposable test database provisioned and review package opened.** Local MariaDB `kinga_ci_test` is a separate engine, bound only to `127.0.0.1:33306`, with a host-restricted `kinga_ci_runner` account scoped only to that schema. Its 238-table DDL-only copy has been reinitialized and verified at 0 `users` / 0 `claims`; no application rows were copied. GitHub PR #102 (`chore/ci-isolated-test-database`, commit `89da38843249b0a5deb1751e7093b58fb556be23`) makes `pnpm test` and direct/default/integration Vitest fail closed unless the exact raw `mysql://<userinfo>@127.0.0.1:33306/kinga_ci_test` URL is used; it rejects host/port/path aliases, URL query or fragment overrides, object targets, `socketPath`, and pool clusters before a client is opened. Focused proof: 26/26 passed; two independent reviews identified and closed initial URI/direct-client/pool-cluster gaps; final review passed. `pnpm check:server` passed. Review record: `audit/ci-disposable-test-database-provisioning-2026-09-18.md`. **Do not merge CI-ISO-01 / PR #100, merge CI-RPT-01 / PR #101, rerun a full suite, alter branch protection, or begin the 176-row cleanup until PR #102 is reviewed and merged.**

- [x] **CI safety Priority 2 — real application DB fixture cleanup:** Completed the owner-authorized cleanup under the short-lived maintenance freeze, with a new one-time archive passphrase that was displayed once to the owner and never recorded. A frozen encrypted archive and disposable restore were proven readable. Primary manifest transaction removed 176 test-created users and 138 test-created claims plus exact dependency closure across 32 tables; each deletion count matched its frozen manifest. A strengthened postflight then found and, under the owner’s explicit `Option 2` decision, separately rehearsed and transactionally removed nine protected-claim test-run artifacts (2 regeneration records and 7 report-provenance rows). It also found domain timestamp columns omitted from the initial convention inventory, so a second independent residual transaction removed 6 `claim_events` and 22 `tenant_isolation_violations` rows created by the full suite after the cutoff. Final independent postflight: 1 user, 101 claims, zero post-cutoff non-migration rows, zero physical-FK orphans, zero direct claim/user-reference orphans outside a separately recorded legacy association. Evidence: `audit/fixture-cleanup-operation-log-2026-09-18.md`.
- [ ] **Separate legacy data-contract investigation required:** 97 of the 101 deliberately protected, pre-cutoff claims carry a non-null `claims.claimant_id` that does not match the sole remaining `users.id`. This predates the fixture cleanup, has no physical FK violation, and must not be modified under cleanup authority. Investigate the intended claimant identity model and any safe repair separately before treating it as a fully enforced identity relationship.
- [x] Maintenance configuration was switched to `KINGA_MAINTENANCE_MODE=false` after the clean postflight. The focused maintenance tests passed 4/4 against the dedicated `kinga_ci_test` database. Public republish/health verification is in progress; do not resume the full suite until the restored public configuration is confirmed.
- [x] Publish and verify public service restoration after fixture cleanup: public `/readyz` now returns **200** with `status: ready`, required configuration healthy, and `/` returns 200. The project publish control confirms the latest version is live. `KINGA_MAINTENANCE_MODE` remains `false`.
- [x] Merge approved CI-ISO-01 fetch-isolation GitHub PR #100 at `cdb20be848d8903a5f8608a5766aa19d876cb605` after the fixture cleanup. The isolated full suite is now running from current `main` against only `kinga_ci_test`; its result, not the prior live-database run, will determine next CI remediation and branch-protection work.
- [x] Run the first full suite after merging CI database isolation PR #102 and fetch-isolation PR #100. It was safely confined to the empty, dedicated `kinga_ci_test` loopback database at current main `cdb20be848d8903a5f8608a5766aa19d876cb605`; no application database test writes occurred. Result: **508 passed / 47 failed / 1 skipped test files; 9,287 passed / 132 failed / 9 skipped tests** in 138.28 seconds. The earlier `fetch` pollution is no longer the dominant failure. The remaining independent clusters are: MariaDB-vs-TiDB datetime compatibility in 19 routing tests; self-contained fixture gaps; stale static/topology assertions; a `Too many connections` harness failure; and runtime/fixture-semantic failures. Full evidence: `audit/ci-full-suite-isolated-database-result-2026-09-18.md`. CI workflow and branch-protection activation remain blocked; no test exception, test weakening, branch-protection change, or bulk assertion update is authorized.
- [x] **Routing timestamp precision assessment:** TiDB round-trip proved ISO-8601-with-`Z` is correct and `routing_history` currently contains zero rows, so no live same-second collision can be measured. The source explicitly permits multiple append-only routing decisions per claim but orders history only by `timestamp DESC`; because the schema is `TIMESTAMP(0)`, two decisions within one second could have ambiguous display order. Record as a **minor deferred data-contract backlog item**: assess `TIMESTAMP(3)` together with a deterministic secondary ordering rule (for example routing version or immutable ID) before routing history becomes operationally populated. No schema change is authorized.
- [x] **Proposal ready — usage-meter JSON integrity:** `audit/usage-meter-json-metadata-proposal-2026-09-18.md`. The native JSON `usage_events.metadata` column is currently fed a pre-stringified object. The proposed narrow correction passes the object directly, tightens `any` to `unknown`, and adds isolated database tests proving `JSON_TYPE(metadata) = OBJECT`. Live aggregate state has zero usage events, so no historical repair is needed. No implementation, schema, data, dependency, configuration, billing, WorkOS, or deployment change is authorized.
- [x] **Proposal ready — vehicle-registry required containment audit:** `audit/vehicle-registry-critical-audit-proposal-2026-09-18.md`. The proposed narrow change adds a strict claim-event writer only for the cross-tenant vehicle-containment event, preserves best-effort semantics for ordinary analytics events, rethrows the named required-audit error through the registry boundary, and makes the fire-and-forget caller attempt a non-PII owner alert. Focused tests cover success, persistence failure visibility, one alert attempt, and unchanged generic event behavior. No implementation, schema, data, provider, WorkOS, or deployment change is authorized.
- [x] **Usage-event JSON metadata integrity:** owner-approved forward-only correction was reviewed, merged as GitHub PR #103, and verified on the dedicated local CI database. Both direct `usageEvents` writers (`server/services/usage-meter.ts` and legacy `server/metering.ts`) now pass native object/null metadata instead of pre-stringifying it; focused canonical and legacy tests assert `JSON_TYPE(metadata) = OBJECT`. No historical usage-event values were backfilled or changed.
- [x] **Vehicle-registry containment audit durability:** owner-approved correction was reviewed, merged as GitHub PR #104, and verified on the dedicated local CI database. Only the cross-tenant containment event is fail-visible; normal `emitClaimEvent` behavior remains best-effort. A required persistence failure reaches the existing fire-and-forget boundary and makes one non-PII operational owner-alert attempt; alert failure cannot interrupt the pipeline.
- [ ] **CI connection lifecycle — source review PR #105:** reviewed package `fix/ci-connection-lifecycle` at `7b22f1fe` adds an explicit process-local pool close helper and per-file Vitest teardown using `vi.importActual` to bypass mocked `./db` modules safely. Full-suite telemetry reduced peak `kinga_ci_test` connections from 151 to 21 and eliminated `Too many connections` / `ER_CON_COUNT_ERROR` failures. Focused pool-recreation and mocked-module tests, server bundle, diff check, and independent review passed. PR #105 is open and unmerged.
- [ ] **CI stale assertion reconciliation — source review PR #106:** reviewed, test-only stacked package `fix/ci-stale-assertions` at `1e6401fa` updates 16 individually traced assertions/fixtures to current modular source locations and contracts. It preserves tenant predicates/helpers, portal-domain policy, L1/L2 evidence/incomplete-scope guards, readiness behavior, and write-off schema shape. Focused result: 16 files / 103 tests passed; server bundle and diff check passed; independent review approved. PR #106 is open, unmerged, and intentionally based on PR #105.
- [x] **Post-reconciliation guarded full suite:** ran only through `pnpm test` against reset local `kinga_ci_test` from the stacked PR #105/#106 branch. Result: 549 passed / 12 failed / 1 skipped files; 9,411 passed / 26 failed / 4 skipped tests; 258.13 seconds. Telemetry confirms peak 21 connections, zero `Too many connections`/`ER_CON_COUNT_ERROR`, and zero mocked close-helper errors. The CI database was reset empty after the run. Evidence: `/home/ubuntu/kinga-ci-reconciliation-result-2026-09-18.md`.
- [ ] **Remaining CI reconciliation — individual only:** before any CI workflow or branch-protection decision, trace and repair the 12 still-failing isolated files individually: self-contained fixtures (`audit-01`, assessor ecosystem, claim comments tenant authority, analytics/tenant/tenant-isolation routers); dataset/document fixtures (`dataset-capture-activation`, `dra`); legacy real-ID test (`e2e-real-claim`); remaining modular source-location assertions (`batch4-audit`, `routers/search-performance`); and logout behavior (`auth.logout`). Do not bulk-update assertions, reintroduce live-DB tests, or make CI non-blocking.

- [x] **Final isolated CI reconciliation:** all remaining twelve previously failing files were traced individually and repaired without bulk assertion changes or any live-database access. The resulting package self-contains fixture closures, replaces historic IDs with test-owned CI data, reconciles source-location drift, and aligns current logout/document-gate contracts. Two trace-discovered runtime corrections were retained: MariaDB-compatible numeric JSON recipient predicates with full tenant constraints, and legacy assessor-report JSON normalization.
- [x] **Assessor-review acceptance integrity:** independent review found that acceptance could commit before its evaluation/audit projection. The final correction runs the accepted review, conditional single-winner state claim, same-tenant supersession, report status, evaluator projection, and audit entry in one transaction. Regressions prove legacy nested JSON-string acceptance, malformed-payload preflight non-mutation, rollback on an injected post-decision evaluator FK failure, and one winner/one evaluation under concurrent acceptance requests.
- [x] **Guarded full-suite result:** after a disposable `kinga_ci_test` reset, `pnpm test` completed with 561 passed / 1 skipped files and 9,439 passed / 4 skipped tests in 251.73 seconds. `pnpm check:server` and `git diff --check` passed. The runner’s isolation integration test passed in the same run. Evidence: `audit/ci-final-isolated-suite-green-2026-09-18.md`.
- [ ] **CI workflow and branch protection decision:** the local isolated suite is green. Prepare or revisit the previously approved GitHub Actions workflow source package only after this reconciliation review package is approved and merged. Do not enable branch protection until the workflow is merged, observed passing on GitHub, and separately approved.

- [x] **GitHub Actions quality-gate implementation:** created a repository-only `KINGA Quality Gate / quality-gate` workflow for push/PR activity on `main`. The source package provisions only a loopback MariaDB 10.11 service with the committed schema-only snapshot, verifies UTC and `IGNORE_SPACE`, and runs frozen install, conflict checking, TypeScript diagnostic-baseline comparison, guarded tests, and production build. Actions are full-SHA pinned, permissions are `contents: read`, and no deployment, provider configuration, GitHub branch-protection change, or live/staging database access occurs.
- [x] **CI hermetic-provider enforcement:** direct storage, LLM, and OAuth transport calls now fail closed in test mode before credentials or HTTP requests. Regressions prove storage/LLM `fetch` and OAuth Axios transport receive zero calls; upload/report operational acceptance tests use test-local storage, authentication, and pipeline-boundary mocks. The platform-super-admin asynchronous middleware assertions are now awaited.
- [x] **Quality-gate dry run:** final local command sequence passed against the disposable database: provisioning verified 238 tables, UTC, and `IGNORE_SPACE`; conflict check passed; typecheck baseline reported 1,001 baseline/current diagnostics with 0 new and 0 resolved (86 message-only presentation changes and 1 exact inherited line relocation reported); `pnpm test` passed 563 files with 1 skipped and 9,442 tests with 4 skipped; and the production build passed. Evidence: `audit/ci-quality-gate-current-main-dry-run-2026-09-18.md`. Branch protection remains deliberately unmodified pending hosted PR/main observation and separate owner approval.

- [x] **Hosted quality-gate environment reconciliation:** PR #108’s first two hosted attempts stopped before dependency installation because `pnpm/action-setup` rejects duplicate workflow/package-manager version sources; workflow now reads the exact repository `packageManager` pin only. The next hosted attempt completed checkout, setup, isolated DB provisioning, conflict checking, and baseline comparison, then exposed six individually traced CI-environment defects: six developer-specific absolute source paths, missing local `pdftoppm`, an absent test-only JWT signing key, and three unmocked notification-provider boundaries. Repairs are test/workflow-only: portable `import.meta.url` source reads, `poppler-utils` installation, a disposable Vitest-step-only JWT key, and scoped notification mocks that retain real email payload, approval persistence, and quote authority assertions. Focused repaired targets passed 36 tests with 2 intentional skips; the complete local quality-gate sequence passed 563 files with 1 skipped and 9,442 tests with 4 skipped. A corrected hosted rerun is required before merge; branch protection remains untouched.
- [x] **Hosted session-environment completion:** the final hosted residual was correctly diagnosed as a missing disposable application identifier in session payloads, not a JWT integrity defect. The Vitest step now scopes `VITE_APP_ID=kinga-ci-test-app` alongside its disposable JWT key. A constrained full local reproduction with host application/JWT/Forge variables removed and only workflow-equivalent disposable inputs passed every gate: 563 files with 1 skipped, 9,442 tests with 4 skipped, zero new TypeScript diagnostics, and production build. Corrected GitHub rerun remains required before merge; branch protection remains untouched.
- [x] **Hosted quality-gate observation:** PR #108 final run `35407810791` passed all steps in 3 minutes 46 seconds, including isolated MariaDB provisioning, guarded full Vitest suite, TypeScript debt regression comparison, and production build. GitHub published the intended stable context `KINGA Quality Gate / quality-gate`. PR review/merge remains separate; branch protection remains deliberately unmodified until the workflow is merged, observed on `main`, and separately approved.
- [x] **Quality-gate protection rollout:** PR #108 merged at `a3551818519298874f9d72ea0e8112fe9d0a9257`; `KINGA Quality Gate / quality-gate` passed on `main` (run `35422814901`). Branch protection now requires the app-bound GitHub Actions `quality-gate` check, requires branches to be current, applies to administrators, requires resolved conversations, disallows force pushes/deletions, and temporarily requires zero approving reviews. PR #109 deliberately failed the required gate; GitHub rejected the merge attempt and the probe was closed without merge.
- [ ] **Temporary sole-operator governance:** when a second person with write access exists, change `main` protection from `required_approving_review_count: 0` to one approval and enable stale-approval dismissal. This is a deliberate temporary accommodation because GitHub prevents pull-request authors from approving their own pull requests; it is not a permanent security posture.
- [ ] **Legacy CI/CD Pipeline retirement:** review-only branch `ci/retire-legacy-pipeline` deletes the overlapping permanently red raw-TypeScript CI/CD workflow after quality-gate rollout. The new quality gate remains the sole required validation; dashboard and external-staging workflows remain untouched. The deletion rationale, removed `develop` trigger, and non-deploying manual release-notification path are documented in `audit/legacy-cicd-pipeline-retirement-proposal-2026-09-19.md`. Await separate owner approval before merge.
- [x] **CI/CD closure (19 September 2026):** PR #110 merged at `a80b88e05dd21b5532288ffc42a4650de32e4216`, retiring only the overlapping raw-TypeScript `.github/workflows/cicd-pipeline.yml`. The **KINGA Quality Gate** remains the sole required main-branch validation. Its hosted check passed on `main`; protected-branch enforcement was previously demonstrated by deliberately failing PR #109, whose merge was blocked and which was closed unmerged. `docs/02-architecture.md` records the active isolated-test, baseline-typecheck, build, and protection controls. The older retirement-pending entry above is superseded by this closure record.
- [ ] **Temporary sole-operator protection exception:** retain `required_approving_review_count: 0` only while a second person with write access is unavailable. When one exists, change the rule to require one approval and dismiss stale approvals before treating the branch-protection posture as steady-state.
- [ ] **WorkOS Package C1 — local auth-transaction proposal:** review-only document at `docs/authentication/workos-package-c-local-auth-transaction-proposal-2026-09-19.md` proposes an inert server-side transaction contract, short-lived browser binding cookie options, S256 PKCE material, safe internal return paths, and durable atomic-consume interface. It expressly excludes routes, durable-store implementation, WorkOS configuration or account setup, provider exchange, local identity linking, session issuance, client navigation, schema/data, staging, production, and deployment. A future Package D must supply and review durable atomic storage before any callback can call the contract; no in-memory fallback is permitted.

- [x] **WorkOS Package C1 implementation:** added only the approved inert local authentication-transaction contract, transaction-binding cookie options, and focused unit tests on `feat/workos-local-auth-transaction`. The contract has no route or provider caller, no production storage implementation, and no database/network/session/linking behavior. It requires a future Package D durable atomic-consume store before any callback can use it. Focused guarded tests passed 3 files / 60 tests; server bundling, changed-path diagnostic check, formatting, and diff checks passed. Evidence: `audit/workos-package-c1-local-auth-transaction-implementation-2026-09-19.md`. Await independent review and source PR; Package D remains separately gated.
- [x] **WorkOS Package C1 final independent review:** approved after two review-discovered return-path hardening passes. The final contract fully decodes bounded input to a fixed point; rejects nested encodings, protocol-relative paths, traversal, backslashes, ASCII/C1 controls, and literal/encoded/trailing-slash aliases of `/login` and `/portal-hub`; and returns one canonical local path. Callback state and browser binding must be exactly 43-character base64url values before any hash/store operation. Focused guarded validation passed 3 files / 74 tests; independent review approved; `check:server`, formatting, diff, and changed-path diagnostic comparison passed. Ready for narrow source-review PR; no Package D scope has been introduced.
- [x] **WorkOS Package C1 publication:** PR #113 merged at `9c3ada51606cf14e7c9668bc54331fe2b738188c`. The protected `main` hosted quality gate passed at run `35441791070`. C1 remains route-free and does not activate WorkOS login; its only production-facing consequence is the availability of inert local contract code for a separately approved Package D.
- [ ] **WorkOS Package D callback and guarded-linking proposal:** review-only proposal at `docs/authentication/workos-package-d-callback-linking-proposal-2026-09-19.md` defines a default-off start/callback path, durable atomic transaction requirement, verified-email and exactly-one-existing-user admission, tenant/organization equality, null-or-equal guarded `workos_user_id` linking, and existing KINGA JWT issuance only after all gates pass. It forbids account/tenant/mapping creation, provider token storage, client navigation changes, WorkOS configuration, feature enablement, staging/production action, and Manus-path modification. Implementation remains pending owner review and explicit approval.
- [x] **WorkOS Package D proposal review:** independent review approved the corrected callback/local-linking proposal. It requires a durable atomic C1 transaction store; narrowly bounded/redacted OAuth `state` and `code` query handling; canonical `/api/auth/workos/callback` configuration/route invariant; bounded expired-PKCE cleanup; and a locked exactly-one canonical-email recheck in the guarded local link. The proposal retains exact tenant/organization equality, null-or-equal WorkOS-ID linking, no auto-provisioning, default-off routes, existing KINGA JWT authority, isolated test-owned fixtures, and no current external/staging/production action. Review-only PR publication is ready; implementation still requires separate owner approval.

- [x] **WorkOS Package D implementation — default-off callback and guarded local link (19 September 2026):** Added `workos_auth_transactions` via migration `0061` and the disposable CI schema snapshot. The new transaction store persists only hashed browser values and a short-lived PKCE verifier, atomically deletes a consumed record, and deletes expired records encountered during callback consumption. Added a default-off server route mount behind `WORKOS_HUMAN_AUTH_ENABLED === "true"`; absent/incomplete configuration does not create an active authentication path. The start/callback pair uses a host-only, HttpOnly, SameSite=Lax binding cookie, canonical callback URI, no-store/no-referrer responses, query-free generic terminal redirects, start compensation cleanup, and dedicated rate limiting. The guarded linker accepts only one existing active human user whose canonical verified provider email, active non-synthetic local tenant, and pre-existing WorkOS organization mapping all match; it never creates a user or tenant and conditionally writes only a null `workos_user_id`. Focused isolated tests passed 95/95 across 7 files; `check:server`, migration scratch replay, and changed-path TypeScript comparison passed. Independent final security review and source review PR publication remain required before merge. No WorkOS secret/configuration, provider callback registration, feature activation, client navigation, staging, production, or live data change is included.
- [x] **WorkOS Package D independent-review remediation (19 September 2026):** The first independent review blocked publication on three concrete gaps: missing Drizzle journal registration for migration `0061`, no bounded cleanup for abandoned expired PKCE verifiers, and missing route-level terminal-failure coverage after provider exchange/local-link/session phases. Remediation registered migration journal entry 61, added a normal Drizzle migration-reader regression and a local scratch replay that seeds the 0060 watermark then invokes the normal migrator, added an enabled-feature-only immediate-plus-hourly cleanup loop and integration/unit evidence, and extended callback tests to verify the same generic no-store/no-referrer redirect, binding clear, and absence of a KINGA session cookie after exchange failure, link denial, persistence unavailability, and session issuance failure. The remediated focused suite passed 105/105 across 9 files; `check:server` and changed-path TypeScript comparison passed. A fresh independent re-review remains required before source review publication.
- [x] **WorkOS Package D final independent security review (19 September 2026):** Second review approved the remediated source package with no required changes. It confirmed journaled migration/replay, bounded enabled-only verifier cleanup, uniform terminal callback failures, default-off composition, database-only replay protection, canonical callback URI, guarded existing-user local linking, KINGA session authority, and non-sensitive Package D logs. The reviewer also observed a green guarded full suite; repository CI will reproduce this independently on the source review branch. Package D is ready for commit, push, and review PR only; merging and every activation/configuration decision remain separately gated.
- [x] **WorkOS Package D publication and activation sequencing (19 September 2026):** PR #115 merged at `87f2907c21ff1f5182808a89c47a6924ed24a705` after the required KINGA Quality Gate passed. The owner-approved disabled-path proof started the application against a disposable local scratch database that deliberately lacked `workos_auth_transactions`; with `WORKOS_HUMAN_AUTH_ENABLED=false`, `/readyz` returned 200. Code-path verification confirmed that the WorkOS route, transaction store, and verifier-cleanup loop execute only inside the exact enabled gate. The mandatory future activation order is recorded in `docs/02-architecture.md`: apply migration `0061`, verify the table and both expected indexes, configure server-only WorkOS values/canonical callback URI, then enable the exact server gate for an explicitly approved environment. This is a deployment dependency, not a browser decision. No WorkOS configuration, flag activation, staging, production, or live-data action occurred.
- [ ] **WorkOS Package E proposal — provider-neutral client navigation (19 September 2026):** Review-only proposal at `docs/authentication/workos-package-e-provider-neutral-client-navigation-proposal-2026-09-19.md` defines a non-behavior-changing client facade that replaces the nine existing client Manus-login/recovery integration locations while preserving the active Manus URL, callback, state, local-storage return-path, recovery, and session behavior. It prohibits all browser WorkOS selection/requests/configuration, client-side feature activation, server callback changes, transaction/linking changes, session changes, migration/data changes, and deployment actions. Package F local session separation, Package G non-human identity separation, and Package H controlled cutover remain separately proposed and gated.
- [x] **WorkOS Package E proposal review (19 September 2026):** Independent architecture/security review initially blocked only a governance ambiguity: the proposal’s earlier closing language could have been read as authorizing source implementation. The document was corrected to state that approval authorizes only the design boundary and that any branch, source/test change, or pull request requires a later explicit owner implementation decision. Final independent review approved the corrected proposal with no required changes. It confirmed that Manus remains preserved, WorkOS remains entirely client-unreferenced/default-off, and the migration-then-verify-then-configure-then-enable activation sequence is correctly recorded.

- [x] **External Staging Foundation CI isolation repair (19 September 2026):** PR #115’s non-required `Runtime and external-staging contract` check was red because `external-staging-readiness.yml` invoked raw Vitest while the repository test guard correctly requires the dedicated disposable database URL. The failure did not contact external staging; the workflow is a non-deploying source/runtime-contract check. The repair adds a loopback-only ephemeral MariaDB 10.11 service pinned by immutable manifest digest, validates the committed disposable schema through `scripts/ci/provision-isolated-test-db.mjs`, and invokes only the two runtime tests through guarded `pnpm test`. It has least-privilege `contents: read`, immutable SHA-pinned actions with verified provenance, a timeout, and cancellation concurrency. Local guarded validation passed 8/8 tests, conflict check, server bundle, YAML formatting, and diff check. Independent DevSecOps review initially blocked the mutable image tag and an inaccurate pnpm action comment; both were remediated and final review approved. No live/external database, staging endpoint, deployment, cloud credential, provider call, migration, or data action is included. Source review PR publication remains required.

- [x] **WorkOS Package E implementation — provider-neutral client navigation (19 September 2026):** Replaced the nine documented browser integration locations with the small client-owned `client/src/auth/login-navigation.ts` facade. It preserves current Manus `/app-auth` URL construction, plain-base64 callback-only `state`, browser-local return-path behavior and loop exclusions, account-recovery URL, redirect labels, and session authority. It deliberately contains no browser WorkOS route, request, value, feature selection, or provider switch. The formerly eager optional `useAuth` login URL is now constructed only if its unused redirect option is requested; this is behavior-preserving. Focused isolated regression passed 6/6 tests; `check:server`, selected-file Prettier, `git diff --check`, and normalized comparison with current `main` passed with zero new TypeScript diagnostics. The existing post-login specification now models only the actual legacy plain-base64 callback state rather than a hypothetical JSON return-path state; its legacy whole-file Prettier drift was intentionally not rewritten. Independent security/frontend review approved with no required changes. Source review PR publication remains required; Package E does not activate WorkOS or change live Manus behavior.

- [x] **External runtime-contract and Package E closure (19 September 2026):** PR #118 merged at `e716f8790c59cfe597ae3e5d1a9e6581b2b33b56` after both the runtime-contract and quality-gate checks passed. The non-deploying runtime workflow now provisions only its immutable-pinned, loopback disposable MariaDB target and invokes guarded `pnpm test`; it does not contact external staging. PR #117 merged at `a7c960ba0dd6395f9f1c52b09499b4d82de78b5a` after a current-main quality-gate rerun passed, completing Package E’s provider-neutral client navigation facade. The architecture document now records that every repository workflow that runs tests inherits the fail-closed disposable-database guard; raw Vitest is prohibited.
...
- [ ] **Recovery deadline automation rebuild (deferred 20 September 2026):** Recovery Portal lacks proactive deadline notifications (90/60/30/14/7-day, lapsed-case) and a true stalled-case indicator since the vulnerable automated sweep was removed. Rebuild this properly as a real feature — using the already-designed REC-SEC-02 fencing/lease/outbox pattern — when recovery case volume justifies it, not as emergency security work. The source portal has only a 90-day approaching count and the click-through currently does not produce a deadline-filtered queue.
- [ ] **Deferred claimant-id zero-sentinel backfill (19 September 2026):** When WorkOS Package G2 is actually being designed, prepare and separately authorize a narrow, evidence-led data transition for the 98 protected document-ingestion claims with `claims.claimant_id = 0`, changing only the fabricated sentinel to `NULL` after a current downstream-impact and referential review. This is deliberately deferred: `NULL` will remove false “Driver 0” attribution from analytics and align existing document-ingestion records with G2’s unlinked/unknown-claimant eligibility semantics. It must not be bundled into current forward-only source remediation, Package G2 design, or any unauthorised production data write.
- [ ] **Approval-notification recipient tenant hardening (19 September 2026):** In a separately reviewed security-hardening package, bind `notifyClaimApproval`’s known-claimant user lookup to the expected tenant (or establish an equivalent explicit claimant-to-claim tenant-integrity proof) rather than resolving only `users.id`. The current technical-approval mutation already resolves the claim tenant-safely and the 2026-09-19 null-claimant correction does not expand the pre-existing bare-ID lookup; this is defense in depth, not a blocker for PR #127.
