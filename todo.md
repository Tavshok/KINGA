...
- [x] Pipeline still stuck at Extracting after presigned URL fix — deep investigation needed
- [x] Trace full pipeline execution path from triggerAiAssessment to completion
- [x] Fix all blocking points in extraction pipeline
- [x] Ensure full pipeline completes end-to-end and produces correct forensic report

## Full Pipeline Quality Overhaul
- [x] Run full pipeline end-to-end on real claim and capture every stage output
- [x] Audit each stage for data loss, misinterpretation, missing fields
...
## Forensic Report Quality Improvement (Admissibility, Coherence, Readability)

### Bug Fixes
- [ ] Fix draft print bug: add `position: relative` to `.kinga-report` container so absolute watermark is scoped
- [ ] Fix print CSS: `.kinga-report *{color:#111 !important}` is too broad — use targeted selectors to avoid overriding inline coloured text
- [ ] Fix: Ensure fraud score on cover page matches fraud score in Section 5 (both use same source chain)
- [ ] Fix: Ensure physics score on cover matches physics score in Section 2

### Admissibility & Legal Language
- [ ] Add AI system disclaimer to cover page ("generated by automated AI system, does not constitute legal advice")
- [ ] Qualify all DECLINE/APPROVE/REVIEW verdicts as "KINGA recommendation" not final decisions
- [ ] Add methodology footnote to each scored section explaining how the score was derived
- [ ] Replace vague "anomaly detected" with specific factual statements citing evidence
- [ ] Ensure all fraud signal descriptions cite the specific evidence, not just the signal name
- [ ] Add "Limitation of Liability" note to report footer
- [ ] Add chain-of-custody note for uploaded documents (what was uploaded, when)

### Coherence & Consistency
- [ ] Standardise score labels: "0–39 Low / 40–69 Moderate / 70–100 High" consistently across all sections
- [ ] Standardise missing value placeholder: use "Not provided" consistently (not "—", "N/A", "Not extracted")
- [ ] Remove duplicate "Final Risk Statement" that appears in both Section 5 and Section 6
- [ ] Add cross-references between sections (e.g., "See Section 2.6 for speed analysis")
- [ ] Ensure section numbering is consistent (no gaps like 1.1 → 1.6)
- [ ] Standardise date format: DD Month YYYY throughout

### Readability & Narrative Quality
- [ ] Add plain-English "What this means" paragraph after each scored section
- [ ] Improve Section 0 verdict banner: add 2-sentence plain-English summary of decision rationale
- [ ] Add "Key Findings" summary box at top of each major section (3–5 bullet points)
- [ ] Improve Section 5 fraud signal descriptions: each signal needs a "why this matters" sentence
- [ ] Add "Recommended Next Steps" as numbered action list in Section 6 (not buried in prose)
- [ ] Improve Section 7 benchmark: add context sentence explaining what the benchmark means for this claim
- [ ] Replace technical jargon in adjuster-facing sections with plain English

### Supporting Visuals (Aid Understanding)
- [ ] Add fraud signal waterfall chart (Section 5): each signal's contribution to total fraud score
- [ ] Add cost comparison bar chart (Section 3): submitted quote vs benchmark
- [ ] Add workflow timeline (Section 6): visual dots showing approval process position
- [ ] Add report integrity seal (footer): hash, timestamp, pipeline stages completed
- [ ] Improve KPI tile score bars: add zone labels (Low/Moderate/High) under each bar

### Structure & Page Flow
- [ ] Move "Data Completeness" and "Extraction Confidence" panels to top of Section 1
- [ ] Collapse Section 1.4 Driver Details into single compact table
- [ ] Add page break before Section 4 (Evidence Inventory)
- [ ] Remove redundant sub-section headers that repeat the section title
- [ ] Ensure Section 7 (Benchmark) only renders when benchmark data is available

### Typography & CSS
- [ ] Add `position: relative` to `.kinga-report` container
