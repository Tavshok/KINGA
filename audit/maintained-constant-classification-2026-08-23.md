...
| Stale, changed, or now-named since the maintained scan | 46 | Present in the old scan but absent from the current matching source context after verified extractions |
| Current matches remaining to classify from live source | 46 | This is the true remaining review pool, not 170 |
...
The maintained scan’s true remaining scope is already **materially smaller than 170**. After the verified initial extractions, the remaining review pool is **46 current matches**; it will shrink further as already-named values, prompt examples, and grouped request-budget literals are excluded with rationale.
