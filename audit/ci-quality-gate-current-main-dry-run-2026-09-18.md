...
The full suite completed in **157.15 seconds**. The production build completed in **35.17 seconds**. Build output retained existing chunk-size warnings only; it did not deploy or publish an artifact. The full-run log contained no CloudFront, Amazon Web Services, presigned-URL, storage-upload, LLM-invocation, or OAuth-exchange request indicator. The SDK’s configuration-only startup messages identify its configured base URL but do not create an HTTP request; direct LLM, OAuth, and storage calls are now rejected in test mode before credentials or transport calls are used.
