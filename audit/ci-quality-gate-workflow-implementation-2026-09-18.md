...
The only JWT key in the workflow is a fixed, non-secret value scoped to the **Full Vitest suite** step. The same step supplies a fixed disposable application identifier for session-payload validation. These values exist solely to sign and verify ephemeral test cookies inside the fresh MariaDB service, never appear in a deployment or runtime step, and must not be reused outside this test job.
