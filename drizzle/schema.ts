...
export const aiAssessments = mysqlTable("ai_assessments", {
	...
	fraudRiskLevel: mysqlEnum("fraud_risk_level", ['low','medium','moderate','high','critical','elevated']),
	...
	// Final pipeline recommendation: APPROVE | REVIEW | REJECT | ESCALATE | NEGOTIATE | PROCEED_TO_ASSESSMENT
...
export const historicalReplayResults = mysqlTable("historical_replay_results", {
	...
	fraudRiskLevel: mysqlEnum("fraud_risk_level", ['none','low','medium','moderate','high','critical','elevated']),
...
export const quotationRequests = mysqlTable("quotation_requests", {
	...
	// Phase 7: Report gating, standalone valuation, fleet inspection
	reportGatingStatus: mysqlEnum("report_gating_status", ['teaser','full','paid']).default('teaser').notNull(),
	reportUnlockedAt: timestamp("report_unlocked_at", { mode: 'string' }),
	isStandaloneValuation: tinyint("is_standalone_valuation").default(0).notNull(),
	paymentIntentId: varchar("payment_intent_id", { length: 255 }),
	inspectionRequired: tinyint("inspection_required").default(0).notNull(),
	inspectionAssignedTo: int("inspection_assigned_to"),
...
export const tenantRoleConfigs = mysqlTable("tenant_role_configs", {
	tenantId: varchar("tenant_id", { length: 64 }).notNull(),
	roleKey: mysqlEnum("role_key", ['executive','claims_manager','claims_processor','assessor_internal','assessor_external','risk_manager','insurer_admin','recovery_officer']).notNull(),
	enabled: tinyint().default(1).notNull(),
