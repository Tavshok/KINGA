/** Gate C Wave 5 loopback-only composed scratch replay runner. */
...
import { PREREQUISITE_TABLES, WAVE_FIVE_TABLES, validateWaveFiveSql } from "./gate-c-wave-five-contract.mjs";
...
export function validateScratchTarget(databaseUrl) {
  ...
  if (target.protocol !== "mysql:") throw new Error("Gate C Wave 5 accepts only mysql:// scratch URLs.");
  if (!ALLOWED_HOSTS.has(target.hostname)) throw new Error("Gate C Wave 5 rejects non-loopback database targets.");
  const database = target.pathname.replace(/^\//, "");
  if (!/^kinga_gatec_[a-z0-9_]+$/i.test(database)) throw new Error("Gate C Wave 5 requires a uniquely named kinga_gatec_* database.");
...
async function main() {
  ...
  if (!databaseUrl || !waveOnePath || !waveTwoPath || !waveThreePath || !waveFourPath || !waveFivePath || !evidenceDir) throw new Error("Usage: --database-url mysql://... --wave-one-sql /absolute/wave-1.sql --wave-two-sql /absolute/wave-2.sql --wave-three-sql /absolute/wave-3.sql --wave-four-sql /absolute/wave-4.sql --wave-five-sql /absolute/wave-5.sql --evidence-dir /absolute/path");
  const target = validateScratchTarget(databaseUrl);
  const [waveOneSql, waveTwoSql, waveThreeSql, waveFourSql, waveFiveSql] = await Promise.all([waveOnePath, waveTwoPath, waveThreePath, waveFourPath, waveFivePath].map((filePath) => readFile(resolve(filePath), "utf8")));
  ...
  try {
    ...
    if (existing.length) throw new Error("Gate C Wave 5 requires an empty scratch database.");
    const replayStatements = [
      ...
      ...waveFive.statements.map((statement) => ({ wave: "Wave 5", statement })),
    ];
    ...
    const expected = [...PREREQUISITE_TABLES, ...WAVE_FIVE_TABLES].sort();
    if (JSON.stringify(tableNames) !== JSON.stringify(expected)) throw new Error(`Gate C Wave 5 scratch metadata has an unexpected table set: ${tableNames.join(", ")}.`);
    await mkdir(resolve(evidenceDir), { recursive: true });
    const metadata = { target, tables, constraints, indexes };
    const replay = {
      scope: "Gate C Wave 5 local loopback-only composed scratch replay", target,
      waveOneStatementCount: waveOneStatements.length, waveTwoStatementCount: waveTwoStatements.length, waveThreeStatementCount: waveThree.statements.length, waveFourStatementCount: waveFour.statements.length, waveFiveStatementCount: waveFive.statements.length,
      waveOneSqlSha256: sha256(waveOneSql), waveTwoSqlSha256: sha256(waveTwoSql), waveThreeSqlSha256: sha256(waveThreeSql), waveFourSqlSha256: sha256(waveFourSql), waveFiveSqlSha256: sha256(waveFiveSql),
    ...
    await writeFile(resolve(evidenceDir, "sql.sha256"), `${replay.waveFiveSqlSha256}  ${resolve(waveFivePath)}\n`);
