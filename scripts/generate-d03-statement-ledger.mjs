...
if (!sourceArg || !ledgerArg || !['generate', 'verify'].includes(mode)) {
  console.error('Usage: node scripts/generate-d03-statement-ledger.mjs <generate|verify> <source.sql> <ledger.json> [summary.md] [statements-dir]');
  process.exit(2);
}
...
function buildLedger() {
  ...
  if (markerCount !== 187) throw new Error(`Expected 187 markers; found ${markerCount}`);
  if (!trailingMarker) throw new Error('Expected the Wave 3 source to end with a literal statement-breakpoint marker');
  if (fragments.length !== 187) throw new Error(`Expected 187 non-empty marker-separated fragments; found ${fragments.length}`);
  ...
  if (counts['CREATE TABLE'] !== 50 || counts['ADD FOREIGN KEY'] !== 26 || counts['CREATE INDEX'] !== 111) {
    throw new Error(`Unexpected statement classes: ${JSON.stringify(counts)}`);
  }

  return {
    schema: 'kinga-d03-statement-ledger/v1',
...
function renderSummary(ledger) {
  ...
  return `# D-03 Wave 3 Marker-Split Statement Hash Ledger\n\n` +
    `This manifest is generated deterministically from \`${ledger.source_file}\`. The source file is immutable; it is split only on the exact literal \`${ledger.marker_literal}\`, each fragment is UTF-8 trimmed, and the source's one empty terminal fragment is discarded because Wave 3 ends with a marker. Each emitted statement retains its terminal semicolon and is hashed with SHA-256. The JSON companion ledger contains the exact full SQL for every row.\n\n` +
    `| Control | Value |\n|---|---|\n` +
    ...
    '```bash\n' +
    `node scripts/generate-d03-statement-ledger.mjs verify ${ledger.source_file} audit/gate-d-d03-statement-hash-ledger-2026-09-13.json\n` +
    '```\n';
}
...
if (mode === 'generate') {
  ...
  console.log(JSON.stringify({
    ...
    trailing_marker: freshLedger.trailing_marker,
