...
if (!liveCsvArg || sourceArgs.length === 0) {
  console.error("Usage: node scripts/verify-d03-postflight-foreign-keys.mjs <live.csv> <wave-1.sql> <wave-2.sql> <revised-wave-3.sql>");
  process.exit(2);
}
...
const expectedRows = sourceArgs.flatMap((arg) => sourceRows(readFileSync(resolve(arg), "utf8")));
...
const result = {
  status: missing.length === 0 && unexpected.length === 0 && expectedRows.length === 35 && liveRows.length === 35 ? "PASS" : "FAIL",
