...
import { splitStatements, validateWaveThreeSql } from "./gate-c-wave-three-contract.mjs";

async function main() {
  const sqlPath = process.argv[2];
  if (!sqlPath) throw new Error("Usage: node scripts/verify-gate-c-wave-three-identifiers.mjs /absolute/wave-3.sql");
  const sql = await readFile(sqlPath, "utf8");
  const inspected = validateWaveThreeSql(sql);
  const names = [];
  for (const statement of splitStatements(sql)) {
    const index = statement.match(/^CREATE\s+(?:UNIQUE\s+)?INDEX\s+`([^`]+)`/i);
  ...
  if (invalid.length || duplicateNames.length) throw new Error(`Invalid Wave 3 identifiers: invalid=${JSON.stringify(invalid)}, duplicate=${JSON.stringify([...new Set(duplicateNames)])}`);
  console.log(JSON.stringify({ tableCount: inspected.createdTables.length, indexCount: names.filter(({ kind }) => kind === "index").length, foreignKeyCount: names.filter(({ kind }) => kind === "foreign_key").length, status: "passed" }, null, 2));
}
