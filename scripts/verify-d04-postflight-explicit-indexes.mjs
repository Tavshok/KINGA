...
if (!liveCsvArg || sourceArgs.length !== 4) {
  console.error("Usage: node scripts/verify-d04-postflight-explicit-indexes.mjs <live.csv> <wave-1.sql> <wave-2.sql> <revised-wave-3-ledger.json> <corrected-wave-4.sql>");
  process.exit(2);
}
...
function explicitIndexes(statements) {
...
const expectedBySource = Object.fromEntries(sourceArgs.map((path) => [basename(path), explicitIndexes(statementText(path))]));
const expected = Object.values(expectedBySource).flat();
const expectedSet = new Set(expected.map(definition));
const liveMatching = [...liveByIdentity.values()].filter((entry) => expectedSet.has(definition(entry)));
...
const definitionMismatches = [...liveByIdentity.values()]
...
const result = {
  status: missing.length === 0 && definitionMismatches.length === 0 && expected.length === 264 ? "PASS" : "FAIL",
  live_non_primary_index_column_rows: liveRows.length,
  expected_explicit_table_index_pairs: expected.length,
  live_matching_explicit_table_index_pairs: liveMatching.length,
