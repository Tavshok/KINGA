...
if (!sourceArg || !ledgerArg || !['generate', 'verify'].includes(mode)) {
  console.error('Usage: node scripts/generate-d02-statement-ledger.mjs <generate|verify> <source.sql> <ledger.json>');
  process.exit(2);
}
...
function buildLedger() {
  ...
  const fragments = sourceText.split(marker).map((fragment) => fragment.trim());

  if (markerCount !== 86) throw new Error(`Expected 86 markers; found ${markerCount}`);
  if (fragments.length !== 87) throw new Error(`Expected 87 marker-separated fragments; found ${fragments.length}`);
  ...
  if (counts['CREATE TABLE'] !== 20 || counts['ADD FOREIGN KEY'] !== 9 || counts['CREATE INDEX'] !== 58) {
    throw new Error(`Unexpected statement classes: ${JSON.stringify(counts)}`);
  }

  return {
    schema: 'kinga-d02-statement-ledger/v1',
    source_file: sourceArg,
    source_sha256: sha256(sourceText),
    marker_literal: marker,
    marker_count: markerCount,
    split_rule: 'Split on the exact literal marker, then UTF-8 trim each fragment. The emitted SQL is the resulting fragment, including its terminal semicolon.',
...
function renderSummary(ledger) {
  ...
  return `# D-02 Wave 2 Marker-Split Statement Hash Ledger\n\n` +
    `This manifest is generated deterministically from \`${ledger.source_file}\`. The source file is immutable; it is split only on the exact literal \`${ledger.marker_literal}\`, then each resulting UTF-8 fragment is trimmed. Each emitted statement retains its terminal semicolon and is hashed with SHA-256. The JSON companion ledger contains the exact full SQL for every row.\n\n` +
    `| Control | Value |\n|---|---|\n` +
    `| Full source SHA-256 | \`${ledger.source_sha256}\` |\n` +
    `| Marker count | ${ledger.marker_count} |\n` +
    ...
    '```bash\n' +
    `node scripts/generate-d02-statement-ledger.mjs verify ${ledger.source_file} audit/gate-d-d02-statement-hash-ledger-2026-09-12.json\n` +
    '```\n';
}
...
function writeStatementFiles(ledger, statementsDir) {
  ...
  for (const statement of ledger.statements) {
    ...
    if (writtenHash !== statement.sha256) {
      throw new Error(`Statement-file hash mismatch for ${outputPath}`);
    }
  }
...
if (mode === 'generate') {
  ...
  if (summaryArg) {
    writeFileSync(resolve(summaryArg), renderSummary(freshLedger), 'utf8');
  }
  const statementsDir = statementsDirArg ? writeStatementFiles(freshLedger, statementsDirArg) : null;
  console.log(JSON.stringify({
    mode,
    source_sha256: freshLedger.source_sha256,
    marker_count: freshLedger.marker_count,
    statement_count: freshLedger.statement_count,
    statement_counts: freshLedger.statement_counts,
    ledger_path: ledgerPath,
    summary_path: summaryArg ? resolve(summaryArg) : null,
    statements_dir: statementsDir,
  }, null, 2));
} else {
  const recordedLedger = JSON.parse(readFileSync(ledgerPath, 'utf8'));
  const exactMatch = JSON.stringify(recordedLedger) === JSON.stringify(freshLedger);
  if (!exactMatch) {
