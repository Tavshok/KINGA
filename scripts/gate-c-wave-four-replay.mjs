#!/usr/bin/env node
/** Gate C Wave 4 loopback-only composed scratch replay runner. */
...
import { PREREQUISITE_TABLES, WAVE_FOUR_TABLES, validateWaveFourSql } from "./gate-c-wave-four-contract.mjs";
...
export function validateScratchTarget(databaseUrl) {
  ...
  if (target.protocol !== "mysql:") throw new Error("Gate C Wave 4 accepts only mysql:// scratch URLs.");
  if (!ALLOWED_HOSTS.has(target.hostname)) throw new Error("Gate C Wave 4 rejects non-loopback database targets.");
  const database = target.pathname.replace(/^\//, "");
  if (!/^kinga_gatec_[a-z0-9_]+$/i.test(database)) throw new Error("Gate C Wave 4 requires a uniquely named kinga_gatec_* database.");
...
async function main() {
  ...
  if (!databaseUrl || !waveOnePath || !waveTwoPath || !waveThreePath || !waveFourPath || !evidenceDir) throw new Error("Usage: --database-url mysql://... --wave-one-sql /absolute/wave-1.sql --wave-two-sql /absolute/wave-2.sql --wave-three-sql /absolute/wave-3.sql --wave-four-sql /absolute/wave-4.sql --evidence-dir /absolute/path");
  ...
  const waveFourSql = await readFile(resolve(waveFourPath), "utf8");
  ...
  try {
    ...
    if (existing.length) throw new Error("Gate C Wave 4 requires an empty scratch database.");
    const replayStatements = [
      ...
      ...waveFour.statements.map((statement) => ({ wave: "Wave 4", statement })),
    ];
    ...
    const expected = [...PREREQUISITE_TABLES, ...WAVE_FOUR_TABLES].sort();
    if (JSON.stringify(tableNames) !== JSON.stringify(expected)) throw new Error(`Gate C Wave 4 scratch metadata has an unexpected table set: ${tableNames.join(", ")}.`);
    await mkdir(resolve(evidenceDir), { recursive: true });
    const metadata = { target, tables, constraints, indexes };
    const replay = {
      scope: "Gate C Wave 4 local loopback-only composed scratch replay",
      target,
      ...
      waveFourStatementCount: waveFour.statements.length,
      waveOneSqlSha256: sha256(waveOneSql), waveTwoSqlSha256: sha256(waveTwoSql), waveThreeSqlSha256: sha256(waveThreeSql), waveFourSqlSha256: sha256(waveFourSql),
    ...
    await writeFile(resolve(evidenceDir, "sql.sha256"), `${replay.waveFourSqlSha256}  ${resolve(waveFourPath)}\n`);
