...
describe("P0 Vehicle Registry cross-tenant containment", () => {
  ...
  beforeAll(async () => {
    ...
    if (!connection)
      throw new Error(
        "Database unavailable for Vehicle Registry containment regression"
      );
    db = connection;
    ...
    await db.insert(users).values([
      {
        openId: openIdA,
        email: `${openIdA}@invalid.example`,
        name: "Vehicle registry tenant A fixture",
        role: "agency",
        tenantId: tenantA,
        emailVerified: 1,
      },
      {
        openId: openIdB,
        email: `${openIdB}@invalid.example`,
        name: "Vehicle registry tenant B fixture",
        role: "agency",
        tenantId: tenantB,
        emailVerified: 1,
      },
    ]);
    userAId = await resolveId(
      db.select({ id: users.id }).from(users).where(eq(users.openId, openIdA)),
      "tenant A user"
    );
    userBId = await resolveId(
      db.select({ id: users.id }).from(users).where(eq(users.openId, openIdB)),
      "tenant B user"
    );
  ...
  afterAll(async () => {
    if (!db) return;
    if (containmentEventIds.length)
      await db
        .delete(claimEvents)
        .where(inArray(claimEvents.id, containmentEventIds));
    if (tenantBClaimIds.length)
      await db.delete(claims).where(inArray(claims.id, tenantBClaimIds));
    if (vehicleAId)
      await db
        .delete(vehicleRegistry)
        .where(eq(vehicleRegistry.id, vehicleAId));
    ...
    const remaining = await Promise.all([
      containmentEventIds.length
        ? db
            .select({ id: claimEvents.id })
            .from(claimEvents)
            .where(inArray(claimEvents.id, containmentEventIds))
        : [],
      tenantBClaimIds.length
        ? db
            .select({ id: claims.id })
            .from(claims)
      ...
      vehicleAId
        ...
        : [],
      userBId
        ? db.select({ id: users.id }).from(users).where(eq(users.id, userBId))
        : [],
      userAId
        ? db.select({ id: users.id }).from(users).where(eq(users.id, userAId))
        : [],
    ]);
  ...
  it("contains both VIN and registration matches rather than mutating tenant A’s vehicle or attaching tenant B’s claims", async () => {
    ...
    const tenantBVehicleRows = await db
      .select({ id: vehicleRegistry.id })
      .from(vehicleRegistry)
      .where(
        and(
    ...
    const incomingClaims = await db
      .select({ id: claims.id, vehicleRegistryId: claims.vehicleRegistryId })
      .from(claims)
      .where(inArray(claims.id, tenantBClaimIds));
    ...
    const events = await db
      .select({
        ...
        tenantId: claimEvents.tenantId,
        userRole: claimEvents.userRole,
        eventType: claimEvents.eventType,
        eventPayload: claimEvents.eventPayload,
      })
      .from(claimEvents)
    ...
    const normalizedEvents = events.map(event => ({
      ...event,
      eventPayload:
        typeof event.eventPayload === "string"
          ? JSON.parse(event.eventPayload)
          : event.eventPayload,
    }));
    containmentEventIds.push(...normalizedEvents.map(event => event.id));
      ...
      expect.arrayContaining([
        expect.objectContaining({
          ...
          eventPayload: expect.objectContaining({
            matchedBy: "vin",
            existingVehicleRegistryId: vehicleAId,
