 ...
 * ────────────────────
 * The LLM prompts in assessment-processor.ts, cross-validation.ts, and
 * quoteExtractionEngine.ts each contain a hardcoded list of canonical part
 * names. If VEHICLE_PARTS is updated (new part added, name changed, part
 * removed) without updating the prompts, the hallucination guard will start
 * rejecting valid AI output — or the prompts will allow names that no longer
...
import { VEHICLE_PARTS } from "../shared/vehicleParts";
