 ...
 * ENVIRONMENT-DEPENDENT: they require a real assessment PDF to be present at
 * the path below before they will execute.
 *
 * Classification : Legitimate integration tests — NOT obsolete, NOT broken.
 * Why skipped    : The PDF fixture file is not committed to the repository
 *                  (it is a confidential client document) and is therefore
 *                  absent from the CI sandbox and standard developer machines.
 * How to enable  : Place the original assessment PDF at:
 *
 *   /home/ubuntu/upload/ZPCTOYOTAHILUXAGA2795ASSESSMENTREPORT_07_Nov_2024-140814-audit-signed.pdf
 *
 *                  Once the file is present, both tests will run automatically
 *                  on the next `pnpm test` invocation — no code changes needed.
 * What they test : The `scripts/extract-pdf-photos.py` pipeline that filters
 *                  photo pages from a multi-page assessment PDF and outputs
 *                  11 `damage-photo-XXX.png` files from a 20-page document.
 * ────────────────────────────────────────────────────────────────────────────
 */
...
describe("External Assessment Upload", () => {
  ...
  // SKIPPED — environment-dependent fixture test.
  // Requires the confidential PDF fixture at PDF_PATH (see file-level comment above).
  ...
  // SKIPPED — environment-dependent fixture test.
  // Requires the confidential PDF fixture at PDF_PATH (see file-level comment above).
  // Validates that scripts/extract-pdf-photos.py correctly isolates 11 photo pages
  // from the 20-page assessment PDF and writes them as damage-photo-XXX.png files.
