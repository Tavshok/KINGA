...
describe("Platform Super Admin Access Control", () => {
  ...
  beforeAll(async () => {
    ...
    await db
      ...
      .onDuplicateKeyUpdate({ set: { name: "Test Tenant 1" } });
    await db
      .insert(insurerTenants)
      .values({
        id: testTenant2Id,
      ...
      .onDuplicateKeyUpdate({ set: { name: "Test Tenant 2" } });
  ...
  it("should log all platform super admin accesses to audit trail", async () => {
    const db = await getDb();
    const { logPlatformSuperAdminAccess } = await import(
  ...
  it("should enforce read-only access through middleware", async () => {
    ...
    const { platformSuperAdminGuard } = await import(
      "./core/platform-super-admin-guard"
    );
    ...
    await expect(queryResult).resolves.toBeDefined();
    ...
    await expect(mutationResult).rejects.toThrow(
      "Platform super admins have read-only access"
    );
  });

  it("should bypass tenant filtering for platform super admin", async () => {
    const { shouldBypassTenantFilter, getTenantFilterClause } = await import(
      "./core/platform-super-admin-guard"
    );
    ...
    expect(
      getTenantFilterClause("platform_super_admin", "any-tenant-id")
    ).toBeUndefined();
