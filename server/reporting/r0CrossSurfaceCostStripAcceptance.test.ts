...
describe("Approved R0-H/R0-I cross-surface cost-strip acceptance", () => {
  ...
  it("keeps an incomplete L2 visible as a review-required priced scope, never a complete payable quote", () => {
    ...
    expect(presentation).toMatchObject({
      ...
      optimisedQuoteAmount: 990,
      optimisedQuoteDetail:
  ...
  it("keeps all CL, CI, FR and client top strips on the active-ledger and typed L2 boundary", () => {
    ...
    const sharedPresentation = read(
      "server/reporting/costDecisionPresentation.ts"
    );
    expect(sharedPresentation).toContain("Submitted Quotations");
    expect(sharedPresentation).toContain("KINGA Optimised Quote");
    expect(read("server/reporting/claimsIntelligenceReport.ts")).toContain(
      "l2LedgerLabel"
    );
    expect(read("server/reporting/forensicDecisionReport.ts")).toContain(
      "l2LedgerLabel"
    );
  ...
  it("guards CI and FR savings and settlement conclusions behind a complete KINGA Optimised Quote", () => {
    ...
    expect(ci).toContain(
      "No savings or settlement figure is calculated from an incomplete L2."
    );
    expect(fr).toContain(
      "const hasSavings = savings > 0 && kingaOptimised !== null"
    );
    expect(fr).toContain(
