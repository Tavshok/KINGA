...
import {
  buildCostDecisionPresentation,
  extractExplicitStructuralReviewEvidence,
  resolveRepairabilityVerdict,
} from "./costDecisionPresentation";
...
function cost(
  overrides: Partial<ReportCostIntegrity> = {}
): ReportCostIntegrity {
  return {
    activeQuotes: [
      {
        repairer: "Repairer North",
        amountUsd: 4280,
        currency: "USD",
        status: "active",
        sourceReference: "quote-n",
        statusReason: "Active",
      },
      {
        repairer: "Repairer Central",
        amountUsd: 3890,
        currency: "USD",
        status: "active",
        sourceReference: "quote-c",
        statusReason: "Active",
      },
      {
        repairer: "Repairer South",
        amountUsd: 4010,
        currency: "USD",
        status: "active",
        sourceReference: "quote-s",
        statusReason: "Active",
      },
    ],
    sourceQuoteCount: 3,
    ...
    quoteReconciliations: [
      {
        repairer: "Repairer North",
        quoteId: "quote-n",
        submittedHeaderTotalUsd: 4280,
        submittedItemisedTotalUsd: 4280,
        unexplainedResidualUsd: null,
        residualCategory: null,
        status: "reconciled",
      },
      {
        repairer: "Repairer Central",
        quoteId: "quote-c",
        submittedHeaderTotalUsd: 3890,
        submittedItemisedTotalUsd: 3890,
        unexplainedResidualUsd: null,
        residualCategory: null,
        status: "reconciled",
      },
      {
        repairer: "Repairer South",
        quoteId: "quote-s",
        submittedHeaderTotalUsd: 4010,
        submittedItemisedTotalUsd: 4010,
        unexplainedResidualUsd: null,
        residualCategory: null,
        status: "reconciled",
      },
...
const canonicalPhotos = [
  {
    url: "https://evidence.example/front-left.jpg",
    impactZone: "front-left",
    severity: "severe",
    confidenceScore: 0.94,
    detectedComponents: ["bumper", "headlamp"],
    sourcePage: 2,
  },
  {
    url: "https://evidence.example/rear.jpg",
    impactZone: "rear",
    severity: "moderate",
    confidenceScore: 0.86,
    detectedComponents: ["bumper rear"],
    sourcePage: 3,
  },
  {
    url: "https://evidence.example/rear.jpg",
...
describe("R0 immutable combined rendered-report acceptance", () => {
  it("presents one complete verified evidence state without independently recomputing cost or photo intelligence", () => {
    ...
    const repairability = resolveRepairabilityVerdict({
      totalLossIndicated: false,
      repairToValueRatio: 42,
    });
    ...
    expect(photos).toMatchObject({
      totalPhotos: 2,
      usablePhotos: 2,
      unclassifiedZoneCount: 0,
      directionConflictCount: 0,
    });
    expect(photos.photos.map(photo => photo.zone)).toEqual([
      "front-left",
      "rear",
    ]);
  });
  ...
  it("retains submitted evidence and suppresses only unsupported all-in conclusions for a named scope gap", () => {
    const decision = buildCostDecisionPresentation(
      cost({
    ...
    expect(decision).toMatchObject({
      ...
      optimisedQuoteAmount: 3290,
      quoteIssue: "Missing submitted price: Front bumper garnish.",
    });
    ...
    expect(decision.optimisedQuoteDetail).toContain(
      "Review-required priced submitted component scope"
    );
  });
        ...
        quoteReconciliations: [
          {
            ...
            quoteId: "quote-n",
            submittedHeaderTotalUsd: 4280,
            submittedItemisedTotalUsd: 3980,
            unexplainedResidualUsd: 300,
            residualCategory: "source_to_ledger_reconciliation",
            status: "reconciliation_required",
          },
        ],
      })
    );
    expect(decision.quoteIssue).toBe(
      "Repairer North: submitted total requires line-item reconciliation."
    );
  ...
  it("renders structural review only from explicit persisted evidence and retains photo uncertainty without relabelling", () => {
    const repairEvidence = extractExplicitStructuralReviewEvidence({
      structuralReviewRequired: true,
      structuralReviewRationale:
        "Front rail alignment measurement is required.",
    });
    expect(
      resolveRepairabilityVerdict({
        totalLossIndicated: false,
      ...
      label: "Further structural review required",
      detail: "Front rail alignment measurement is required.",
    });
    const photos = normaliseCanonicalPhotoEvidence([
      {
        url: "https://evidence.example/unclassified.jpg",
        confidenceScore: 0.83,
      },
    ]);
    expect(photos.photos[0]).toMatchObject({
      zone: undefined,
      uncertainty: "zone_unclassified",
    });
  });

  it("uses shared cost and P0-safe photo contracts in CL, CI, and FR", () => {
    for (const file of [
      ...
      expect(source).toContain("projectP0A2DescriptivePhotoEvidence");
      expect(source).not.toContain("extractExplicitStructuralReviewEvidence");
    }
    ...
    expect(forensicRenderer).toContain("projectP0A2DescriptivePhotoEvidence");
    expect(forensicRenderer).not.toContain(
      "extractExplicitStructuralReviewEvidence"
    );
