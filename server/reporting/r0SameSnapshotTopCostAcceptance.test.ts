...
describe("Approved R0 same-snapshot top-cost contract acceptance", () => {
  it.each(R0_QUOTE_EVIDENCE_FIXTURES)("projects the approved evidence state for $name without inferred payable amounts", (fixture) => {
    ...
    expect(presentation.optimisedQuoteAmount).toBe(fixture.expected.comparisonUsd);
  ...
  it("keeps a concrete reconciliation issue separate from the evidence-qualified comparison amount", () => {
    ...
    expect(presentation).toMatchObject({ quoteVerification: "RECONCILIATION REQUIRED", optimisedQuoteState: "evidence_qualified", optimisedQuoteAmount: 990, quoteIssue: "Repairer Alpha: submitted total requires line-item reconciliation." });
  });
