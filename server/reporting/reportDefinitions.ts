...
async function generateClaimsManagerPortfolioReport(params: Record<string, unknown>, tenantId?: string): Promise<string> {
  ...
  const body = `<div class="section"><div class="section-title">Claims Portfolio & Processing</div><div class="kv-grid cols-4"><div class="kv-item"><div class="kv-label">Total Claims</div><div class="kv-value bold">${p.totalClaims}</div></div><div class="kv-item"><div class="kv-label">In Progress</div><div class="kv-value">${p.inProgressCount}</div></div><div class="kv-item"><div class="kv-label">Approved</div><div class="kv-value">${p.approvedCount}</div></div><div class="kv-item"><div class="kv-label">AI Estimated Portfolio Value</div><div class="kv-value">${fmtCurrency(p.aiEstimatedValueUsd)}</div></div></div></div><div class="section"><div class="section-title">Operational Ageing</div><table><thead><tr><th>Status</th><th>Claims</th><th>Average elapsed time</th><th>Maximum elapsed time</th></tr></thead><tbody>${aggregate.dwellTimeByStatus.map(r => `<tr><td>${escHtml(r.status)}</td><td>${r.claimCount}</td><td>${r.averageElapsedHours.toFixed(1)} hours</td><td>${r.maximumElapsedHours.toFixed(1)} hours</td></tr>`).join("") || "<tr><td colspan='4'>No data for selected period</td></tr>"}</tbody></table></div>`;
