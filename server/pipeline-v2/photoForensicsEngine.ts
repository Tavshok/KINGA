...
import { computeThumbnailHash } from "./imageIntelligence";
...
export interface PhotoForensicsSummary {
  ...
  /** T5: True when ALL analysed photos are missing EXIF metadata. */
  exifAbsent: boolean;
  /** T3: Per-photo perceptual hashes (64-char binary strings). Null entries = hash failed. */
  pHashes: (string | null)[];
  /** T6: Average AI-generation likelihood score (0-1) across all analysed photos. Null when vision not run. */
  aiGenerationScore: number | null;
  /** T6: True when aiGenerationScore >= 0.6. */
  aiGenerationFlag: boolean;
}
...
interface RawAnalysisResult {
  ...
  /** T5: True when this photo has no EXIF metadata (< 3 fields extracted). */
  exif_absent?: boolean;
  /** T6: AI-generation likelihood score (0-1) for this photo. Null when vision was not run. */
  ai_generation_score?: number | null;
  error?: string;
}
...
async function runNodeForensics(buffer: Buffer): Promise<RawAnalysisResult> {
  ...
  // T5: exifAbsent — true when fewer than 3 EXIF fields were extracted
  const exifAbsent = Object.keys(exifData).length < 3;
...
interface VisionClassification {
  ...
  /** T6: AI-generation likelihood score (0-1). Null when not parsed. */
  aiGenerationScore?: number | null;
}
...
7. AI_GENERATION_SCORE: Rate the likelihood this image was generated by an AI image generator (0.0 = definitely real photograph, 1.0 = definitely AI-generated). Consider: unnatural lighting uniformity, perfect symmetry, texture inconsistencies, background incoherence, unrealistic reflections. Write: AI_GENERATION_SCORE: [0.00-1.00]
...
Be concise but thorough. Flag any fraud indicators clearly.`,
    ...
    if (typeMatch) {
      ...
      // T6: Parse AI_GENERATION_SCORE
      const aiGenMatch = text.match(/AI_GENERATION_SCORE\s*:\s*(\d+(?:\.\d+)?)/i);
      const aiGenerationScore = aiGenMatch ? Math.min(1, Math.max(0, parseFloat(aiGenMatch[1]))) : null;
      return { isNonVehicle: false, description: analysisText || text, crushDepthCm, aiGenerationScore };
    }
  ...
  try {
    ...
    if (runVision) {
      ...
      if (vision) {
        ...
        // T6: store per-photo AI generation score
        if (vision.aiGenerationScore != null) {
          result.ai_generation_score = vision.aiGenerationScore;
        }
      }
    }
...
export async function runPhotoForensics(
  ...
  if (photoUrls.length === 0) {
    return {
      ...
      exifAbsent: false,
      pHashes: [],
      aiGenerationScore: null,
      aiGenerationFlag: false,
    };
  }
  ...
  // ── T3: Compute perceptual hashes for all photos (non-blocking) ───────────
  const pHashes: (string | null)[] = await Promise.all(
    photos.map(p => (p.analysisResult && !p.error) ? computeThumbnailHash(p.url) : Promise.resolve(null))
  );
  ...
  // ── T5: exifAbsent aggregate ─────────────────────────────────────────────────
  const exifAbsentAll = analysedCount > 0 && photos
    .filter(p => p.analysisResult && !p.error)
    .every(p => p.analysisResult!.exif_absent === true);

  // ── T6: aiGenerationScore aggregate ──────────────────────────────────────
  const AI_GENERATION_FLAG_THRESHOLD = 0.6;
  const aiGenScores = photos
    .filter(p => p.analysisResult?.ai_generation_score != null)
    .map(p => p.analysisResult!.ai_generation_score as number);
  const aiGenerationScore = aiGenScores.length > 0
    ? aiGenScores.reduce((s, v) => s + v, 0) / aiGenScores.length
    : null;
  const aiGenerationFlag = aiGenerationScore != null && aiGenerationScore >= AI_GENERATION_FLAG_THRESHOLD;

  // Add AI generation fraud indicator if flagged
  if (aiGenerationFlag && aiGenerationScore != null) {
    indicators.push({
      indicator: "photo_ai_generated",
      category: "photo_forensics",
      score: Math.round(aiGenerationScore * 30),
      description: `AI-generation analysis scored ${(aiGenerationScore * 100).toFixed(0)}% likelihood that submitted photos are AI-generated rather than real photographs.`,
    });
  }
