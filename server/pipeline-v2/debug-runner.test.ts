...
/**
 * Unit tests for the Pipeline Debug diagnostic report structure.
 * 
 ...
 * and that the system health computation logic is sound.
 */
...
function createMinimalReport(overrides: Partial<DebugDiagnosticReport> = {}): DebugDiagnosticReport {
  return {
    ...
    stageSummaries: {
      "1_ingestion": { status: "success", durationMs: 500 },
      "2_extraction": { status: "success", durationMs: 8000 },
      "3_structured_extraction": { status: "success", durationMs: 6000 },
      "4_validation": { status: "success", durationMs: 200 },
      "5_assembly": { status: "success", durationMs: 100 },
      "6_damage_analysis": { status: "success", durationMs: 3000 },
      "7_physics": { status: "success", durationMs: 2000 },
      "8_fraud": { status: "success", durationMs: 2500 },
      "9_cost": { status: "success", durationMs: 2000 },
      "10_report": { status: "success", durationMs: 700 },
    },
    ...overrides,
  };
}
...
describe("System health computation", () => {
  ...
  it("should report degraded when some engines fail", () => {
    const report = createMinimalReport({
      systemHealth: {
        dataExtractionCompleteness: 70,
        engineSuccessRate: 75,
        totalEngines: 4,
        successfulEngines: 3,
        failedEngines: 1,
        skippedEngines: 0,
    ...
    expect(report.systemHealth.failedEngines).toBe(1);
    expect(report.systemHealth.engineSuccessRate).toBeLessThan(100);
  });

  it("should report critical when most engines fail", () => {
    const report = createMinimalReport({
      systemHealth: {
  ...
  it("should have engine counts that add up", () => {
    ...
    expect(h.successfulEngines + h.failedEngines + h.skippedEngines).toBe(h.totalEngines);
  });
      ...
      errorDiagnostics: [
        {
          ...
          description: "Physics engine failed because vehicle mass was not extracted.",
