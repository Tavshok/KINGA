...
export interface Stage7Output {
   ...
   * - 'SKIPPED_INSUFFICIENT_GEOMETRY' — no qualifying VGE/VGR calibrated crush measurement
   * - 'SKIPPED_ENGINE_FAILURE' — calibrated geometry existed but physics did not complete; no numerical fallback was produced
   ...
   */
  physicsStatus: 'EXECUTED' | 'SKIPPED_NO_SPEED' | 'SKIPPED_INSUFFICIENT_GEOMETRY' | 'SKIPPED_ENGINE_FAILURE' | 'SKIPPED_NON_PHYSICAL' | 'ESTIMATED_FALLBACK';
...
export interface PipelineContext {
  ...
  /**
   * Unique run identifier (UUID) for this pipeline execution.
   * Generated by the caller before invoking runPipelineV2.
   * Used by the observability layer to group all stage records for this run.
   */
  runId?: string;

  /**
   * Observability callback — fired after each stage completes.
   * Non-blocking: the orchestrator does not await this callback.
   * Errors inside the callback must not propagate to the pipeline.
   */
  onStageComplete?: (stageId: string, result: {
    durationMs: number;
