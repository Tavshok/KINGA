...
async function waitForSimulatedAssessment(
  ...
  while (Date.now() < deadline) {
    const [claim] = await db
      ...
      .limit(1);
    if (claim?.status === "human_review_required") return claim;
    await new Promise(resolve => setTimeout(resolve, 25));
  }
  throw new Error(
    `Timed out waiting for isolated upload claim ${claimId} to reach the local pipeline boundary`
  );
}
...
describe("isolated authenticated upload-to-report operational acceptance", () => {
  ...
  beforeAll(async () => {
    ...
    if (!connection)
      throw new Error("Database is unavailable for isolated upload OAT");
    db = connection;

    const stamp = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    tenantId = `test-upload-oat-${stamp}`;
    uploaderOpenId = `oat-upload-user-${stamp}`;
    uploaderEmail = `oat-upload-${stamp}@invalid.example`;
    const [userInsert] = await db.insert(users).values({
      openId: uploaderOpenId,
      email: uploaderEmail,
    ...
    sessionCookie = "test-local-session";
    ...
    if (!address || typeof address === "string")
      throw new Error("Unable to establish isolated upload test server");
    origin = `http://127.0.0.1:${address.port}`;
  });

  beforeEach(() => {
    vi.spyOn(console, "log").mockImplementation(() => undefined);
    mocks.createSessionToken.mockResolvedValue("test-local-session");
  ...
  it("creates canonical upload records, reaches a simulated local pipeline boundary, and renders CL CI FR", async () => {
    const requestBody = new FormData();
    requestBody.append(
      "files",
      new Blob([ONE_PIXEL_PNG], { type: "image/png" }),
      "synthetic-oat-evidence.png"
    );
    ...
    const [document] = await db
      ...
      .where(
        and(
          eq(ingestionDocuments.id, payload.documents[0]!.document_db_id),
    ...
    const completedClaim = await waitForSimulatedAssessment(
      db,
      claimId,
      tenantId
    );
    ...
    const [syntheticVehicle] = await db
  ...
  afterAll(async () => {
    ...
    if (syntheticVehicleRegistryId) {
      await db
        ...
        .where(
          and(
            eq(vehicleRegistry.id, syntheticVehicleRegistryId),
            eq(vehicleRegistry.tenantId, tenantId)
          )
        );
    }
    await db
      .delete(ingestionDocuments)
      .where(eq(ingestionDocuments.tenantId, tenantId));
    await db
